...
<html>
	<head>
		<title>DATO MAESTRO - EDITAR</title>
		<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">		
		<script languaje='javaScript'>
			<!--
			var _accion = true;
			function hacerValidaciones_frmEditar(forma){
				if (_accion){ 					
						...
						case "28":
			  			  validarLista(forma.lista1, "- Área", 1)
								if (trim(forma.texto2.value).length == 0)
										forma.texto2.value=forma.nombre.value;
					...
					case "30":	
							...
							validarLista(forma.lista2, "- Asignatura", 1)
								if (trim(forma.texto3.value).length == 0)
										forma.texto3.value=forma.nombre1.value;
					...
					case "36":	
						...
						case "38":
							...
							validarCampo(forma.texto5, "- Nombre", 1, 200)
							validarCampo(forma.texto6, "- Abreviatura [1-10]", 1,10)
							validarCampo(forma.texto7, "- Descripción ",1,200)
						break;	
					...
					switch(forma.dato.value){
						 ...
						 case "28":
							 if(n==1){  //lista1
								...
								forma.texto4.value= forma.texto2.value.substring(0,8);								
								forma.action='<c:url value="/datoMaestro/DatoMaestroEditarEdit.do?ext=1"/>';
								forma.submit();							
							}
						break;
			...
			function refrescar(frm,frm2){
				...
				function ayuda(){
	 			 remote = window.open("");
				 remote.location.href='<c:url value="/tutor/datosmaestros/actualizar.html"/>';
				}
			function boton(){				
				location.href="<c:url value="/datoMaestro/DatoMaestroNuevoEdit.do?ext=1"/>";	
			}
			function confirmar(){
...
<body onload="refrescar(frm,frmEditar)"><%@include file="mensaje.jsp"%>
	<form method="post" name="frm" action='<c:url value="/datoMaestro/DatoMaestroListaEdit.do?ext=1"/>'></form>
	<form method="post" name="frmEditar" action='<c:url value="/datoMaestro/DatoMaestroEditarSave.do?ext=1"/>' onSubmit='return validarForma(frmEditar)'>
		...
		<table border="0" align="center" bordercolor="#FFFFFF" width="100%">
			<caption>·.·.·.· Edición de <%=(String)request.getAttribute("titulo")%> ·.·.·.·</caption>
			<tr>
			  <td>
					<input class="boton" name="cmd" type="submit" value="Guardar">
					<input class="boton" name="cmd" type="submit" value="Eliminar" onClick="return confirmar()">
					<input class="boton" name="cmd3" type="button" value="Nuevo" onClick="boton()">
					<input class="boton" name="cmd4" type="button" value="Cancelar" onClick="boton()">
<!-- 					<input class="boton" id="cmd2" name="cmd5" type="button" style="display:" value="Ayuda" onClick ="ayuda()"> -->
			  </td>
			</tr>				
		</table>
		<table border="0" align="center" bordercolor="#FFFFFF" width="100%">
