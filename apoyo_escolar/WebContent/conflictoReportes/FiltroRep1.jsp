...
<!--
	...
	function buscar(){
		if(validarForma(document.frmFiltro)){
			...
			document.frmFiltro.tipo.value=3;
			...
			<table>
				...
				<tr>
					<td rowspan="2" width="780">
						<!-- <img src='<c:url value="/etc/img/tabs/reporte1_1.gif"/>' alt="Colegios que no han cerrado Conflicto Escolar" height="26" border="0">
		      ...
		      <td colspan="3">
		      	<select name="local1" class="entrada1" id="local1" onChange="">
			        ...
			        <c:forEach begin="0" items="${filtroLoc}" var="fila">
								<option value="<c:out value="${fila[0]}"/>" <c:if test="${sessionScope.filtroreportes.local1 == fila[0]}">SELECTED</c:if>>
	            ...
	            <c:forEach begin="0" items="${filtroPeriodo}" var="fila">
	        	    <option value="<c:out value="${fila[0]}"/>" <c:if test="${sessionScope.filtroreportes.periodo1== fila[0]}">SELECTED</c:if>><c:out value="${fila[1]}"/>
							</c:forEach>
	          </select>
	         </td> 
				</tr>
