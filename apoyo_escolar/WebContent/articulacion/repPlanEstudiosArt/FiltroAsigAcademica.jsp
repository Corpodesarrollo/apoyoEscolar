...
<jsp:useBean id="filtroAsig" class="articulacion.repPlanEstudiosArt.vo.FiltroBeanReporteAsignacion" scope="session"/><jsp:setProperty name="filtroAsig" property="*"/>
...
<c:import url="/mensaje.jsp"/>
	<form method="post" name="frm" onSubmit="return validarForma(frm)" action='<c:url value="/articulacion/repPlanEstudiosArt/FiltroGuardarAsignacion.jsp"/>'>
...
<!--//////////////////-->	
	<table border="0" align="center" width="100%">
		<tr height="1">
			<td rowspan="2" width="420" bgcolor="#FFFFFF"><a href="javaScript:lanzar(1)"><img src="../../etc/img/tabs/plan_de_estudios_0.gif" width="84"  height="26" border="0"></a><img src="../../etc/img/tabs/carga_academica_1.gif" width="84"  height="26" border="0"></td>
		</tr>
  </table>
					...
					<td colspan="3">
						<select name="sede" style='width:350px;' onChange='filtro(document.frm.sede,document.frm.jornada)'>
							...
							<c:forEach begin="0" items="${requestScope.lSedeVO}" var="sede">
								<option value="<c:out value="${sede.codigo}"/>" <c:if test="${sede.codigo==sessionScope.filtroAsig.sede}">selected</c:if>><c:out value="${sede.nombre}"/></option>
							</c:forEach>
						</select>
					</td>
				</tr>
				...
				<td>
					<select name="filAnoVigenciaArt" style='width:50px;'>
						...
						<c:forEach begin="0" items="${requestScope.VigenciaArtPlan}" var="vig">
							<option value="<c:out value="${vig}"/>" <c:if test="${vig==sessionScope.filtroAsig.filAnoVigenciaArt}">selected</c:if>><c:out value="${vig}"/></option>
						</c:forEach>
					</select>
					<select name="filPerVigenciaArt" style='width:30px;'>
						<option value="-99">--</option>
						<c:forEach begin="0" items="${requestScope.PeriodoArtPlan}" var="vig">
							<option value="<c:out value="${vig}"/>" <c:if test="${vig==sessionScope.filtroAsig.filPerVigenciaArt}">selected</c:if>><c:out value="${vig}"/></option>
						</c:forEach>
					</select>
				</td>			
...
<form method="post" name="frmAjax" action="<c:url value="/articulacion/repPlanEstudiosArt/ajaxRepArt.do"/>"><input type="hidden" name="ajax"><input type="hidden" name="ajax"><input type="hidden" name="ajax"><INPUT TYPE="hidden" NAME="tipo" VALUE='<c:out value="${paramsVO.FICHA_HORARIO_DOCENTE}"/>'><input type="hidden" name="cmd" value='<c:out value="${paramsVO.CMD_AJAX}"/>'></form>