...
<body onLoad="mensaje(document.getElementById('msg'));">
	<form action='<c:url value="/articulacion/artEncuesta/Save2.jsp"/>' method="post" name="frmFiltro" onSubmit="return validarForma(frmFiltro)">
		...
		<table cellpadding="0" cellspacing="0" border="0" ALIGN="CENTER" width="100%" >
			<tr height="1">
				...
				<img src='<c:url value="/etc/img/tabs/consolidado_num_estudiantes_1.gif"/>' alt="Consolidado # Estudiantes" height="26" border="0">					
		...
		<table border="0" align="center" width="95%" cellpadding="2" cellspacing="0">
			...
			<tr>
  			<td><b>Colegio:</b></td>
	  		<td colspan="3">
					<select name="plaInstitucion" onchange="ajaxSedes(document.frmFiltro.plaInstitucion,document.frmFiltro.plaSede,document.frmFiltro.plaJornada,document.frmFiltro.plaGrado,document.frmFiltro.plaGrupo)" style='width:250px;' <c:if test='${sessionScope.encuesta2VO.habilitarColegio==1}'>disabled</c:if>>
						<option value="-99">-Seleccione uno-</option>
						<c:forEach begin="0" items="${requestScope.listaColegiosVO}" var="col">
							<option value='<c:out value="${col.codigo}"/>' <c:if test="${col.codigo==sessionScope.encuesta2VO.plaInstitucion}">selected</c:if>><c:out value="${col.nombre}"/></option>
						</c:forEach>
					</select>
  			</td>
	  	</tr>
		 		...
		 		<td colspan="3">
					<select name="plaSede" onchange="ajaxJornadas(document.frmFiltro.plaSede,document.frmFiltro.plaJornada,document.frmFiltro.plaGrado,document.frmFiltro.plaGrupo)" style='width:250px;'>
						...
						<c:forEach begin="0" items="${requestScope.listaSedeVO}" var="sede">
							<option value="<c:out value="${sede.codigo}"/>" <c:if test="${sede.codigo==sessionScope.encuesta2VO.plaSede}">selected</c:if>><c:out value="${sede.nombre}"/></option>
						</c:forEach>
					</select>
  			</td>
  		</tr>
	  		...
	  		<td>
					...
					<option value="-99">-Seleccione uno-</option>
						<c:forEach begin="0" items="${requestScope.listaJornadaVO}" var="jor">
							<option value="<c:out value="${jor.codigo}"/>" <c:if test="${jor.codigo==sessionScope.encuesta2VO.plaJornada}">selected</c:if>><c:out value="${jor.nombre}"/></option>
						</c:forEach>
					</select>
  			</td>
  			<td><b>Grado:</b></td>
	  		<td>
					<select name="plaGrado" onchange="ajaxGrupo(document.frmFiltro.plaGrado,document.frmFiltro.plaGrupo)" style='width:100px;'>
						...
						<c:forEach begin="0" items="${requestScope.listaGradoVO}" var="grado">
							<option value='<c:out value="${grado.codigo}"/>' <c:if test="${grado.codigo==sessionScope.encuesta2VO.plaGrado}">selected</c:if>><c:out value="${grado.nombre}"/></option>
						</c:forEach>
					</select>
  			</td>
  		</tr>
	  		...
	  		<td>
					<select name="plaGrupo" style='width:100px;'>
						...
						<c:forEach begin="0" items="${requestScope.listaGrupoVO}" var="grupo">
							<option value='<c:out value="${grupo.codigo}"/>' <c:if test="${grupo.codigo==sessionScope.encuesta2VO.plaGrupo}">selected</c:if>><c:out value="${grupo.nombre}"/></option>
						</c:forEach>
					</select>
  			</td>
	  	</tr>
