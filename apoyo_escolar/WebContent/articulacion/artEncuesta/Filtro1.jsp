...
<!--
	...
	function hacerValidaciones_frmFiltro(forma){
		validarLista(forma.plaLocalidad, "- Localidad", 1)
		validarLista(forma.plaInstitucion, "- Institución", 1)
...
<body onLoad="mensaje(document.getElementById('msg'));">
	<form action='<c:url value="/articulacion/artEncuesta/Save.jsp"/>' method="post" name="frmFiltro" onSubmit="return validarForma(frmFiltro)">
		<INPUT TYPE="hidden" NAME="tipo" VALUE='<c:out value="1"/>'>
		...
		<table cellpadding="0" cellspacing="0" border="0" ALIGN="CENTER" width="100%" >
			<tr height="1">
				...
				<td rowspan="2" width="520">
					<img src='<c:url value="/etc/img/tabs/EstadoPorEstudiante_1.gif"/>' alt="Estado por Estudiante" height="26" border="0">
					<a href='javaScript:lanzar(<c:out value="2"/>)'><img src='<c:url value="/etc/img/tabs/consolidado_num_estudiantes_0.gif"/>' alt="Consolidado # Estudiantes" width="84" height="26" border="0"></a>
					<a href='javaScript:lanzar(3)'><img src='<c:url value="/etc/img/tabs/consolidado_encuesta_0.gif"/>' alt="Consolidado Encuesta" width="84" height="26" border="0"></a>
		...
		<table border="0" align="center" width="95%" cellpadding="2" cellspacing="0">
			<tr>
  			<td><span class="style2">*</span><b>Localidad:</b></td>
	  		<td colspan="3"><select name="plaLocalidad" onchange="javaScript:ajaxInstitucion(document.frmFiltro.plaLocalidad,document.frmFiltro.plaInstitucion,document.frmFiltro.plaSede,document.frmFiltro.plaJornada,document.frmFiltro.plaGrado,document.frmFiltro.plaGrupo)" style='width:160px;' <c:if test='${sessionScope.encuestaVO.habilitarLocalidad==1}'>disabled</c:if>>
						<option value="-99">-Seleccione uno-</option>
						<c:forEach begin="0" items="${requestScope.listaLocalidadesVO}" var="loc">
							<option value='<c:out value="${loc.codigo}"/>' <c:if test="${loc.codigo==sessionScope.encuestaVO.plaLocalidad}">selected</c:if>><c:out value="${loc.nombre}"/></option>
						</c:forEach>
					</select>
  			</td>
  		</tr>
  		<tr>
  			<td><span class="style2">*</span><b>Colegio:</b></td>
	  		<td colspan="3">
					<select name="plaInstitucion" onchange="ajaxSedes(document.frmFiltro.plaInstitucion,document.frmFiltro.plaSede,document.frmFiltro.plaJornada,document.frmFiltro.plaGrado,document.frmFiltro.plaGrupo)" style='width:250px;' <c:if test='${sessionScope.encuestaVO.habilitarColegio==1}'>disabled</c:if>>
						<option value="-99">-Seleccione uno-</option>
						<c:forEach begin="0" items="${requestScope.listaColegiosVO}" var="col">
							<option value='<c:out value="${col.codigo}"/>' <c:if test="${col.codigo==sessionScope.encuestaVO.plaInstitucion}">selected</c:if>><c:out value="${col.nombre}"/></option>
						</c:forEach>
					</select>
  			</td>
	  	</tr>
		 		...
		 		<td colspan="3">
					<select name="plaSede" onchange="ajaxJornadas(document.frmFiltro.plaSede,document.frmFiltro.plaJornada,document.frmFiltro.plaGrado,document.frmFiltro.plaGrupo)" style='width:250px;'>
						...
						<c:forEach begin="0" items="${requestScope.listaSedeVO}" var="sede">
							<option value="<c:out value="${sede.codigo}"/>" <c:if test="${sede.codigo==sessionScope.encuestaVO.plaSede}">selected</c:if>><c:out value="${sede.nombre}"/></option>
						</c:forEach>
					</select>
  			</td>
  		</tr>
  		<tr>
	  		...
	  		<td>
					...
					<c:forEach begin="0" items="${requestScope.listaJornadaVO}" var="jor">
							<option value="<c:out value="${jor.codigo}"/>" <c:if test="${jor.codigo==sessionScope.encuestaVO.plaJornada}">selected</c:if>><c:out value="${jor.nombre}"/></option>
						</c:forEach>
					</select>
  			</td>
	  		...
	  		<td>
					<select name="plaGrado" onchange="ajaxGrupo(document.frmFiltro.plaGrado,document.frmFiltro.plaGrupo)" style='width:100px;'>
						...
						<c:forEach begin="0" items="${requestScope.listaGradoVO}" var="grado">
							<option value='<c:out value="${grado.codigo}"/>' <c:if test="${grado.codigo==sessionScope.encuestaVO.plaGrado}">selected</c:if>><c:out value="${grado.nombre}"/></option>
						</c:forEach>
					</select>
  			</td>
  		</tr>	
	  		...
	  		<td>
					<select name="plaGrupo" style='width:100px;'>
						...
						<c:forEach begin="0" items="${requestScope.listaGrupoVO}" var="grupo">
							<option value='<c:out value="${grupo.codigo}"/>' <c:if test="${grupo.codigo==sessionScope.encuestaVO.plaGrupo}">selected</c:if>><c:out value="${grupo.nombre}"/></option>
						</c:forEach>
					</select>
  			</td>
  			<td><b>Ordenado por:</b></td>
	  		<td>
					<select name="plaOrden" style='width:100px;'>
						<option value="-99">-Seleccione uno-</option>
						<c:forEach begin="0" items="${requestScope.listaOrdenVO}" var="orden">
							<option value='<c:out value="${orden.codigo}"/>' <c:if test="${orden.codigo==sessionScope.encuestaVO.plaOrden}">selected</c:if>><c:out value="${orden.nombre}"/></option>
						</c:forEach>
					</select>
  			</td>
	  	</tr>
	  	<tr>  			
  			<td><b>Por Estado:</b></td>
	  		<td>
					<select name="plaEstado" style='width:100px;'>
						<option value="-99">-Todos-</option>
						<c:forEach begin="0" items="${requestScope.listaEstadoVO}" var="est">
							<option value='<c:out value="${est.codigo}"/>' <c:if test="${est.codigo==sessionScope.encuestaVO.plaEstado}">selected</c:if>><c:out value="${est.nombre}"/></option>
						</c:forEach>
					</select>
  			</td>
	 	</tr>
