...
<jsp:useBean id="filtroGru" class="articulacion.repHorariosArt.vo.FiltroBeanReporte" scope="session"/>
...
<c:import url="/parametros.jsp"/>
	...
	function ajaxGrupo(){
		borrar_combo(document.frm.filGrupo); 
		...
		document.frmAjax.ajax[3].value=document.frm.filAnoVigenciaArt.options[document.frm.filAnoVigenciaArt.selectedIndex].value;				
		document.frmAjax.ajax[4].value=document.frm.filPerVigenciaArt.options[document.frm.filPerVigenciaArt.selectedIndex].value;
		document.frmAjax.ajax[5].value=document.frm.filComponenteArt.options[document.frm.filComponenteArt.selectedIndex].value;
		document.frmAjax.ajax[6].value=document.frm.filEspecialidad.options[document.frm.filEspecialidad.selectedIndex].value;						
		document.frmAjax.cmd.value='<c:out value="${paramsVO.CMD_AJAX_GRUPO}"/>';
		document.frmAjax.tipo.value='<c:out value="${paramsVO.FICHA_HORARIO_GRUPO}"/>';
 		document.frmAjax.target="frameAjax";
		document.frmAjax.submit();
	}		
</script>
<c:import url="/mensaje.jsp"/>
	<form method="post" name="frm" onSubmit="return validarForma(frm)" action='<c:url value="/articulacion/repHorariosArt/FiltroGuardarGrupo.jsp"/>'>
...
<!--//////////////////-->	
	...
	<table border="0" align="center" width="100%">
		<tr height="1">
			<td rowspan="2" width="420" bgcolor="#FFFFFF"><a href="javaScript:lanzar(5)"><img src="../../etc/img/tabs/art_grupos_0.gif" width="84"  height="26" border="0"></a><a href="javaScript:lanzar(4)"><img src="../../etc/img/tabs/Horario_Estudiante_0.gif" width="84"  height="26" border="0"></a><img src="../../etc/img/tabs/horario_grupo_1.gif" width="84"  height="26" border="0"><a href="javaScript:lanzar(2)"><img src="../../etc/img/tabs/horario_docente_0.gif" width="84"  height="26" border="0"></a><a href="javaScript:lanzar(3)"><img src="../../etc/img/tabs/horario_espacio_fisico_0.gif" width="84"  height="26" border="0"></a></td>
		</tr>
  </table>
								...
								<td colspan="3">
									<select name="sede" style='width:350px;' onChange='filtro(document.frm.sede,document.frm.jornada)'>
										...
										<c:forEach begin="0" items="${requestScope.lSedeVO}" var="sede">
											<option value="<c:out value="${sede.codigo}"/>" <c:if test="${sede.codigo==sessionScope.filtroGru.sede}">selected</c:if>><c:out value="${sede.nombre}"/></option>
										</c:forEach>
									</select>
								</td>
							</tr>
							...
							<td>
								<select name="filAnoVigenciaArt" style='width:50px;'>
									...
									<c:forEach begin="0" items="${requestScope.VigenciaArtHor}" var="vig">
										<option value="<c:out value="${vig}"/>" <c:if test="${vig==sessionScope.filtroGru.filAnoVigenciaArt}">selected</c:if>><c:out value="${vig}"/></option>
									</c:forEach>
								</select>
								<select name="filPerVigenciaArt" style='width:30px;'>
									<option value="-99">--</option>
									<c:forEach begin="0" items="${requestScope.PeriodoArtHor}" var="vig">
										<option value="<c:out value="${vig}"/>" <c:if test="${vig==sessionScope.filtroGru.filPerVigenciaArt}">selected</c:if>><c:out value="${vig}"/></option>
									</c:forEach>
								</select>
							</td>
						</tr>
						<tr>
							...
							<td>
								<select name="filComponenteArt" style='width:100px;' onchange="ajaxGrupo()">
									<option value="-99">--Seleccione una--</option>
									<c:forEach begin="0" items="${requestScope.comArt}" var="comp">
										<option value="<c:out value="${comp.codigo}"/>" <c:if test="${comp.codigo==sessionScope.filtroGru.filComponenteArt}">selected</c:if>><c:out value="${comp.nombre}"/></option>
									</c:forEach>
								</select>
							</td>
							<td><span class="style2">*</span><b>&nbsp;Especialidad:</b></td>
							<td>
								<select name="filEspecialidad" style='width:200px;' onchange="ajaxGrupo()">
									<option value="-99">--Seleccione una--</option>
									<c:forEach begin="0" items="${requestScope.lEspecialidadVO}" var="esp">
										<option value="<c:out value="${esp.codigo}"/>" <c:if test="${esp.codigo==sessionScope.filtroGru.filEspecialidad}">selected</c:if>><c:out value="${esp.nombre}"/></option>
									</c:forEach>
								</select>
							</td>
						</tr>	
						<tr>	
							...
							<td colspan="3">
								<select name="filGrupo" style='width:100px;'>
									<option value="-99">--Seleccione una--</option>
									<c:forEach begin="0" items="${requestScope.lGrupoVO}" var="est">
										<option value="<c:out value="${est.codigo}"/>" <c:if test="${est.codigo==sessionScope.filtroGru.filGrupo}">selected</c:if>><c:out value="${est.nombre}"/></option>
									</c:forEach>
								</select>
							</td>
