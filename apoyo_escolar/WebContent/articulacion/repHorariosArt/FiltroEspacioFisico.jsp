...
<jsp:useBean id="filtroEspacio" class="articulacion.repHorariosArt.vo.FiltroBeanReporteEspacio" scope="session"/>
...
<c:import url="/parametros.jsp"/>
					...
					<c:if test="${!empty requestScope.lSedeVO && !empty requestScope.lJornadaVO}">
						...
						<c:forEach begin="0" items="${requestScope.lSedeVO}" var="sede" varStatus="st">
							...
							<c:forEach begin="0" items="${requestScope.lJornadaVO}" var="jornada">
								<c:if test="${sede.codigo==jornada.sede}"> Sel_Hijos[k] = '<c:if test="${sessionScope.filtroEspacio.jornada==jornada.codigo}">SELECTED</c:if>'; id_Hijos[k] = '<c:out value="${jornada.codigo}"/>';  Hijos[k] = '<c:out value="${jornada.nombre}"/>'; id_Padre[k] = '<c:out value="${jornada.sede}"/>'; k++;</c:if>
	...
	function ajaxEspacio(){
		borrar_combo(document.frm.espacio); 		
		...
		document.frmAjax.ajax[3].value=document.frm.tipoEspacio.options[document.frm.tipoEspacio.selectedIndex].value;								
		document.frmAjax.cmd.value='<c:out value="${paramsVO.CMD_AJAX_ESPFISICO}"/>';
 		document.frmAjax.target="frameAjax";
		document.frmAjax.submit();
	}				
</script>
<c:import url="/mensaje.jsp"/>
<form method="post" name="frm" onSubmit="return validarForma(frm)" action='<c:url value="/articulacion/repHorariosArt/FiltroGuardarEspacioFisico.jsp"/>'>
...
<!--//////////////////-->	
	...
	<table border="0" align="center" width="100%">
		<tr height="1">
			<td rowspan="2" width="420" bgcolor="#FFFFFF"><a href="javaScript:lanzar(5)"><img src="../../etc/img/tabs/art_grupos_0.gif" width="84"  height="26" border="0"></a><a href="javaScript:lanzar(4)"><img src="../../etc/img/tabs/Horario_Estudiante_0.gif" width="84"  height="26" border="0"></a><a href="javaScript:lanzar(1)"><img src="../../etc/img/tabs/horario_grupo_0.gif" width="84"  height="26" border="0"></a><a href="javaScript:lanzar(2)"><img src="../../etc/img/tabs/horario_docente_0.gif" width="84"  height="26" border="0"></a><img src="../../etc/img/tabs/horario_espacio_fisico_1.gif" width="84"  height="26" border="0"></td>
		</tr>
  </table>
		<table width="100%" border="0" cellspacing="2" cellpadding="0">
				...
				<td colspan="3">
					<select name="sede" style='width:350px;' onChange='filtro(document.frm.sede,document.frm.jornada)'>
						...
						<c:forEach begin="0" items="${requestScope.lSedeVO}" var="sede">
							<option value="<c:out value="${sede.codigo}"/>" <c:if test="${sede.codigo==sessionScope.filtroEspacio.sede}">selected</c:if>><c:out value="${sede.nombre}"/></option>
						</c:forEach>
					</select>
				</td>
			</tr>
			<tr>
				...
				<td>
					<select name="jornada" style='width:100px;' onChange='ajaxEspacio()'>
						<option value="-99">--Seleccione una--</option>
					</select>
				</td>
				...
				<td>
					<select name="filAnoVigenciaArt" style='width:50px;'>
						...
						<c:forEach begin="0" items="${requestScope.VigenciaArtHor}" var="vig">
							<option value="<c:out value="${vig}"/>" <c:if test="${vig==sessionScope.filtroEspacio.filAnoVigenciaArt}">selected</c:if>><c:out value="${vig}"/></option>
						</c:forEach>
					</select>
					<select name="filPerVigenciaArt" style='width:30px;'>
						<option value="-99">--</option>
						<c:forEach begin="0" items="${requestScope.PeriodoArtHor}" var="vig">
							<option value="<c:out value="${vig}"/>" <c:if test="${vig==sessionScope.filtroEspacio.filPerVigenciaArt}">selected</c:if>><c:out value="${vig}"/></option>
						</c:forEach>
					</select>
				</td>
			</tr>
				...
				<td colspan="1">
					<select name="filComponenteArt" style='width:150px;'>
						...
						<c:forEach begin="0" items="${requestScope.comArt}" var="comp">
							<option value="<c:out value="${comp.codigo}"/>" <c:if test="${comp.codigo==sessionScope.filtroEspacio.filComponenteArt}">selected</c:if>><c:out value="${comp.nombre}"/></option>
						</c:forEach>
					</select>
				</td>
				<td><b>&nbsp;Tipo Espacio:</b></td>
				<td>
					<select name="tipoEspacio" style='width:150px;' onChange='ajaxEspacio()'>
						<option value="-99">--Seleccione una--</option>
						<c:forEach begin="0" items="${requestScope.TiposEspacio}" var="tipoes">
							<option value="<c:out value="${tipoes.codigo}"/>" <c:if test="${tipoes.codigo==sessionScope.filtroEspacio.tipoEspacio}">selected</c:if>><c:out value="${tipoes.nombre}"/></option>
						</c:forEach>
					</select>
				</td>
			</tr>	
			<tr>	
				<td><b>&nbsp;Espacio:</b></td>
				<td colspan="3">
					<select name="espacio" style='width:230px;'>
						<option value="-99">--Seleccione una--</option>
						<c:forEach begin="0" items="${requestScope.lEspacioVO}" var="esp">
							<option value="<c:out value="${esp.codigo}"/>" <c:if test="${esp.codigo==sessionScope.filtroEspacio.docente}">selected</c:if>><c:out value="${esp.nombre}"/></option>
						</c:forEach>
					</select>
				</td>
...
<form method="post" name="frmAjax" action="<c:url value="/articulacion/repHorariosArt/ajaxRepArt.do"/>"><input type="hidden" name="ajax"><input type="hidden" name="ajax"><input type="hidden" name="ajax"><input type="hidden" name="ajax"><INPUT TYPE="hidden" NAME="tipo" VALUE='<c:out value="${paramsVO.FICHA_HORARIO_ESPFISICO}"/>'><input type="hidden" name="cmd" value='<c:out value="${paramsVO.CMD_AJAX}"/>'></form>