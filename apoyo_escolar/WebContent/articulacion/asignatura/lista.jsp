...
<body onLoad="mensaje(document.getElementById('msg'));">
	<form action='<c:url value="/articulacion/asignatura/Nuevo.do"/>' method="post" name="frmLista" onSubmit="return validarForma(frmLista)">
		...
		<table border="1" align="center" width="95%" cellpadding="0" cellspacing="0">
		 	...
		 	<c:if test="${empty sessionScope.listaAsignaturaVO}">
				<tr>
			...
			<c:if test="${!empty sessionScope.listaAsignaturaVO}">
				...
				<c:forEach begin="0" items="${sessionScope.listaAsignaturaVO}" var="lista" varStatus="st">
					<tr>
						<th class='Fila<c:out value="${st.count%2}"/>'>
							<a href='javaScript:editar(<c:out value="${lista.artAsigCodigo}"/>);'>
								<img border='0' src='../../etc/img/<c:if test="${requestScope.guia==lista.artAsigCodigo}">X</c:if>editar.png' width='15' height='15'>
							</a>
							<c:if test="${sessionScope.NivelPermiso==2}">
								<a href='javaScript:eliminar(<c:out value="${lista.artAsigCodigo}"/>);'>
								<img border='0' src='<c:url value="/etc/img/eliminar.png"/>' width='15' height='15'></a>
							</c:if>
						</th>
						<td class='Fila<c:out value="${st.count%2}"/>' align="center"><c:out value="${lista.artAsigCodAsigIns}"/></td>
						<td class='Fila<c:out value="${st.count%2}"/>' align="center"><c:out value="${lista.artAsigNombre}"/></td>
					</tr>
				</c:forEach>
			</c:if>
		</table>
