...
<!--
  	...
  	document.frmLista.tipo.value=tipo;
		...
		function Padre(id_Hijos, Hijos, Sel_Hijos,id_Padre){
			...
			function borrar_combo(combo){
				for(var i = combo.length - 1; i >= 0; i--) { if(navigator.appName == "Netscape") combo.options[i] = null; else combo.remove(i);}combo.options[0] = new Option("--Seleccione una--","-99");
			}
			function borrar_comboTodos(combo){
				for(var i = combo.length - 1; i >= 0; i--) { if(navigator.appName == "Netscape") combo.options[i] = null; else combo.remove(i);}combo.options[0] = new Option("--Todos--","-99");
			}
	...
	function ajaxGrupo(){
		borrar_comboTodos(document.frmLista.filAsignatura); 
		borrar_comboTodos(document.frmLista.filGrupo); 
		document.frmAjax.ajax[0].value=document.frmLista.filInstitucion.value;
	...
	function eliminar(){
		if(validarForma(document.frmLista)){
			if(confirm('¿Desea eliminar el horario indicado?')){
				document.frmLista.filEspecialidad.disabled=false;
...
<c:import url="/mensaje.jsp"/>
	<form method="post" name="frmAjax" action="<c:url value="/articulacion/artRotacion/Ajax.do"/>"><input type="hidden" name="ajax"><input type="hidden" name="ajax"><input type="hidden" name="ajax"><input type="hidden" name="ajax"><input type="hidden" name="ajax"><input type="hidden" name="ajax"><input type="hidden" name="ajax"><input type="hidden" name="ajax"><input type="hidden" name="ajax"><INPUT TYPE="hidden" NAME="tipo" VALUE='<c:out value="${paramsVO.FICHA_BORRAR_HORARIO}"/>'><input type="hidden" name="cmd" value='<c:out value="${paramsVO.CMD_AJAX}"/>'></form>
	<form action='<c:url value="/articulacion/artRotacion/SaveHorario.jsp"/>' method="post" name="frmLista" onSubmit="return validarForma(frmLista)">
		<INPUT TYPE="hidden" NAME="tipo" VALUE='<c:out value="${paramsVO.FICHA_BORRAR_HORARIO}"/>'>
		...
		<table width="100%" border="0" cellspacing="2" cellpadding="0">
	 	<caption>FILTRO DE BÚSQUEDA</caption>
				<tr><td width="45%">
		        <input name="cmd1" type="button" value="Aceptar" onClick="eliminar()" class="boton">
			  </td></tr>	
	  </table>
		<table width="100%" border="0" cellspacing="2" cellpadding="0">
			<tr>
				...
				<td colspan="3">
					<select name="filSede" style='width:350px;' onChange='filtro(document.frmLista.filSede,document.frmLista.filJornada)'>
...
<script>
filtro(document.frmLista.filSede,document.frmLista.filJornada);
