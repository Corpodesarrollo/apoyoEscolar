...
<body onLoad="mensaje(document.getElementById('msg'));">
	<form action='<c:url value="/articulacion/especialidadBase/Nuevo.do"/>' method="post" name="frmLista" onSubmit="return validarForma(frmLista)">
		...
		<table cellpadding="0" cellspacing="0" border="0" ALIGN="CENTER" width="100%" >
			<tr height="1">
				...
				<td rowspan="2" width="469">
					...
					<img src='<c:url value="/etc/img/tabs/esp_base_1.gif"/>' alt="Especialidad" height="26" border="0">
		...
		<table border="1" align="center" width="95%" cellpadding="0" cellspacing="0">
		 	<caption>LISTADO DE ESPECIALIDAD BASE</caption>
		 	<c:if test="${empty requestScope.listaEspecialidadBaseVO}">
				<tr>
					<th class="Fila1" colspan='6'>NO EXISTEN REGISTROS DE CARRERAS BASE</th>
				</tr>
			</c:if>
			<c:if test="${!empty requestScope.listaEspecialidadBaseVO}">
				<tr>
					...
					<td class="EncabezadoColumna" align="center">Nombre de Especialidad</td>
				</tr>
				<c:forEach begin="0" items="${requestScope.listaEspecialidadBaseVO}" var="lista" varStatus="st">
					<tr>
						<th class='Fila<c:out value="${st.count%2}"/>'>
							<a href='javaScript:editar(<c:out value="${lista.gespbasCodigo}"/>);'>
								<img border='0' src='../../etc/img/<c:if test="${requestScope.guia==lista.gespbasCodigo}">X</c:if>editar.png' width='15' height='15'>
							</a>
							<c:if test="${sessionScope.NivelPermiso==2}">
								<a href='javaScript:eliminar(<c:out value="${lista.gespbasCodigo}"/>);'>
								<img border='0' src='<c:url value="/etc/img/eliminar.png"/>' width='15' height='15'></a>
							</c:if>
						</th>
						<td class='Fila<c:out value="${st.count%2}"/>' align="center"><c:out value="${lista.gespbasCodigo}"/></td>
						<td class='Fila<c:out value="${st.count%2}"/>' align="center"><c:out value="${lista.gespbasNombre}"/></td>
