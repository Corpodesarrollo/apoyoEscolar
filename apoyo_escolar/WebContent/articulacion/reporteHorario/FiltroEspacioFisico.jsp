...
<jsp:useBean id="filtroEsp" class="articulacion.reporteHorario.beans.FiltroBeanReporteEspacio" scope="session"/><jsp:setProperty name="filtroEsp" property="*"/>
...
<c:import url="/parametros.jsp"/>
		<script languaje='JavaScript'>
			...
			function hacerValidaciones_frm(forma){
			   validarLista(forma.tipoEspacioFisico, "- Tipo de Espacio", 1);
			   validarLista(forma.espacioFisico, "- Espacio Fisico", 1);
			   validarLista(forma.estructura, "- Estructura", 1);
			}
			...
			function cancelar(){
				if (confirm('¿No desea generar Reportes?')){
					...
					<c:if test="${!empty requestScope.filtroSedeF && !empty requestScope.filtroJornadaF}">
						...
						<c:set var="z" value="${fila[0]}" scope="session"/>
							...
							<c:if test="${fila[0]==fila2[2]}">
									Sel_Hijos[k] = '<c:if test="${sessionScope.filtroEsp.jornada== fila2[0]}">SELECTED</c:if>';id_Hijos[k] = '<c:out value="${fila2[0]}"/>'; Hijos[k] = '<c:out value="${fila2[1]}"/>'; id_Padre[k] = '<c:out value="${fila2[2]}"/>'; k++;
							</c:if></c:forEach>
				...
				if(combo_padre3.selectedIndex==0)
					...
					<c:if test="${!empty requestScope.filtroSedeF && !empty requestScope.filtroGradoF2 && !empty requestScope.filtroMetodologiaF}">;var Padres = new Array();
						...
						<c:forEach begin="0" items="${requestScope.filtroGradoF2}" var="fila3">
							...
							Sel_Hijos[k] = '<c:if test="${sessionScope.filtroEsp.grado== fila3[0]}">SELECTED</c:if>';
			...
			function filtro3(combo_padre,combo_padre1,combo_padre2,combo_hijo){
				if(combo_padre2.selectedIndex==-9)
					borrar_combo(combo_hijo); 
				else{
					borrar_combo(combo_hijo);
					<c:if test="${!empty requestScope.filtroJornadaF && !empty requestScope.filtroTipoEspacioF && !empty requestScope.filtroEspacioF}">
						var Padres = new Array();
						<c:forEach begin="0" items="${requestScope.filtroJornadaF}" var="fila2" varStatus="st">
							<c:forEach begin="0" items="${requestScope.filtroTipoEspacioF}" var="fila4" varStatus="st1">
							id_Hijos=new Array(); Hijos= new Array(); Sel_Hijos= new Array(); id_Padre= new Array(); var k=0;
							  <c:forEach begin="0" items="${requestScope.filtroEspacioF}" var="fila3">
							  <c:if test="${fila2[2]==fila3[3] && fila2[0]==fila3[5] && fila3[4]==fila4[0]}">
									Sel_Hijos[k] = '<c:if test="${sessionScope.filtroEsp.espacioFisico== fila3[0]}">SELECTED</c:if>';
							...
							Padres[<c:out value="${st1.index}"/>] = new Padre(id_Hijos, Hijos, Sel_Hijos,id_Padre);
							</c:forEach>
						</c:forEach>
						var niv=combo_padre.value+'|'+combo_padre2.options[combo_padre2.selectedIndex].value+'|'+combo_padre1.value;
						var val_padre = -9;
						for(var k=0;k<Padres.length;k++){
							if(Padres[k].id_Padre[0]==niv) val_padre=k;
						}
...
<c:import url="/mensaje.jsp"/>
	<font size="1">
	<form method="post" name="frm" onSubmit="return validarForma(frm)" action='<c:url value="/articulacion/reporteHorario/FiltroGuardarEspacioFisico.jsp"/>'>
	<br>
...
<!--//////////////////-->	
	...
	<table border="1" align="center" bordercolor="#FFFFFF" width="100%">
		<tr height="1">
			...
			<a href="javaScript:lanzar(1)"><img src='<c:url value="/etc/img/tabs/horario_grupo_0.gif"/>' width="84"  height="26" border="0"></a><a href="javaScript:lanzar(2)"><img src='<c:url value="/etc/img/tabs/horario_docente_0.gif"/>' width="84"  height="26" border="0"></a><img src='<c:url value="/etc/img/tabs/horario_espacio_fisico_1.gif"/>' width="84"  height="26" border="0"></td>
		</tr>
  </table>
			<table border="1" align="center" bordercolor="#FFFFFF" width="100%">
			<tr>
...
<!-- 												<select name="sede" onChange='filtro(document.frm.sede, document.frm.jornada,document.frm.grado,document.frm.grupo,document.frm.metodologia)' style='width:300px;'>
													...
													<c:forEach begin="0" items="${requestScope.filtroSedeF}" var="fila">
														<option value="<c:out value="${fila[0]}"/>" <c:if test="${sessionScope.filtroEsp.sede== fila[0]}">SELECTED</c:if>>
...
-->												
									  ...
									  <tr>
										  ...
										  <td>
                                            <select name="tipoEspacioFisico" style='width:120px;' onChange='filtro3(document.frm.sede,document.frm.jornada,document.frm.tipoEspacioFisico,document.frm.espacioFisico)'>
                                              <option value='-9'>-- Seleccione una --</option>
                                              <c:forEach begin="0" items="${requestScope.filtroTipoEspacioF}" var="fila">
												<option value="<c:out value="${fila[0]}"/>" <c:if test="${sessionScope.filtroEsp.tipoEspacioFisico==fila[0]}">SELECTED</c:if>>
												<c:out value="${fila[1]}"/></option>
                                              </c:forEach>
                                            </select>
                                          </td>
                                          
										  <td><span class="style2" >*</span>Espacio Físico:</td>
										  <td>
                                            <select name="espacioFisico" style='width:120px;'>
                                              <option value='-9'>-- Todos --</option>
                                              <c:forEach begin="0" items="${requestScope.filtroEspacioF}" var="fila">
												<option value="<c:out value="${fila[0]}"/>" <c:if test="${sessionScope.filtroEsp.espacioFisico==fila[0]}">SELECTED</c:if>>
												<c:out value="${fila[1]}"/></option>
                                              </c:forEach>
                                            </select>
                                          </td>
									  ...
									  <tr>
											...
											<td>
												<select name="estructura" style='width:120px;'>
	                                              <option value='-9'>-- Seleccione una --</option>
    	                                          <c:forEach begin="0" items="${requestScope.filtroEstructuraF}" var="fila">
	    	                                         <option value='<c:out value="${fila[0]}"/>' <c:if test="${sessionScope.filtroEsp.estructura== fila[0]}">SELECTED</c:if>> <c:out value="${fila[1]}"/> 
	                                               </c:forEach>
	                                            </select>
                                            </td>
									 </tr>
