...
<jsp:useBean id="filtroDoc" class="articulacion.reporteHorario.beans.FiltroBeanReporteDocente" scope="session"/><jsp:setProperty name="filtroDoc" property="*"/>
<c:import url="/parametros.jsp"/>
					...
					<c:if test="${!empty requestScope.filtroSedeF && !empty requestScope.filtroJornadaF}">
						...
						<c:set var="z" value="${fila[0]}" scope="session"/>
							...
							<c:if test="${fila[0]==fila2[2]}">
									Sel_Hijos[k] = '<c:if test="${sessionScope.filtroDoc.jornada== fila2[0]}">SELECTED</c:if>';id_Hijos[k] = '<c:out value="${fila2[0]}"/>'; Hijos[k] = '<c:out value="${fila2[1]}"/>'; id_Padre[k] = '<c:out value="${fila2[2]}"/>'; k++;
							</c:if></c:forEach>
				...
				if(combo_padre3.selectedIndex==0)
					...
					<c:if test="${!empty requestScope.filtroSedeF && !empty requestScope.filtroGradoF2 && !empty requestScope.filtroMetodologiaF}">;var Padres = new Array();
						...
						<c:forEach begin="0" items="${requestScope.filtroGradoF2}" var="fila3">
							...
							Sel_Hijos[k] = '<c:if test="${sessionScope.filtroDoc.grado== fila3[0]}">SELECTED</c:if>';
						...
						<c:forEach begin="0" items="${requestScope.filtroGradoF2}" var="fila2"  varStatus="st">
							...
							<c:forEach begin="0" items="${requestScope.filtroGrupoF}" var="fila3">
								<c:if test="${fila2[0]==fila3[4] && fila2[3]==fila3[2] && fila2[4]==fila3[3] && fila2[5]==fila3[5]}">
									Sel_Hijos[k] = '<c:if test="${sessionScope.filtroDoc.grupo== fila3[0]}">SELECTED</c:if>';
	...
	<font size="1">
	<form method="post" name="frm" onSubmit="return validarForma(frm)" action='<c:url value="/articulacion/reporteHorario/FiltroGuardarDocente.jsp"/>'>
	<br>
...
<!--//////////////////-->	
	...
	<table border="1" align="center" bordercolor="#FFFFFF" width="100%">
		<tr height="1">
			<td rowspan="2" width="420" bgcolor="#FFFFFF"><a href="javaScript:lanzar(1)"><img src='<c:url value="/etc/img/tabs/horario_grupo_0.gif"/>' width="84"  height="26" border="0"></a><img src='<c:url value="/etc/img/tabs/horario_docente_1.gif"/>' width="84"  height="26" border="0"><a href="javaScript:lanzar(3)"><img src='<c:url value="/etc/img/tabs/horario_espacio_fisico_0.gif"/>' width="84"  height="26" border="0"></a></td>
		</tr>
		<tr>
		</tr>
  </table>
...
<!-- 												<select name="sede" onChange='filtro(document.frm.sede, document.frm.jornada,document.frm.grado,document.frm.grupo,document.frm.metodologia)' style='width:300px;'>
													...
													<c:forEach begin="0" items="${requestScope.filtroSedeF}" var="fila">
														<option value="<c:out value="${fila[0]}"/>" <c:if test="${sessionScope.filtroDoc.sede== fila[0]}">SELECTED</c:if>>
										  ...
										  <td><span class="style2" >*</span>Docente:</td>
											<td>
												<select name="docente" style='width:250px;'>
	                                              <option value='-9'>-- Seleccione una --</option>
												<c:forEach begin="0" items="${requestScope.filtroDocenteF}" var="fila">
													<option value='<c:out value="${fila[0]}"/>'><c:out value="${fila[1]}"/> <c:out value="${fila[2]}"/> </option>
												</c:forEach>												
	                                            </select>
                                            </td>
											...
											<td>
												<select name="estructura" style='width:120px;'>
    	                                          ...
    	                                          <c:forEach begin="0" items="${requestScope.filtroEstructuraF}" var="fila">
	    	                                         <option value='<c:out value="${fila[0]}"/>' <c:if test="${sessionScope.filtroDoc.estructura== fila[0]}">SELECTED</c:if>> <c:out value="${fila[1]}"/> 
	                                               </c:forEach>
	                                            </select>
                                            </td>
										  </tr>
