		...
		<script language="JavaScript">
			...
			function remoto(forma,indice,dia){
				...
				if(forma.falla[indice].disabled==true){
					alert('No puede cambiar el motivo, dia inhabilitado')
					return;
				}
				...
				//frmPopUp
				remote = window.open("","3","width=300,height=150,resizable=no,toolbar=no,directories=no,menubar=no,status=no");
...
<body onLoad="mensaje(document.getElementById('msg'));">
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
		<tr><td width="100%" valign='top'>
			<div style="width:100%;height:130px;overflow:auto;vertical-align:top;background-color:#ffffff;"><c:import url="/inasistencia/Filtro.do?cmd=2"/></div>
		</td></tr>
	</table>
	    ...
	    <table style="display:" id='t2' name='t2' width="100%" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#CCCCCC">
				...
				<tr>
					...
					<c:forEach begin="0" items="${estudiante.evalFallas}" var="falla" varStatus="st3"><c:set var="indice" value="${pageScope.indice+1}" scope="page"/>
						 <th class='Fila<c:out value="${st2.count%2}"/>'><input type="hidden" name="falla2" value='<c:out value="${estudiante.evalCodigo}"/>|<c:out value="${falla.faDia}"/>|<c:out value="${falla.faMotivo}"/>'><input type="checkbox" onclick='javaScript:cambio(document.frmNuevo,<c:out value="${pageScope.indice}"/>);' name="falla" <c:if test="${falla.faChecked==1}">value='<c:out value="${estudiante.evalCodigo}"/>|<c:out value="${falla.faDia}"/>|<c:out value="${falla.faMotivo}"/>' CHECKED</c:if> <c:if test="${falla.faDisabled==1}">DISABLED</c:if>><a href='javaScript:remoto(document.frmNuevo,<c:out value="${pageScope.indice}"/>,<c:out value="${falla.faDiaSemana}"/>);'><img src='<c:url value="/etc/img/z.gif"/>' border='0'></a></th>
					</c:forEach></tr>
