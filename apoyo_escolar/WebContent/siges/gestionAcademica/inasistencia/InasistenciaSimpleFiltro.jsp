...
<input type="hidden" name="tipo" value='<c:out value="${paramsVO.FICHA_INASISTENCIA_SIMPLE}"/>'>
<input type="hidden" name="cmd">
<input type="hidden" name="filInstitucion" value='<c:out value="${sessionScope.login.instId}"/>'>
<input type="hidden" name="filGrado_" id="filGrado_" value='<c:out  value="${sessionScope.filtroInasistencia.filGrado}"/>'>
...
<table cellpadding="0" cellspacing="0" border="0" ALIGN="CENTER" width="100%" >
	...
	<TABLE width="100%" cellpadding="1" cellSpacing="0">
			<tr>											
				...
				<td>
					<select name="filGrado" id="filGrado" style='width:120px;' onChange='setVisible();filtroGrado(document.frmFiltro.filMetodologia,document.frmFiltro.filGrado,document.frmFiltro.filGrupo);'>
						<option value='-99'>-- Seleccione uno --</option>
						<c:forEach begin="0" items="${requestScope.lGrado}" var="grado">
							  <option value="<c:out value="${grado.codigo}"/>" <c:if test="${sessionScope.filtroInasistencia.filGrado == grado.codigo}">SELECTED</c:if>>
							 <c:out value="${grado.nombre }"/>
							 </option>
						</c:forEach>
					</select>
				</td>	
