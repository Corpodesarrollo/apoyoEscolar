    ...
    function SetupTicker() { 
      ...
      RunTicker();
			...
			function borrar_combo2(combo){
				...
				combo.options[0] = new Option("--Todas--","-99");
			}
			...
			function ajaxAsignatura(){
				borrar_combo2(document.frmFiltro.filAsignatura); 
				document.frmAjax.ajax[0].value=document.frmFiltro.filInstitucion.value;
				document.frmAjax.ajax[1].value=document.frmFiltro.filMetodologia.value;
				document.frmAjax.ajax[2].value=document.frmFiltro.filGrado.options[document.frmFiltro.filGrado.selectedIndex].value;
			...
			function filtroPeriodo(combo_padre,combo_hijo){
				borrar_combo(combo_hijo); 
					...
					if(val_padre!=-1){
						...
						combo_hijo.selectedIndex=1;
				}
			}
			//-->
		</script>
<c:import url="/mensaje.jsp"/>
<form method="post" name="frmAjax" action="<c:url value="/inasistencia/Ajax.do"/>"><input type="hidden" name="ajax"><input type="hidden" name="ajax"><input type="hidden" name="ajax"><input type="hidden" name="ajax"><INPUT TYPE="hidden" NAME="tipo" VALUE='<c:out value="${paramsVO.FICHA_INASISTENCIA}"/>'><input type="hidden" name="cmd" value='<c:out value="${paramsVO.CMD_AJAX}"/>'></form>
...
<caption>Filtro de inasistencia</caption>
...
<table cellpadding="0" cellspacing="0" border="0" ALIGN="CENTER" width="100%" >
	<tr height="1">
		...
		<td width="600" bgcolor="#FFFFFF">
			<img src='<c:url value="/etc/img/tabs/Inasistencia_1.gif"/>' alt="-Inasistencia-" width="84"  height="26" border="0">
			<a href="javaScript:lanzar(<c:out value="${paramsVO.FICHA_RETARDO}"/>)"><img src='<c:url value="/etc/img/tabs/retardo_0.gif"/>' alt="Retardo"  height="26" border="0"></a>
	...
	<TABLE width="100%" cellpadding="1" cellSpacing="0">
			<tr>
				...
				<td>
					<select name="filGrado" style='width:120px;' onChange='setVisible();filtroGrado(document.frmFiltro.filMetodologia,document.frmFiltro.filGrado,document.frmFiltro.filGrupo);ajaxAsignatura()'>
						<option value='-99'>-- Seleccione uno --</option>
						<c:forEach begin="0" items="${requestScope.lGrado}" var="grado">
				...
				<td><select name="filGrupo" style='width:120px;' onChange='setVisible()'>
							<option value='-99'>-- Seleccione uno --</option>
						</select>
				</td>
			</tr>
			<tr>
				...
				<td><select name="filPeriodo" style='width:100px;' onChange='setVisible();filtroPeriodo(document.frmFiltro.filPeriodo,document.frmFiltro.filMes)'>
						<option value='-99'>-- Seleccione uno --</option>
				...
				<td>Asignatura:</td>
				<td><select name="filAsignatura" style='width:150px;' onChange='setVisible()'>
						<option value='-99'>-- Todas --</option>
						<c:forEach begin="0" items="${requestScope.lAsignatura}" var="asig">
							<option value="<c:out value="${asig.codigo}"/>" <c:if test="${sessionScope.filtroInasistencia.filAsignatura==asig.codigo}">SELECTED</c:if>><c:out value="${asig.nombre}"/></option>
						</c:forEach>
				</select></td>
