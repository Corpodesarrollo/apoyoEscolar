...
<head>
 ...
 <!--
   ...
   function ajaxGrupo(){
	  borrar_combo2(document.frmFiltro.filarea);
	  document.frmAjaxNuevo.target = 'frameAjaxNuevo';
      document.frmAjaxNuevo.ajax[0].value =  document.frmFiltro.filinst.value;
      document.frmAjaxNuevo.ajax[1].value = document.frmFiltro.filsede.value;
      document.frmAjaxNuevo.ajax[2].value = document.frmFiltro.filjornd.value;
      ...
      document.frmAjaxNuevo.ajax[3].value = document.frmFiltro.filmetod.value;
      document.frmAjaxNuevo.ajax[4].value = document.frmFiltro.filgrado.value;
      document.frmAjaxNuevo.cmd.value = document.frmAjaxNuevo.CMD_GRUPO_AREA.value;
	  document.frmAjaxNuevo.submit();
   }
    function ajaxEstd(){
...
<body onLoad="mensaje(document.getElementById('msg'));">
		...
		<input type="hidden" name="tipo" value='<c:out value="${ paramsVO.FICHA_REP_DESCRIPTOR  }"/>'>
		...
		<input type="hidden" name="CMD_AJAX_GRUPO_AREA"  id="CMD_GRUPO_AREA"  value='<c:out value="${paramsVO.CMD_GRUPO_AREA}"/>'>
		 ...
		 <table cellpadding="0" cellspacing="0" border="0" ALIGN="CENTER" width="100%" >
		  <caption>Gestión Académica >> Reportes Estadísticos </caption>
	        <tr height="1">
				...
				<td width="14%" height="26" class="norepeat" >
				<p align="center" class="sombra">Evaluación Tipo <c:out value="${logdes[1]}"/></p>
				</td>
			</c:forEach>
   ...
   <!-- FORMUALARIO DE EDICION CREACION--> 		  
	<form method="post" name="frmFiltro"  onKeyDown="checkKey(event)"   action='<c:url value="/siges/gestionAcademica/repEstadisticos/Save.jsp"/>'>
        ...
        <table border="0" align="center" width="100%" cellpadding="2" cellspacing="2">
		  <caption align="center">REPORTE EVALUACIÓN TIPO DESCRIPTOR</caption>
     	  ...
     	  <table border="0" align="center" width="100%" cellpadding="2" cellspacing="2">
			<tr><td style="width: 15%;display:none;"><span class="style2" >*</span>Sede</td>
		 	 ...
		 	 <tr><td><span class="style2" >*</span>Metodología</td>
		 	    ...
		 	    <td><select name="filgrado" id="filgrado" onchange="limpiarCampos();ajaxGrupoArea();">
		 	        <option value="-99">-- Seleccione uno--</option>
		 	  ...
		 	  <tr><td><span class="style2" >*</span>Grupo</td>
		 	   ...
		 	   <td>Área</td>
		 	    <td colspan="1" >
		 	       <select  style="width: 250px;" name="filarea" id="filarea" onchange="limpiarCampos();" >
		 	        <option value="-99">-- Todas --</option>
		 	         <c:forEach begin="0" items="${sessionScope.listaArea }" var="vig">
				      <option value="<c:out value="${vig.codigo}"/>" <c:if test="${sessionScope.filtroRepEstadisticoVO.filarea == vig.codigo}">SELECTED</c:if>><c:out value="${vig.nombre}"/></option>
                     </c:forEach>
		 	        </select>
		 	    </td>
		 	 </tr>  
