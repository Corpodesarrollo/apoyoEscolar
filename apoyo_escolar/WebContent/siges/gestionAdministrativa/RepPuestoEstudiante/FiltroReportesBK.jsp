...
function validarNumeros2(e) { 
	...
	function hacerValidaciones_frmNuevo(forma){
		...
		}else if(forma.tipoReporte.value==5){
			validarLista(forma.metodo,'- Metodologia', 1);
			validarLista(forma.periodo,'- Período', 1);
			...
			var msjFechaProm="";		
				...
				if(document.frmNuevo.prov.value>=0){
					...
					if(document.frmNuevo.loc.value>=0){
						...
						msjFechaProm="La última actualización de promedio por grupos para el municipio "+entidad+" se realizó el "+document.frmNuevo.fechaProm.value+". ¿Desea realizar actualización para este reporte?";
   	...
   	function ajaxReporte(){		 
		//alert("validar tipo de reporte: "+document.frmNuevo.tipoReporte.value);
		 if(document.frmNuevo.tipoReporte.value>0){
			  ...
			  document.frmNuevo.conAreAsi_[0].disabled=false;
			  document.frmNuevo.conAreAsi_[1].disabled=false;
			  document.frmNuevo.conAreAsi_[1].checked=true;
			  document.frmNuevo.conAreAsi.value=2;
			  if(document.frmNuevo.tipoReporte.value==1){
				  ...
				  document.getElementById('verDocente').style.display=OCULTO;
				  ...
				  document.getElementById('verDocente').style.display=OCULTO;
				  var msj="";
				  //alert("ESCALA: "+<c:out value="${sessionScope.filtroReportesVO.tipoEscala}"/>);
			  ...
			  else if(document.frmNuevo.tipoReporte.value==5){
				  document.frmNuevo.grupo.disabled=true; 
				  document.frmNuevo.periodo.disabled=false;
				  document.frmNuevo.asigNombres.rows='1';
				  document.getElementById('rep4').style.display=OCULTO;
				  document.getElementById('rep2').style.display=VISIBLE;
				  document.getElementById('verDocente').style.display=VISIBLE;
				  document.frmNuevo.conGraGru_[0].disabled=true;
				  document.frmNuevo.conGraGru_[1].disabled=true;
				  document.frmNuevo.asigCodigos.value='';
				  document.frmNuevo.asigNombres.value='';

				  document.frmNuevo.conAreAsi_[0].disabled=true;
				  document.frmNuevo.conAreAsi_[1].disabled=true;
				  document.frmNuevo.conAreAsi_[1].checked=true;
				  document.frmNuevo.conAreAsi.value=2;
   ...
   <!-- FORMUALARIO DE EDICION CREACION--> 		  
	<form method="post" name="frmNuevo"  action='<c:url value="/siges/gestionAdministrativa/comparativos/Save.jsp"/>'>
		...
		<input type="hidden" name="valorFin_" value='100'>
		 ...
		 <table cellpadding="0" cellspacing="1" border="0" ALIGN="CENTER" width="100%" >
		  <caption>Gestión Administrativa >> Reporte Puesto Estudiante</caption>	         
		    <tr>						
			 <td rowspan="2" bgcolor="#FFFFFF">			      
				   <img src='<c:url value="/etc/img/tabs/comparativos_1.gif"/>' border="0"  height="26" alt='Reporte Puesto Estudiante'>
		        </td>
            </tr>
          </table>
	...
	<div id="rep1" style="display: none">
		  ...
		  <tr><td style="width: 15%"><span class="style2" >*</span>Vigencia</td>
			...
			<tr><td style="width: 15%">Provincia</td>
		 	    <td style="width: 35%">
		 	    <select  name="prov" id="prov" style="width: 150px;" onchange="JavaScript: ajaxMunicipios();" <c:if test="${sessionScope.filtroReportesVO.hab_loc ==0}">DISABLED</c:if>>
		 	        <option value="-9">-- Todos--</option>
		 	         <c:forEach begin="0" items="${requestScope.listaProvincia}" var="item">
				     <option value="<c:out value="${item.codigo}"/>" <c:if test="${sessionScope.filtroReportesVO.prov ==item.codigo}">SELECTED</c:if>><c:out value="${item.nombre}"/></option>
                     </c:forEach>
		 	      </select>
		 	    </td>
		 	...
		 	<tr><td style="width: 15%">Sede</td>
		 	 ...
		 	 <tr id="verDocente" style="display: none">
		 	 <td><span class="style2" >*</span>Docente</td>
		 	    <td colspan="3"><select  name="docente" id="docente" style="width: 280px;">
		 	        <option value="-9">-- Todos--</option>
		 	         <c:forEach begin="0" items="${requestScope.listaDocente}" var="item">
				      <option value="<c:out value="${item.codigo2}"/>" <c:if test="${sessionScope.filtroReportesVO.docente == item.codigo2}">SELECTED</c:if>><c:out value="${item.nombre}"/></option>
                     </c:forEach>
		 	        </select>
		 	    </td>		 	   
		 	 </tr>  
