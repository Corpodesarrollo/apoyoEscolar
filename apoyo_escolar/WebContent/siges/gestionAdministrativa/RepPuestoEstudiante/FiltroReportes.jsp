		  ...
		  <caption>Gestión Administrativa >> Reporte Puesto Estudiante</caption>	         
		    <tr>						
			 <td rowspan="2" bgcolor="#FFFFFF">			      
				   <img src='<c:url value="/etc/img/tabs/reportes/puestoEstudiante_1.gif"/>' border="0"  height="26" alt='Reporte Puesto Estudiante'>
		 	...
		 	<tr><td style="width: 15%">Sede</td>
		 	 ...
		 	 <tr><td><span class="style2" ></span>Metodología</td>
		 	  ...
		 	  <tr><td>Grupo</td>
		 	    ...
		 	    <td>Asignatura</td>
		 	    <td><select  name="asig" id="asig" style="width: 180px;">
		 	        <option value="-9">-- Todas--</option>
		 	         <c:forEach begin="0" items="${requestScope.listaAsignatura}" var="item">
				      <option value="<c:out value="${item.codigo}"/>" <c:if test="${sessionScope.filtroReportesVO.asig == item.codigo}">SELECTED</c:if>><c:out value="${item.nombre}"/></option>
                     </c:forEach>
		 	        </select>
		 	    </td>
		 	    </tr>   
