...
function validarNumeros2(e) { 
  ...
  function nuevo(){
   ...
   function ajaxAsig(){
	  ...
	  if(document.frmNuevo.loc.value>0){
		  var cont=1;
				...
				var codsAsig_=document.frmNuevo.instCodigos.value;
				var aresig_=0;				
   ...
   function ajaxGradosSel(){
		  // alert(<c:out value="${sessionScope.filtroReportesVO.numAsig}"/>);
		  if(document.frmNuevo.loc.value>0){
			  var cont=1;
				  
					var inst_=document.frmNuevo.loc.value;				
					var codsAsig_=document.frmNuevo.gradosCodigos.value;
					var aresig_=0;				
					var tipoRep_ = document.frmNuevo.tipoReporte.value;
					var vig_ = document.frmNuevo.vig.value;			
					
				    var aux=document.frmNuevo.actionPopup.value+'?cmd='+<c:out value="${paramsVO.CMD_AJAX_GRADOS}"/>+'&tipo='+4+'&loc='+inst_+'&codsAsig='+codsAsig_+'&conAreAsig='+aresig_+'&tipoRep='+tipoRep_+'&vig='+vig_;
...
<body onLoad="mensaje(document.getElementById('msg'));">
			...
			<input type="hidden" name="tipo" value='<c:out value="${ paramsVO.FICHA_REPORTES4}"/>'>
   ...
   <!-- FORMUALARIO DE EDICION CREACION--> 		  
	<form method="post" name="frmNuevo"  action='<c:url value="/siges/gestionAdministrativa/RepPuestoEstudiante/Save.jsp"/>'>
		...
		<input type="hidden" name="tipoReporte" value='4'>
		...
		<input type="hidden" name="inst" value='-99'>
		 ...
		 <table cellpadding="0" cellspacing="1" border="0" ALIGN="CENTER" width="100%" >
		  <caption>Gestión Administrativa >> Reporte Mortalidad Colegios</caption>	         
		    <tr>						
			 <td rowspan="2" bgcolor="#FFFFFF">
				   ...
				   <img src='<c:url value="/etc/img/tabs/reportes/puestoMortalidad_1.gif"/>' border="0"  height="26" alt='Reporte mortalidad colegios'>
		   ...
		   <tr><td style="width: 15%"><span class="style2" >*</span>Vigencia</td>
			...
			<tr>
		 	...
		 	<tr>
		 	 ...
		 	 <tr>
		 	    <td colspan="4">
		 	    <textarea name="instNombres" id="instNombres"  cols="80" row="3" value='<c:out value="${sessionScope.filtroReportesVO.instNombres}"/>' readonly="true"></textarea>	
		 	  ...
		 	  <tr>
		 	    <td>Grados</td>
		 	    <td><a href='javaScript:ajaxGradosSel();'>
								<img border='0' src='<c:url value="/etc/img/todas.gif"/>' width='18' height='18' alt="Seleccionar Grados">
					 </a>
		 	    </td>
		 	 </tr>   
		 	 <tr>
		 	    <td colspan="4">
		 	    <textarea name="gradosNombres" id="gradosNombres"  cols="80" row="2" value='<c:out value="${sessionScope.filtroReportesVO.gradosNombres}"/>' readonly="true"></textarea>	
		 	    	<!-- <input type='text' name='asigNombres' id='asigNombres' size="80" maxlength="500" readonly="true" value='<c:out value="${sessionScope.filtrob.id}"/>'> -->
