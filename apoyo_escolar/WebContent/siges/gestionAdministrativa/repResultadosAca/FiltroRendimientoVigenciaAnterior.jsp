...
<jsp:useBean  id="filtroRendimientoVigenciaAnteriorVO" class="siges.gestionAdministrativa.repResultadosAca.vo.FiltroRendimientoVigenciaAnteriorVO" scope="session"/>
...
<body onLoad="mensaje(document.getElementById('msg'));">
   ...
   <!-- FORMUALARIO DE EDICION CREACION--> 		  
	<form method="post" name="frmNuevo"  action='<c:url value="/siges/gestionAdministrativa/repResultadosAca/Save2.jsp"/>'>
        <input type="hidden" name="tipo" value='<c:out value="${paramsVO.FICHA_COMPRENDIMIENTOACADEMICO}"/>'>
		...
		<table cellpadding="0" cellspacing="1" border="0" ALIGN="CENTER" width="100%" >
		  <caption>Gestión Administrativa >> Rendimiento con vigencia anterior</caption>	         
		    <tr>						
		     <td rowspan="1" bgcolor="#FFFFFF">			      
		           ...
		           <img src='<c:url value="/etc/img/tabs/resultadosAca_comparativo2.gif"/>' border="0"  height="26" alt='Reportes comparativo vigenica anterior'>		        
		        </td>
            </tr>
          </table>
			...
			<tr>
      			...
      			<select name="filVigencia" style='width:120px;'>
        			<option value='-99'>-- Seleccione una --</option>
        				<c:forEach begin="0" items="${requestScope.filtroVigencia}" var="vig">
							<option value="<c:out value="${vig.codigo}"/>" <c:if test="${sessionScope.filtroRendimientoVigenciaAnteriorVO.filVigencia==vig.codigo}">SELECTED</c:if>><c:out value="${vig.nombre}"/></option>
        				</c:forEach>
        		</select>
     			</td>
		 	 </tr> 
		 	...
		 	<tr><td style="width: 10%"><span class="style2" >*</span>Período</td>
		 	    <td style="width: 35%"><select  name="periodo" id="periodo" style="width: 80px;">
				     ...
				     <option value="<c:out value="${item.codigo}"/>" <c:if test="${sessionScope.filtroRendimientoVigenciaAnteriorVO.periodo ==item.codigo}">SELECTED</c:if>><c:out value="${item.nombre}"/></option>
                     </c:forEach>
		 	      </select>
		 	    </td>
