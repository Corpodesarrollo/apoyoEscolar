...
<body onLoad="mensaje(document.getElementById('msg'));">
   ...
   <!-- FORMUALARIO DE EDICION CREACION--> 		  
	<form method="post" name="frmNuevo"  action='<c:url value="/siges/gestionAdministrativa/repResultadosAca/Save.jsp"/>'>
        <input type="hidden" name="tipo" value='<c:out value="${paramsVO.FICHA_REPORTES}"/>'>
		...
		<table cellpadding="0" cellspacing="1" border="0" ALIGN="CENTER" width="100%" >
		  <caption>Gestión Administrativa >> Reportes Resultados Académicos</caption>	         
		    <tr>						
			 <td rowspan="1" bgcolor="#FFFFFF">			      
				   <img src='<c:url value="/etc/img/tabs/resultadosAca_1.gif"/>' border="0"  height="26" alt='Reportes Carnés'>
				   <a href="javaScript:lanzar(2)"><img src='<c:url value="/etc/img/tabs/resultadosAca_comparativo1.gif"/>' border="0"  height="26" alt='Resultados academicos'></a>		   
		        </td>
            </tr>
         </table>
		...
		<!-- REPORTE UNO -->
		  ...
		  <table border="0" align="center" width="100%" cellpadding="1" cellspacing="1">	
			...
			<tr>
		 	   <td style="width: 10%"><span class="style2" >*</span>Localidad</td>
		 	    <td style="width: 35%"><select  name="loc" id="loc" style="width: 190px;" onchange="JavaScript: ajaxColegios();" <c:if test="${sessionScope.filtroRepResultadosVO.hab_loc ==0}">DISABLED</c:if>>
		 	        <option value="-99">-- Seleccione Uno--</option>
		 	         <c:forEach begin="0" items="${requestScope.listaLocalidad}" var="item">
				     <option value="<c:out value="${item.codigo}"/>" <c:if test="${sessionScope.filtroRepResultadosVO.loc ==item.codigo}">SELECTED</c:if>><c:out value="${item.nombre}"/></option>
                     </c:forEach>
		 	      </select>
		 	    </td>
		 	 </tr> 
		 	 ...
		 	 <tr style="display:none;">
		 	    <td style="width: 10%"><span class="style2" >*</span>Colegio</td>
		 	    <td style="width: 35%" colspan="2"><select name="inst" id="inst" style="width: 350px;" onchange="ajaxPeriodo();" <c:if test="${sessionScope.filtroRepResultadosVO.hab_inst ==0}">DISABLED</c:if> >
		 	        <option value="-99">-- Todos--</option>
		 	          <c:forEach begin="0" items="${requestScope.listaColegio}" var="item">
				     <option value="<c:out value="${item.codigo}"/>" <c:if test="${sessionScope.filtroRepResultadosVO.inst ==item.codigo}">SELECTED</c:if>><c:out value="${item.nombre}"/></option>
                     </c:forEach>
		 	        </select>
		 	    </td>		 	    
		 	 </tr> 
		 	...
		 	<tr><td style="width: 10%"><span class="style2" >*</span>Período</td>
		 	    <td style="width: 35%"><select  name="periodo" id="periodo" style="width: 80px;">
		 	        <option value="-99">-- Seleccione uno--</option>
	...
	<!-- 
	 <table border="0" align="center" width="70%" cellpadding="5" cellspacing="3">
	 	<tr>
			<td colspan="2" align="center">
						<fieldset>
							...
							<table border="0" align="center" width="90%" cellpadding="1" cellspacing="0">
								 <tr>												
									<td colspan="0" align="left" width="70%">
										Listado Académico de Estudiantes
									</td>
									<td colspan="0" align="center" width="40%">
										<input type="radio" name="tipoReporte" value="1" <c:if test="${sessionScope.filtroRepResultadosVO.tipoReporte==1}">checked</c:if>>
									</td>
									</tr>
									<tr>
									<td colspan="0" align="left" width="70%">	
										Cuadro Resumen Resultados Académicos por Áreas&nbsp;&nbsp;
									</td>
									<td colspan="0" align="center" width="40%">	
										<input type="radio" name="tipoReporte"  value="2" <c:if test="${sessionScope.filtroRepResultadosVO.tipoReporte==2}">checked</c:if>>
									</td>
									</tr>
									<tr>
									<td colspan="0" align="left" width="70%">	
										Cuadro Resumen Resultados Académicos por Grados
									</td>
									<td colspan="0" align="center" width="50%">	
										<input type="radio" name="tipoReporte"  value="3" <c:if test="${sessionScope.filtroRepResultadosVO.tipoReporte==3}">checked</c:if>>
									</td>
								 </tr>
							</table>
						</fieldset>					
