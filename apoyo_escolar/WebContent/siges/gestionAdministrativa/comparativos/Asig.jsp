...
<head>
<title>Sistema de Gestión Escolar - SIGES- Selección Áreas/Asignaturas.</title>
<script languaje="javaScript">
var codigosAsig="-99";
var abrevAsig="";
var numChecks=0;
var numAsig=<c:out value="${sessionScope.filtroReportesVO.numAsig}"/>;
var tipoRep=<c:out value="${requestScope.ajaxTipoRep}"/>;
...
<c:if test="${requestScope.ajaxTipoRep!=1}">
                ...
                for(var i=0;i<forma.nuevoAsig_.length ;i++){
                    if(forma.nuevoAsig_[i].checked==true){
                        if(numAsig>1){
	                    	...
	                    	abrevAsig=forma.nuevoAsigNombre[i].value;
	                    	numChecks=numChecks+1;
	...
	function enviar(){
		...
		if(numChecks<=numAsig){
			...
			if(conAreAsig==1){
				alert("Ha seleccionado un número mayor de áreas a las permitidas, por favor verifique esta información");
			}else if(conAreAsig==2){
			...
			<c:if test="${!empty requestScope.listaAreasAsignaturas}">
				...
				<c:forEach begin="0" items="${requestScope.listaAreasAsignaturas}" var="lista" varStatus="st">
					<tr>
						...
						<c:if test="${requestScope.ajaxTipoRep!=1}">	
							<input type="radio" name="nuevoAsig_" <c:out value="${lista.padre2}"/>/><br>
						</c:if>	
