...
<c:import url="/observacion/Nuevo.do"><c:param name="tipo" value="${paramsVO.FICHA_OBSERVACION_ASIGNATURA}"/><c:param name="cmd" value="${paramsVO.CMD_NUEVO}"/></c:import>
<html>
<head>
...
<body onLoad="mensaje(document.getElementById('msg'));">
	<form action='<c:url value="/observacion/SaveEval.jsp"/>' method="post" name="frmNuevo" onSubmit="return validarForma(frmNuevo)">
		...
		<table border="1" align="center" width="95%" cellpadding="0" cellspacing="0" bordercolor="gray">
			...
			<c:forEach begin="0" items="${requestScope.listaObservacionAsignatura}" var="obs">
				<tr>
					<td><input type="hidden" name="obsAsignatura" value='<c:out value="${obs.codigo}"/>'><c:out value="${obs.nombre}"/></td>
