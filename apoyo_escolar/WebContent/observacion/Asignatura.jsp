	...
	<script language="javaScript">
		<!--
				...
				<c:if test="${!empty requestScope.listaSede && !empty requestScope.listaJornada}">
					...
					<c:forEach begin="0" items="${requestScope.listaSede}" var="sede" varStatus="st">
						...
						<c:forEach begin="0" items="${requestScope.listaJornada}" var="jornada">
							<c:if test="${sede.codigo==jornada.padre}"> Sel_Hijos[k] = '<c:if test="${sessionScope.observacionAsignaturaVO.obsJornada==jornada.codigo}">SELECTED</c:if>'; id_Hijos[k] = '<c:out value="${jornada.codigo}"/>';  Hijos[k] = '<c:out value="${jornada.nombre}"/>'; id_Padre[k] = '<c:out value="${jornada.padre}"/>'; k++;</c:if>
			...
			function ajaxGrado(){
				...
				document.frmAjax.param[2].value=document.frmLista.obsSede.options[document.frmLista.obsSede.selectedIndex].value;
				document.frmAjax.param[3].value=document.frmLista.obsJornada.options[document.frmLista.obsJornada.selectedIndex].value;
...
<body onLoad="mensaje(document.getElementById('msg'));">
	...
	<form method="post" name="frmLista" class="formulario-evaluacion-general" onSubmit="return validarForma(frmLista)" action='<c:url value="/observacion/SaveEval.jsp"/>'>
		<table cellpadding="0" cellspacing="0" border="0" align="center" bordercolor="#FFFFFF" width="95%" >
			<caption>OBSERVACIÓN POR ASIGNATURA</caption>
		</table>
		
		<!-- FICHAS A MOSTRAR AL USUARIO -->
		<input type="hidden" name="tipo" value='<c:out value="${paramsVO.FICHA_OBSERVACION_ASIGNATURA}"/>'>
		...
		<table cellpadding="0" cellspacing="0" border="0" ALIGN="CENTER" width="100%" class="menu_top">
			<tr height="1">
				...
				<td width="14%" height="26"><a class="active" href="javaScript:lanzarServicio2('<c:out value="${paramsVO.FICHA_OBSERVACION_ASIGNATURA}"/>')">Observación por Asignatura</a></td>
				<td width="14%" height="26"><a href="javaScript:lanzarServicio2('<c:out value="${paramsVO.FICHA_OBSERVACION_ESTUDIANTE}"/>')">Observación por Estudiante</a></td>	
			</tr>
		</table>
		...
		<table border="0" align="center" width="95%" cellpadding="3" cellspacing="0" class="tabla-filtros">
				...
				<td colspan="5">
					<select name="obsSede" style='width:350px;' onChange='filtro(document.frmLista.obsSede,document.frmLista.obsJornada)'>
						...
						<c:forEach begin="0" items="${requestScope.listaSede}" var="sede">
							<option value="<c:out value="${sede.codigo}"/>" <c:if test="${sede.codigo==sessionScope.observacionAsignaturaVO.obsSede}">selected</c:if>><c:out value="${sede.nombre}"/></option>
						</c:forEach>
					</select>
				</td>
			</tr>
			<tr>
				...
				<td>
					<select name="obsJornada" style='width:100px;' onchange="ajaxMetodologia()">
						<option value="-99">-- Seleccione una --</option>
					</select>
				</td>
				...
				<td>
					<select name="obsMetodologia" style='width:120px;' onchange="ajaxGrado()">
						...
						<c:forEach begin="0" items="${requestScope.listaMetodologia}" var="metod">
							<option value='<c:out value="${metod.codigo}"/>' <c:if test="${metod.codigo==sessionScope.observacionAsignaturaVO.obsMetodologia}">selected</c:if>><c:out value="${metod.nombre}"/></option>
						</c:forEach>
					</select>
				</td>
				...
				<td>
					<select name="obsGrado" style='width:100px;' onchange="ajaxGrupo()">
						...
						<c:forEach begin="0" items="${requestScope.listaGrado}" var="grado">
							<option value='<c:out value="${grado.codigo}"/>' <c:if test="${grado.codigo==sessionScope.observacionAsignaturaVO.obsGrado}">selected</c:if>><c:out value="${grado.nombre}"/></option>
						</c:forEach>
					</select>
				</td>
			</tr>
				...
				<td>
					<select name="obsGrupo" style='width:100px;'>
						...
						<c:forEach begin="0" items="${requestScope.listaGrupo}" var="grupo">
							<option value='<c:out value="${grupo.codigo}"/>' <c:if test="${grupo.codigo==sessionScope.observacionAsignaturaVO.obsGrupo}">selected</c:if>><c:out value="${grupo.nombre}"/></option>
						</c:forEach>
					</select>
				</td>
				...
				<td>
					<select name="obsPeriodo" style='width:100px;'>
						...
						<c:forEach begin="0" items="${requestScope.listaPeriodo}" var="per">
							<option value="<c:out value="${per.codigo}"/>" <c:if test="${per.codigo==sessionScope.observacionAsignaturaVO.obsPeriodo}">selected</c:if>><c:out value="${per.nombre}"/></option>
						</c:forEach>
					</select>
				</td>
			</tr>
