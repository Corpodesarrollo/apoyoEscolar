...
//				   validarLista(forma.tipodescriptores, "- Tipo de descriptor", 0);				   
			}
					...
					<c:if test="${!empty requestScope.filtroSedeF && !empty requestScope.filtroJornadaF}">
						...
						<c:set var="z" value="${fila[0]}" scope="request"/>
							...
							<c:if test="${fila[0]==fila2[2]}">
									Sel_Hijos[k] = '<c:if test="${sessionScope.filtroH.jornada== fila2[0]}">SELECTED</c:if>';id_Hijos[k] = '<c:out value="${fila2[0]}"/>'; Hijos[k] = '<c:out value="${fila2[1]}"/>'; id_Padre[k] = '<c:out value="${fila2[2]}"/>'; k++;
							</c:if></c:forEach>
			...
			function filtro2(combo_padre,combo_padre2,combo_padre3,combo_hijo,combo_hijo2){
					...
					<c:if test="${!empty requestScope.filtroSedeF && !empty requestScope.filtroJornadaF && !empty requestScope.filtroGradoF2 && !empty requestScope.filtroMetodologiaF}">var Padres = new Array();
						...
						<c:if test="${fila2[2]==fila3[3] && fila2[0]==fila3[4] && fila3[5]==fila4[0]}">Sel_Hijos[k] = '<c:if test="${sessionScope.filtroH.grado== fila3[0]}">SELECTED</c:if>';id_Hijos[k] = '<c:out value="${fila3[0]}"/>'; Hijos[k] = '<c:out value="${fila3[1]}"/>'; id_Padre[k] = '<c:out value="${fila3[3]}"/>|<c:out value="${fila3[4]}"/>|<c:out value="${fila3[5]}"/>'; k++;</c:if></c:forEach>
						...
						<c:forEach begin="0" items="${requestScope.filtroGradoF2}" var="fila2"  varStatus="st">
							...
							<c:forEach begin="0" items="${requestScope.filtroGrupoF}" var="fila3">
								<c:if test="${fila2[0]==fila3[4] && fila2[3]==fila3[2] && fila2[4]==fila3[3] && fila2[5]==fila3[5]}">
									Sel_Hijos[k] = '<c:if test="${sessionScope.filtroH.grupo== fila3[0]}">SELECTED</c:if>';id_Hijos[k] = '<c:out value="${fila3[0]}"/>'; Hijos[k] = '<c:out value="${fila3[1]}"/>'; id_Padre[k] = '<c:out value="${fila3[2]}"/>|<c:out value="${fila3[3]}"/>|<c:out value="${fila3[5]}"/>|<c:out value="${fila3[4]}"/>'; k++;
								</c:if>
							</c:forEach>
						Padres[<c:out value="${st.index}"/>] = new Padre(id_Hijos, Hijos, Sel_Hijos,id_Padre);
						</c:forEach>
					...
					<c:if test="${!empty requestScope.filtroMetodologiaF && !empty requestScope.filtroGradoF2 && !empty requestScope.filtroAreaF}">
						var Padres = new Array();
							...
							<c:forEach begin="0" items="${requestScope.filtroAreaF}" var="fila3">
								<c:if test="${fila3[3]==fila4[0] && fila3[2]==fila4[5]}">
									Sel_Hijos[k] = '<c:if test="${sessionScope.filtroH.area== fila3[0]}">SELECTED</c:if>';
...
<%@include file="../mensaje.jsp"%>
	<form method="post" name="frm"  onSubmit="return validarForma(frm)" action='<c:url value="/reporte/FiltroGuardarEvaluacionTipoDescriptor.jsp"/>'>
		...
		<tr height="1">
			...
			<td rowspan="2" width="588" bgcolor="#FFFFFF"><a href="javaScript:lanzar(1)"><img src="../etc/img/tabs/asistencia_entrega_boletines_0.gif" width="84" height="26" border="0"></a><a href="javaScript:lanzar(2)"><img src="../etc/img/tabs/evaluacion_logros_grupo_0.gif" width="84" height="26" border="0"></a><a href="javaScript:lanzar(3)"><img src="../etc/img/tabs/evaluacion_asig_grupo_0.gif" width="84"  height="26" border="0"></a><a href="javaScript:lanzar(4)"><img src="../etc/img/tabs/evaluacion_areas_grupo_0.gif" width="84"  height="26" border="0"></a><a href="javaScript:lanzar(5)"><img src="../etc/img/tabs/evaluacion_logros_grupo1_0.gif" width="84" height="26" border="0"></a><a href="javaScript:lanzar(6)"><img src="../etc/img/tabs/evaluacion_asig_por_grupo_0.gif" width="84"  height="26" border="0"></a><a href="javaScript:lanzar(7)"><img src="../etc/img/tabs/evaluacion_area_por_grupo_0.gif" width="84"  height="26" border="0"></a><img src="../etc/img/tabs/eval_tipo_de_descriptor_1.gif" width="84"  height="26" border="0"></td>
										    ...
										    <td>
												<select name="sede" onChange='filtro(document.frm.sede, document.frm.jornada,document.frm.grado,document.frm.grupo,document.frm.metodologia)' style='width:300px;'>
													...
													<c:forEach begin="0" items="${requestScope.filtroSedeF}" var="fila">
														<option value="<c:out value="${fila[0]}"/>" <c:if test="${sessionScope.filtroH.sede==fila[0]}">SELECTED</c:if>><c:out value="${fila[1]}"/></option>
													</c:forEach>
												</select>											
									    </td>
                        ...
                        <c:forEach begin="0" items="${requestScope.filtroMetodologiaF}" var="fila">
												<option value="<c:out value="${fila[0]}"/>" <c:if test="${sessionScope.filtroH.metodologia==fila[0]}">SELECTED</c:if>><c:out value="${fila[1]}"/></option>
                        </c:forEach>
                        </select>
                      </td>
										  <td><span class="style2" >*</span>Grado:</td>
										  <td>
                      	<select name="grado" onChange='javaScript:filtro3(document.frm.sede, document.frm.jornada,document.frm.metodologia,document.frm.grado,document.frm.grupo);filtro4(document.frm.metodologia,document.frm.grado,document.frm.area)' style='width:120px'>
										  	<option value='-9'>-- seleccione uno --</option>
										  	</select>
                      </td>
								    </tr>
                        ...
                        <option value="<c:out value="${fila[0]}"/>" <c:if test="${sessionScope.filtroH.periodo== fila[0]}">SELECTED</c:if>> <c:out value="${fila[1]}"/> </c:forEach>
                        </select>
											</td>
										</tr>
										<tr>
                      	...
                      	<option value='-9'>-- Todos --</option>
										...
										<tr>
											<td style='display:none;'><span class="style2">*</span>Tipo de descriptor:</td>
										  <td style='display:none;'>
                      	<select name="tipodescriptores" size="4" multiple  style='width:120px;'>
                        <c:forEach begin="0" items="${requestScope.filtroTipoDescriptor}" var="fila">
                        <option value='<c:out value="${fila[0]}"/>' <c:if test="${sessionScope.filtroH.tipodescriptores== fila[0]}">SELECTED</c:if>><c:out value="${fila[1]}"/></c:forEach>
                        </select>
										  </td>
		 							   </tr>
								</TABLE>
