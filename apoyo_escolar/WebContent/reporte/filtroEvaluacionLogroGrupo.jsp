...
<%@include file="../parametros.jsp"%><c:set var="tip" value="1" scope="page"/>
		...
		<!--
			...
			function hacerValidaciones_frm(forma){
				   ...
				   validarEntero(forma.parametro,"- Parámetro [1-2])",1,99);
					...
					<c:if test="${!empty requestScope.filtroSedeF && !empty requestScope.filtroJornadaF}">
						...
						<c:set var="z" value="${fila[0]}" scope="request"/>
							...
							<c:if test="${fila[0]==fila2[2]}">
									Sel_Hijos[k] = '<c:if test="${sessionScope.filtroE.jornada== fila2[0]}">SELECTED</c:if>';id_Hijos[k] = '<c:out value="${fila2[0]}"/>'; Hijos[k] = '<c:out value="${fila2[1]}"/>'; id_Padre[k] = '<c:out value="${fila2[2]}"/>'; k++;
							</c:if></c:forEach>
			...
			function filtro2(combo_padre,combo_padre2,combo_padre3,combo_hijo,combo_hijo2){
					...
					<c:if test="${!empty requestScope.filtroSedeF && !empty requestScope.filtroJornadaF && !empty requestScope.filtroGradoF2 && !empty requestScope.filtroMetodologiaF}">var Padres = new Array();
						...
						<c:if test="${fila2[2]==fila3[3] && fila2[0]==fila3[4] && fila3[5]==fila4[0]}">Sel_Hijos[k] = '<c:if test="${sessionScope.filtroE.grado== fila3[0]}">SELECTED</c:if>';id_Hijos[k] = '<c:out value="${fila3[0]}"/>'; Hijos[k] = '<c:out value="${fila3[1]}"/>'; id_Padre[k] = '<c:out value="${fila3[3]}"/>|<c:out value="${fila3[4]}"/>|<c:out value="${fila3[5]}"/>'; k++;</c:if></c:forEach>
						...
						<c:forEach begin="0" items="${requestScope.filtroGradoF2}" var="fila2"  varStatus="st">
							...
							<c:forEach begin="0" items="${requestScope.filtroGrupoF}" var="fila3">
								<c:if test="${fila2[0]==fila3[4] && fila2[3]==fila3[2] && fila2[4]==fila3[3] && fila2[5]==fila3[5]}">
									Sel_Hijos[k] = '<c:if test="${sessionScope.filtroE.grupo== fila3[0]}">SELECTED</c:if>';id_Hijos[k] = '<c:out value="${fila3[0]}"/>'; Hijos[k] = '<c:out value="${fila3[1]}"/>'; id_Padre[k] = '<c:out value="${fila3[2]}"/>|<c:out value="${fila3[3]}"/>|<c:out value="${fila3[5]}"/>|<c:out value="${fila3[4]}"/>'; k++;
								</c:if>
							</c:forEach>
						Padres[<c:out value="${st.index}"/>] = new Padre(id_Hijos, Hijos, Sel_Hijos,id_Padre);
						</c:forEach>
...
<%@include file="../mensaje.jsp"%>
	<form method="post" name="frm"  onSubmit="return validarForma(frm)" action='<c:url value="/reporte/FiltroGuardarEvaluacionLogroGrupo.jsp"/>'>
...
<!--//////////////////-->		
	...
	<table cellpadding="0" cellspacing="0" border="0" ALIGN="CENTER" width="100%">
		<tr height="1">
			...
			<td rowspan="2" width="588" bgcolor="#FFFFFF"><a href="javaScript:lanzar(1)"><img src="../etc/img/tabs/asistencia_entrega_boletines_0.gif" width="84" height="26" border="0"></a><a href="javaScript:lanzar(2)"><img src="../etc/img/tabs/evaluacion_logros_grupo_0.gif" width="84" height="26" border="0"></a><a href="javaScript:lanzar(3)"><img src="../etc/img/tabs/evaluacion_asig_grupo_0.gif" width="84"  height="26" border="0"></a><a href="javaScript:lanzar(4)"><img src="../etc/img/tabs/evaluacion_areas_grupo_0.gif" width="84"  height="26" border="0"></a><img src="../etc/img/tabs/evaluacion_logros_grupo1_1.gif" width="84" height="26" border="0"><a href="javaScript:lanzar(6)"><img src="../etc/img/tabs/evaluacion_asig_por_grupo_0.gif" width="84"  height="26" border="0"></a><a href="javaScript:lanzar(7)"><img src="../etc/img/tabs/evaluacion_area_por_grupo_0.gif" width="84"  height="26" border="0"></a><a href="javaScript:lanzar(8)"><img src="../etc/img/tabs/eval_tipo_de_descriptor_0.gif" width="84"  height="26" border="0"></a></td>
		</tr>
										...
										<tr>
										    ...
										    <td>
												<select name="sede" onChange='filtro(document.frm.sede, document.frm.jornada,document.frm.grado,document.frm.grupo,document.frm.metodologia)' style='width:300px;'>
													...
													<option value="<c:out value="${fila[0]}"/>" <c:if test="${sessionScope.filtroE.sede==fila[0]}">SELECTED</c:if>><c:out value="${fila[1]}"/></c:forEach>
												</select>											
									    </td>
                        ...
                        <c:forEach begin="0" items="${requestScope.filtroMetodologiaF}" var="fila">
												<option value="<c:out value="${fila[0]}"/>" <c:if test="${sessionScope.filtroE.metodologia==fila[0]}">SELECTED</c:if>><c:out value="${fila[1]}"/></option>
                        </c:forEach>
                        </select>
                      </td>
                        ...
                        <option value="<c:out value="${fila[0]}"/>" <c:if test="${sessionScope.filtroE.periodo== fila[0]}">SELECTED</c:if>> <c:out value="${fila[1]}"/> </c:forEach>
                        </select>
											</td>
										</tr>
										<tr>
											<td><span class="style2" >*</span>Par&aacute;metro:</td>
											<td>
										  	<input type='text' name='parametro'  maxlength="2" size="3" value='<c:out value="${sessionScope.filtroE.parametro}"/>' onKeyPress='return acepteNumeros(event)'>
										  </td>
										</tr>		
										<tr>								  
										  ...
										  <td style='display:none;'>
										  	<select name="orden" >
										  		<option value='-9' <c:if test="${sessionScope.filtroE.orden== '-9'}">SELECTED</c:if>>Apellidos</option>
										  		<option value='0' <c:if test="${sessionScope.filtroE.orden== '0'}">SELECTED</c:if>>Identificación</option>
										  		<option value='1' <c:if test="${sessionScope.filtroE.orden== '1'}">SELECTED</c:if>>Nombres</option>
										  		<option value='2' <c:if test="${sessionScope.filtroE.orden== '2'}">SELECTED</c:if>>Código</option>
										  	</select>										  
										  </td>
		 							   </tr>
								</TABLE>
