...
<!--//////////////////-->	
	...
	<TABLE width="100%" cellpadding="1" cellSpacing="0">
		...
		<tr>
				...
				<td>
        	<select name="orden">
						<option value='-9' <c:if test="${requestScope.filtroAreas.orden== '-9'}">SELECTED</c:if>>Área</option>
						<option value='0' <c:if test="${requestScope.filtroAreas.orden== '0'}">SELECTED</c:if>>Asignatura</option>
						<option value='2' <c:if test="${requestScope.filtroAreas.orden== '1'}">SELECTED</c:if>>Código</option>
					</select>																			  											
        </td>
		</tr>
	</TABLE>
