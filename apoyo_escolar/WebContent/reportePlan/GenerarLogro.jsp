...
<jsp:useBean id="filtroLogros" class="siges.reportePlan.beans.FiltroBeanReportesLogros" scope="session"/><jsp:setProperty name="filtroLogros" property="*"/>
		...
		<script languaje='JavaScript'>
					...
					<c:if test="${!empty requestScope.filtroSedeF && !empty requestScope.filtroJornadaF}">
						...
						<c:set var="z" value="${fila[0]}" scope="request"/>
							...
							<c:if test="${fila[0]==fila2[2]}">
									Sel_Hijos[k] = '<c:if test="${requestScope.filtroLogros.jornada== fila2[0]}">SELECTED</c:if>';id_Hijos[k] = '<c:out value="${fila2[0]}"/>'; Hijos[k] = '<c:out value="${fila2[1]}"/>'; id_Padre[k] = '<c:out value="${fila2[2]}"/>'; k++;
							</c:if></c:forEach>
						...
						<c:set var="z" value="${fila[0]}" scope="request"/>
							...
							<c:if test="${fila[0]==fila2[2]}">
								<c:set var="y" value="${fila2[2]}" scope="request"/>									
									Sel_Hijos[k] = '<c:if test="${sessionScope.filtroLogros.grado== fila2[0]}">SELECTED</c:if>';
			...
			function filtro3(combo_padre,combo_padre2,combo_hijo){
				if(combo_padre2.selectedIndex==0){
					...
					borrar_combo(combo_hijo);
					<c:if test="${!empty requestScope.filtroMetodologiaF && !empty requestScope.filtroGradoF2 && !empty requestScope.filtroAsignaturaF}">
						var Padres = new Array();
							<c:forEach begin="0" items="${requestScope.filtroGradoF2}" var="fila4" varStatus="st">
							id_Hijos=new Array(); Hijos= new Array(); Sel_Hijos= new Array(); id_Padre= new Array(); var k=0;
							<c:forEach begin="0" items="${requestScope.filtroAsignaturaF}" var="fila3">
								<c:if test="${fila3[3]==fila4[0] && fila3[2]==fila4[2]}">
									Sel_Hijos[k] = '<c:if test="${requestScope.filtroLogros.asignatura== fila3[0]}">SELECTED</c:if>';
...
<%@include file="../mensaje.jsp"%>
	<form method="post" name="frm" onSubmit="return validarForma(frm)" action='<c:url value="/reportePlan/FiltroGuardarLogro.jsp"/>'>
	<input type="hidden" name="repTipo" id="repTipo" value="9">
	<input type="hidden" name="repOrden" id="repOrden" value="2">
	
	<table border="0" align="center" width="100%">
	
	<caption>Generar Reporte de Logros</caption>
		<tr>
		  <td width="45%">
...
<!--//////////////////-->	
	...
	<input type="hidden" name="sedenom" value='<c:out value="${requestScope.filtroLogros.sedenom}"/>'>
	<input type="hidden" name="jornadanom" value='<c:out value="${requestScope.filtroLogros.jornadanom}"/>'>
	<input type="hidden" name="cmd" value="Cancelar">
	<INPUT TYPE="hidden" NAME="ext2"  VALUE=''><INPUT TYPE="hidden" NAME="height" VALUE='180'>

	<table border="0" align="center" width="100%">
		<tr height="1">
			<td rowspan="2" width="420"><a href="javaScript:lanzar(1)"><img src="../etc/img/tabs/plan_de_estudios_0.gif" alt="Reporte de &Aacute;reas y Asignaturas" width="84"  height="26" border="0"></a><a href="javaScript:lanzar(2)"><img src="../etc/img/tabs/logro_1.gif" alt="Reporte de Logros" width="84"  height="26" border="0"></a><a href="javaScript:lanzar(3)"><img src="../etc/img/tabs/descriptor_0.gif" alt="Reporte de Descriptores" width="84"  height="26" border="0"></a></td>
		</tr>
  </table>
		...
		<tr>
        	...
        	<select name="metodologia" style='width:120px;' onChange='filtro2(document.frm.metodologia,document.frm.grado)'>
          	<option value='-9'>-- Seleccione una --</option>
						...
						<option value="<c:out value="${fila[0]}"/>" <c:if test="${requestScope.filtroLogros.metodologia==fila[0]}">SELECTED</c:if>><c:out value="${fila[1]}"/></option>
						</c:forEach>
          </select>
        </td>
        </tr>
			<tr>
				<td>Grado:</td>
				<td>
        	<select name="grado" style='width:120px;' onChange='filtro3(document.frm.metodologia,document.frm.grado,document.frm.asignatura)'>
            ...
            <option value='<c:out value="${fila.codigo}"/>'><c:out value="${fila.nombre}"/></option>
				...
				<td>		
        	<select name="orden">
						<option value='-9' <c:if test="${requestScope.filtroLogros.orden== '-9'}">SELECTED</c:if>>Asignatura</option>
						<option value='1' <c:if test="${requestScope.filtroLogros.orden== '1'}">SELECTED</c:if>>Código</option>
					</select>																			  											
        </td>
			</tr>
		</TABLE>
