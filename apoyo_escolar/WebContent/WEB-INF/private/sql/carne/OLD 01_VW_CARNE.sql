...
SELECT inscodigo,insnombre,estcodigo,grucodigo,estgrupo,SEDCODIGO,sednombre,gc.G_ConCodigo,gc.G_ConNombre,GRACODMETOD,gracodigo,granombre,grunombre,gc2.G_ConAbrev,estnumdoc,CASE estgenero WHEN 1 THEN 'M' ELSE 'F' END AS genero,estedad,CONCAT(EstNombre1,CONCAT(' ',EstNombre2)),CONCAT(EstApellido1,CONCAT(' ',EstApellido2)),EstVigencia,
ESTFECHANAC
FROM SEDE,G_CONSTANTE gc,G_CONSTANTE gc2,GRADO,G_JERARQUIA a,ESTUDIANTE,INSTITUCION,GRUPO
WHERE a.g_jertipo=1 AND a.g_jernivel=8
AND estgrupo=a.g_jercodigo
AND inscodigo=a.g_jerinst
AND a.g_jerinst=sedcodins
...
AND grucodigo=a.g_jergrupo
AND grucodjerar IN
(SELECT b.g_jercodigo
FROM G_JERARQUIA b
WHERE b.g_jertipo=1 AND b.g_jernivel=7 AND b.g_jerinst=a.g_jerinst AND b.g_jersede=a.g_jersede AND b.g_jerjorn=a.g_jerjorn AND b.g_jergrado=a.g_jergrado AND b.g_jermetod=a.g_jermetod)
GROUP BY (inscodigo,insnombre,estcodigo,grucodigo,estgrupo,SEDCODIGO,sednombre,gc.G_ConCodigo,gc.G_ConNombre,GRACODMETOD,gracodigo,granombre,grunombre,gc2.G_ConAbrev,estnumdoc,estgenero,estedad,EstNombre1,EstNombre2,EstApellido1,EstApellido2,EstVigencia,ESTFECHANAC)
/
