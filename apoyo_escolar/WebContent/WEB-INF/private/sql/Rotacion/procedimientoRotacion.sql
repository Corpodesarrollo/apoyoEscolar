...
CREATE OR REPLACE PACKAGE PK_ROTACION AS
	PROCEDURE getAsignatura(id in NUMERIC,inst in NUMERIC,sede in NUMERIC,jor in NUMERIC,met in NUMERIC,grado in NUMERIC,vigencia in NUMERIC,horas in numeric,indiceGrupo in numeric,jerGrupo in numeric);
	...
	PROCEDURE validarEspacio(id in NUMERIC,inst in NUMERIC,sede in NUMERIC,jor in NUMERIC,met in NUMERIC,grado in NUMERIC,vigencia in NUMERIC);
END PK_ROTACION;
/
/*<TOAD_FILE_CHUNK>*/

CREATE OR REPLACE PACKAGE BODY PK_ROTACION AS
  PROCEDURE getAsignatura(id in NUMERIC,inst in NUMERIC,sede in NUMERIC,jor in NUMERIC,met in NUMERIC,grado in NUMERIC,vigencia in NUMERIC,horas in numeric,indiceGrupo in numeric,jerGrupo in numeric) IS
	asig number(6);
	ih number(3);
	cat number(3);
	doc number(12);
  ...
  BEGIN
	...
	--traer la iteracion mas grande en la que ha estado
	SELECT count(ROTNNIVEL) into iteracion FROM ROT_NIVEL where ROTNGRUPO=jerGrupo and ROTNCATEGORIA=(SELECT nvl(max(ROTNCATEGORIA),-1)
	FROM ROT_NIVEL where ROTNGRUPO=jerGrupo);
	if iteracion>0 then
	   SELECT ROTNNIVEL into iteracion FROM ROT_NIVEL where ROTNGRUPO=jerGrupo and ROTNCATEGORIA=(SELECT nvl(max(ROTNCATEGORIA),-1)
		FROM ROT_NIVEL where ROTNGRUPO=jerGrupo);
	end if;	
	...
	FOR rec in cursor_asig LOOP	
		...
		if(cat=0) then 
				  indiceGeneral:=mod(indiceGrupo,totalCat0);
				  
				  indiceCat0:=indiceCat0+1;
				  if ((indiceCat0+indiceGeneral)>totalCat0) then
				  	 orden:=indiceCat0+indiceGeneral-totalCat0;
				  else
				  	 orden:=indiceCat0+indiceGeneral;	  	 
				  end if;
		end if;			
		if(cat=1) then
				  indiceGeneral:=mod(indiceGrupo,totalCat1); 
				  indiceCat1:=indiceCat1+1;
				  if ((indiceCat1+indiceGeneral)>totalCat1) then
				  	 orden:=indiceCat1+indiceGeneral-totalCat1;
				  else
				  	 orden:=indiceCat1+indiceGeneral;	  	 
				  end if;
		end if;			
		if(cat=15) then 
				   indiceGeneral:=mod(indiceGrupo,totalCat15);
				  indiceCat15:=indiceCat15+1;
				  if ((indiceCat15+indiceGeneral)>totalCat15) then
				  	 orden:=indiceCat15+indiceGeneral-totalCat15;
				  else
				  	 orden:=indiceCat15+indiceGeneral;	  	 
				  end if;
		end if;			
		if(cat=2) then 
				indiceGeneral:=mod(indiceGrupo,totalCat2);
				  indiceCat2:=indiceCat2+1; 
				  if ((indiceCat2+indiceGeneral)>totalCat2) then
				  	 orden:=indiceCat2+indiceGeneral-totalCat2;
				  else
				  	 orden:=indiceCat2+indiceGeneral;	  	 
				  end if;
		end if;			
		if(cat=25) then
				indiceGeneral:=mod(indiceGrupo,totalCat25); 
				  indiceCat25:=indiceCat25+1; 
				  if ((indiceCat25+indiceGeneral)>totalCat25) then
				  	 orden:=indiceCat25+indiceGeneral-totalCat25;
				  else
				  	 orden:=indiceCat25+indiceGeneral;
				  end if;
		end if;			
		if(cat=3) then
				  indiceGeneral:=mod(indiceGrupo,totalCat3); 
				  indiceCat3:=indiceCat3+1; 
				  if ((indiceCat3+indiceGeneral)>totalCat3) then
				  	 orden:=indiceCat3+indiceGeneral-totalCat3;
				  else
				  	 orden:=indiceCat3+indiceGeneral;	  	 
				  end if;
		end if;			
		if(cat=35) then
				indiceGeneral:=mod(indiceGrupo,totalCat35); 
				  indiceCat35:=indiceCat35+1; 
				  if ((indiceCat35+indiceGeneral)>totalCat35) then
				  	 orden:=indiceCat35+indiceGeneral-totalCat35;
				  else
				  	 orden:=indiceCat35+indiceGeneral;	  	 
				  end if;
		end if;			
		if(cat=4) then
				indiceGeneral:=mod(indiceGrupo,totalCat4); 
				  indiceCat4:=indiceCat4+1; 
				  if ((indiceCat4+indiceGeneral)>totalCat4) then
				  	 orden:=indiceCat4+indiceGeneral-totalCat4;
				  else
				  	 orden:=indiceCat4+indiceGeneral;	  	 
				  end if;
		end if;			
		if(cat=45) then
				indiceGeneral:=mod(indiceGrupo,totalCat45); 
				  indiceCat45:=indiceCat45+1; 
				  if ((indiceCat45+indiceGeneral)>totalCat45) then
				  	 orden:=indiceCat45+indiceGeneral-totalCat45;
				  else
				  	 orden:=indiceCat45+indiceGeneral;	  	 
				  end if;
		end if;			
		if(cat=5) then
				  indiceGeneral:=mod(indiceGrupo,totalCat5); 
				  indiceCat5:=indiceCat5+1; 
				  if ((indiceCat5+indiceGeneral)>totalCat5) then
				  	 orden:=indiceCat5+indiceGeneral-totalCat5;
				  else
				  	 orden:=indiceCat5+indiceGeneral;	  	 
				  end if;
		end if;			
		INSERT INTO ROT_ASIG_TEMP (ID, ASIGNATURA, CATEGORIA, PRIORIDAD,DOCENTE, ESPACIO,IH,BLOQUE,ORDEN) VALUES (id,rec.asig_,cat,prioridad,doc,esp,rec.ih_,bloque,orden);
	END LOOP;
	commit;
	EXCEPTION
	WHEN others THEN
  ...
  PROCEDURE validarEspacio(id in NUMERIC,inst in NUMERIC,sede in NUMERIC,jor in NUMERIC,met in NUMERIC,grado in NUMERIC,vigencia in NUMERIC) IS
	cont number(3);
	clases number(3);
	jerJor number(11);
	espa number(5);
	...
	total number(5);
	CURSOR cursor IS
		SELECT ROTEAGESPACIO,ROTEAGASIGNATURA
		FROM ROT_ESPACIO_ASIG_GRADO
		where ROTEAGJERJORNADA=jerJor
		and ROTEAGVIGENCIA=vigencia
		group by ROTEAGESPACIO,ROTEAGASIGNATURA;
	CURSOR cursor2 IS
		SELECT ROTEAGJERGRADO
		FROM ROT_ESPACIO_ASIG_GRADO
		where ROTEAGJERJORNADA=jerJor
		and ROTEAGESPACIO=espa
		and ROTEAGVIGENCIA=vigencia
		and ROTEAGASIGNATURA=asig;			 
  BEGIN
	cont:=0;
	clases:=0;
	delete from ROT_VALIDAR_ESPACIO WHERE ROTVALID=id;
	SELECT nvl(max(G_JERCODIGO),0) into jerJor
	FROM G_JERARQUIA
	where G_JERTIPO=1
	and G_JERNIVEL=6
	...
	FOR rec in cursor LOOP
		total:=0;
		espa:=rec.ROTEAGESPACIO;
		asig:=rec.ROTEAGASIGNATURA;
		FOR rec2 in cursor2 LOOP
			--total de grupos 
			SELECT count(G_JERGRUPO) into grupos 
			FROM G_JERARQUIA
			where G_JERTIPO=1
			and G_JERNIVEL=8
			and G_JERINST=inst
			...
			and G_JERJORN=jor
			and G_JERMETOD=met
			and G_JERGRADO=rec2.ROTEAGJERGRADO;
			--IH  
			SELECT GRAASIIH into ih			   
			FROM GRADO_ASIGNATURA
			where GRAASICODINST=inst
			and GRAASICODGRADO=rec2.ROTEAGJERGRADO
			and GRAASICODMETOD=met 
			and GRAASICODASIGN=rec.ROTEAGASIGNATURA
			and GRAASIVIGENCIA=vigencia;
			total:=total+(grupos*ih);
		END LOOP;
			SELECT count(ROTSTRDURBLQ) into cont  
			FROM ROT_ESTRUCTURA
			where ROTSTRJERCODIGO=jerJor
			and ROTSTRVIGENCIA=vigencia;
			if cont>0 then
				SELECT (max(ROTSTRDURBLQ)*min(ROTSTRNUMBLQ))*5 into clases 
				FROM ROT_ESTRUCTURA
				where ROTSTRJERCODIGO=jerJor
				and ROTSTRVIGENCIA=vigencia;
				if ((total)>(clases)) then
					INSERT INTO ROT_VALIDAR_ESPACIO (ROTVALID, ROTVALGRADO, ROTVALASIGNATURA,ROTVALN) 
					VALUES (id,rec.ROTEAGESPACIO,rec.ROTEAGASIGNATURA, (total)-(clases));
				end if;
			end if;
		END LOOP;
