...
CREATE OR REPLACE PACKAGE BODY PK_ROTACION AS
  ...
  BEGIN
	...
	FOR rec in cursor_asig LOOP	
		...
		--calcula docente		
			...
			from ROT_DOC_ASIG_GRADO,DOCENTE_SEDE_JORNADA
			where DOCSEDJORNUMDOCUM=ROTDAGDOCENTE
			and ROTDAGJERGRADO=jerGra 
			and ROTDAGASIGNATURA=rec.asig_
			and DOCSEDJORCODINST=inst
			and DOCSEDJORCODSEDE=sede 
			and DOCSEDJORCODJOR=jor 
			and (nvl(DOCSEDJORIHTOTAL,0)- nvl(DOCSEDJORIHREAL,0))>=rec.ih_;
			if cont=0 then
				doc:=0;
			else
				if cont=1 then
	...
	--EXCEPTION
	--WHEN others THEN
	--	RAISE_APPLICATION_ERROR (-666, 'Error');
	END getAsignatura;
  ...
  BEGIN
	...
	DECLARE CURSOR cursor_doc IS
		...
		and DOCSEDJORCODSEDE=sede 
		and DOCSEDJORCODJOR=jor 
		and (nvl(DOCSEDJORIHTOTAL,0)- nvl(DOCSEDJORIHREAL,0))>=ih;
		--espacios asignados a los docentes
	CURSOR cursor_esp IS
		SELECT RTEFDOESPACIO as espacio_
		FROM ROT_ESPACIO_DOCENTE,DOCENTE_SEDE_JORNADA
		where RTEFDODOCENTE=docente
		and DOCSEDJORNUMDOCUM=RTEFDODOCENTE
		and RTEFDOJERCODIGO=jerJor
		and DOCSEDJORCODINST=inst
		and DOCSEDJORCODSEDE=sede 
		and DOCSEDJORCODJOR=jor 
	...
	--EXCEPTION  
	--WHEN others THEN  
		--RAISE_APPLICATION_ERROR (-665, 'Error');	 
  END getDocEf;
...
--call PK_ROTACION.getAsignatura(1,29,2,1,1,6,2006,2);
--call PK_ROTACION.getDocEf(1,29,2,1,1,6,2006,75,20443570,-1);
--call PK_ROTACION.getDocEf(1,29,2,1,1,6,2006,60,-1,3);
--call PK_ROTACION.getDocEf(1,29,2,1,1,6,2006,9,-1,-1);


--select * from ROT_ASIG_TEMP
--select * from ROT_DOCEF_TEMP

--select count(*) from ROT_DOCEF_TEMP
--select count(*) from ROT_ASIG_TEMP



--call PK_ROTACION.getDocEf(1,29,2,1,1,6,2006,75,20443570,-1);
--call PK_ROTACION.getDocEf(1,29,2,1,1,6,2006,9,-1,-1,1);
