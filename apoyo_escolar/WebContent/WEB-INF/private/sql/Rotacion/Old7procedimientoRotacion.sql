...
CREATE OR REPLACE PACKAGE BODY PK_ROTACION AS
  ...
  PROCEDURE validarEspacio(id in NUMERIC,inst in NUMERIC,sede in NUMERIC,jor in NUMERIC,met in NUMERIC,grado in NUMERIC,vigencia in NUMERIC) IS
	...
	jerJor number(11);
	CURSOR cursor IS
		SELECT ROTEAGESPACIO,ROTEAGASIGNATURA,GRAASIIH,count(G_JERGRUPO) as grupos
		FROM ROT_ESPACIO_ASIG_GRADO,GRADO_ASIGNATURA,G_JERARQUIA
		where ROTEAGJERJORNADA=jerJor
		and GRAASICODINST=inst
		...
		and G_JERJORN=jor
		and G_JERMETOD=met
		and G_JERGRADO=ROTEAGJERGRADO
		group by ROTEAGESPACIO,ROTEAGASIGNATURA,GRAASIIH;
  BEGIN
	cont:=0;
	clases:=0;
	...
	FOR rec in cursor LOOP
			...
			if cont>0 then
				...
				FROM ROT_ESTRUCTURA
				where ROTSTRJERCODIGO=jerJor;
				if ((rec.GRAASIIH*rec.grupos)>(clases)) then
					INSERT INTO ROT_VALIDAR_ESPACIO (ROTVALID, ROTVALGRADO, ROTVALASIGNATURA,ROTVALN) 
					VALUES (id,rec.ROTEAGESPACIO,rec.ROTEAGASIGNATURA, (rec.GRAASIIH*rec.grupos)-(clases));
				end if;
			end if;
		END LOOP;
