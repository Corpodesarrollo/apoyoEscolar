...
CREATE OR REPLACE PACKAGE BODY PK_ROTACION AS
  ...
  PROCEDURE validarEspacio(id in NUMERIC,inst in NUMERIC,sede in NUMERIC,jor in NUMERIC,met in NUMERIC,grado in NUMERIC,vigencia in NUMERIC) IS
	...
	jerJor number(11);
	CURSOR cursor IS
		SELECT ROTEAGJERGRADO, ROTEAGASIGNATURA,GRAASIIH,count(ROTEAGESPACIO) as espacios
		FROM ROT_ESPACIO_ASIG_GRADO,GRADO_ASIGNATURA
		where ROTEAGJERJORNADA=jerJor
		and GRAASICODINST=inst
		and GRAASICODGRADO=ROTEAGJERGRADO
		and GRAASICODMETOD=met 
		and GRAASIVIGENCIA=vigencia
		and GRAASICODASIGN=ROTEAGASIGNATURA
		group by ROTEAGJERGRADO, ROTEAGASIGNATURA,GRAASIIH;
  BEGIN
	cont:=0;
	clases:=0;
	grupos:=0;
	...
	SELECT nvl(max(G_JERCODIGO),0) into jerJor
		...
		FOR rec in cursor LOOP
			...
			FROM ROT_ESTRUCTURA,ROT_ESTRUCTURA_GRADO
			where ROTSTRJERCODIGO=jerJor
			and ROTSTRCODIGO=RTESGR_STRCOD
			and RTESGRGRADO=rec.ROTEAGJERGRADO;
			if cont>0 then
				SELECT (ROTSTRDURBLQ*ROTSTRNUMBLQ)*5 into clases  
				FROM ROT_ESTRUCTURA,ROT_ESTRUCTURA_GRADO
				where ROTSTRJERCODIGO=jerJor
				and ROTSTRCODIGO=RTESGR_STRCOD
				and RTESGRGRADO=rec.ROTEAGJERGRADO;
				...
				and G_JERJORN=jor
				and G_JERMETOD=met 
				and G_JERGRADO=rec.ROTEAGJERGRADO;
				
				if ((rec.GRAASIIH*grupos)>(rec.espacios*clases)) then
					INSERT INTO ROT_VALIDAR_ESPACIO (ROTVALID, ROTVALGRADO, ROTVALASIGNATURA,ROTVALN) 
					VALUES (id,rec.ROTEAGJERGRADO,rec.ROTEAGASIGNATURA, ((rec.GRAASIIH*grupos)-(rec.espacios*clases)));
				end if;
			end if;
			i:=i+1;
		END LOOP;
