...
<!--
	...
	<c:choose>
	  ...
	  <c:when test="${requestScope.ajaxParam==paramsVO.CMD_AJAX_INSTANCIA}">
	  		borrar_combo(parent.document.frmNuevo.ranInstancia);
		  	<c:forEach begin="0" items="${requestScope.listaInstanciaVO}" var="linea" varStatus="st">
					parent.document.frmNuevo.ranInstancia.options[<c:out value="${st.count}"/>] = new Option('<c:out value="${linea.nombre}"/>','<c:out value="${linea.codigo}"/>');
				</c:forEach>
		</c:when>
	</c:choose>
//-->
