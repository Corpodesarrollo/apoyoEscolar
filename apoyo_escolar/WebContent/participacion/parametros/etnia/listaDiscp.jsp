...
<!--
	...
	function editar(n){
		if(document.frmLista.id2){
				document.frmLista.id2.value=n;
...
<body onLoad="mensaje(document.getElementById('msg'));">
	<form action='<c:url value="/participacion/parametros/Nuevo.do"/>' method="post" name="frmLista" onSubmit="return validarForma(frmLista)">
		<input TYPE="hidden" NAME="tipo" VALUE='<c:out value="${paramsVO.FICHA_ETNIA}"/>'>
		...
		<table cellpadding="0" cellspacing="0" border="0" ALIGN="CENTER" width="100%" >
			<tr height="1">
				...
				<td rowspan="2" width="469">
					<a href="javaScript:lanzarServicio(1)"><img src='<c:url value="/etc/img/tabs/participacion_discapacidad_0.gif"/>' alt="Parametros discapacidad" height="26" border="0"></a>
					<img src='<c:url value="/etc/img/tabs/participacion_etnias_1.gif"/>' alt="Etnias" height="26" border="0">
		...
		<table border="1" align="center" width="95%" cellpadding="0" cellspacing="0">
		 	<caption>LISTADO DE ETNIAS</caption>
		 	<c:if test="${empty requestScope.listaEtniasVO}">
				<tr>
					<th class="Fila1" colspan='6'>NO EXISTEN REGISTROS DE ETNIAS</th>
				</tr>
			</c:if>
			<c:if test="${!empty requestScope.listaEtniasVO}">
				<tr>
				...
				<c:forEach begin="0" items="${requestScope.listaEtniasVO}" var="lista" varStatus="st">
					<tr>
