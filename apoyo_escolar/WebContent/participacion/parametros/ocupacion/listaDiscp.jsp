...
<!--
	...
	function hacerValidaciones_frmLista(forma){
		validarSeleccion(forma.id4, "- Debe seleccionar un item");
	}
	
	function editar(n){
		if(document.frmLista.id4){
				document.frmLista.id4.value=n;
...
<body onLoad="mensaje(document.getElementById('msg'));">
	<form action='<c:url value="/participacion/parametros/Nuevo.do"/>' method="post" name="frmLista" onSubmit="return validarForma(frmLista)">
		<input TYPE="hidden" NAME="tipo" VALUE='<c:out value="${paramsVO.FICHA_OCUPACION}"/>'>
		...
		<table cellpadding="0" cellspacing="0" border="0" ALIGN="CENTER" width="100%" >
			<tr height="1">
				...
				<td rowspan="2" width="469">
					...
					<img src='<c:url value="/etc/img/tabs/participacion_ocupacion_1.gif"/>' alt="Parametros discapacidad" height="26" border="0">
		...
		<table border="1" align="center" width="95%" cellpadding="0" cellspacing="0">
		 	<caption>LISTADO DE OCUPACI&Oacute;N</caption>
		 	<c:if test="${empty requestScope.listaOcupacionVO}">
				<tr>
					<th class="Fila1" colspan='6'>NO EXISTEN REGISTROS DE OCUPACION</th>
				</tr>
			</c:if>
			<c:if test="${!empty requestScope.listaOcupacionVO}">
				<tr>
				...
				<c:forEach begin="0" items="${requestScope.listaOcupacionVO}" var="lista" varStatus="st">
					<tr>
