...
<!--
	...
	function hacerValidaciones_frmLista(forma){
		validarSeleccion(forma.id5, "- Debe seleccionar un item");
	}
	
	function editar(n){
		if(document.frmLista.id5){
				document.frmLista.id5.value=n;
...
<body onLoad="mensaje(document.getElementById('msg'));">
	<form action='<c:url value="/participacion/parametros/Nuevo.do"/>' method="post" name="frmLista" onSubmit="return validarForma(frmLista)">
		<input TYPE="hidden" NAME="tipo" VALUE='<c:out value="${paramsVO.FICHA_ROL}"/>'>
		...
		<table cellpadding="0" cellspacing="0" border="0" ALIGN="CENTER" width="100%" >
			<tr height="1">
				...
				<td rowspan="2" width="469">
					<a href="javaScript:lanzarServicio(1)"><img src='<c:url value="/etc/img/tabs/participacion_discapacidad_0.gif"/>' alt="Etnias" height="26" border="0"></a>
					<a href="javaScript:lanzarServicio(2)"><img src='<c:url value="/etc/img/tabs/participacion_etnias_0.gif"/>' alt="LGTB" height="26" border="0"></a>
					 <a href="javaScript:lanzarServicio(3)"><img src='<c:url value="/etc/img/tabs/participacion_lgtb_0.gif"/>' alt="Ocupaci&oacute;" height="26" border="0"></a>
					 <a href="javaScript:lanzarServicio(4)"><img src='<c:url value="/etc/img/tabs/participacion_ocupacion_0.gif"/>' alt="Rol" height="26" border="0"></a>
                    <img src='<c:url value="/etc/img/tabs/participacion_rol_1.gif"/>' alt="Parametros discapacidad" height="26" border="0">
				</td>
			</tr>
		</table>
		<table border="1" align="center" width="95%" cellpadding="0" cellspacing="0">
		 	<caption>LISTADO DE ROL</caption>
		 	<c:if test="${empty requestScope.listaRolVO}">
				<tr>
					<th class="Fila1" colspan='6'>NO EXISTEN REGISTROS DE ROL</th>
				</tr>
			</c:if>
			<c:if test="${!empty requestScope.listaRolVO}">
				<tr>
				...
				<c:forEach begin="0" items="${requestScope.listaRolVO}" var="lista" varStatus="st">
					<tr>
