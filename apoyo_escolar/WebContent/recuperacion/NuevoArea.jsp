...
<body onLoad="mensaje(document.getElementById('msg'));">
	<form method="post" name="frmAjax0" action='<c:url value="/recuperacion/Ajax.do"/>'>
		<INPUT TYPE="hidden" NAME="tipo" VALUE='<c:out value="${paramsVO.FICHA_RECUPERACION_AREA}"/>'>
	...
	<form action='<c:url value="/recuperacion/SaveArea.jsp"/>' method="post" name="frmFiltro" onSubmit="return validarForma(frmFiltro)">
		<input TYPE="hidden" NAME="tipo" VALUE='<c:out value="${paramsVO.FICHA_RECUPERACION_AREA}"/>'>
		...
		<table cellpadding="0" cellspacing="0" border="0" ALIGN="CENTER" width="100%" >
			<tr height="1">
				...
				<td rowspan="2" width="469">
					<img src='<c:url value="/etc/img/tabs/recuperacion_area_1.gif"/>' alt="Area" height="26" border="0">
					<a href="javaScript:lanzar(<c:out value="${paramsVO.FICHA_RECUPERACION_ASIGNATURA}"/>)"><img src='<c:url value="/etc/img/tabs/recuperacion_asig_0.gif"/>' alt="Asignatura" height="26" border="0"></a>
				</td>
			</tr>
		</table>
		
		<table border="0" align="center" width="95%">
		 	<caption>Recuperación de área</caption>
			<tr>
				<td>
		...
		<table border="0" align="center" width="95%">
			<tr>
				...
				<td>
					<select name="filVigencia" style="width:70px" onchange="javaScript:ajaxGrado()">
						<option value="-99" selected>--Seleccione uno--</option>
			...
			<tr>
				...
				<td>
					<select name="filArea" style="width:220px">
						<option value="-99" selected>--Seleccione uno--</option>
						<c:forEach begin="0" items="${requestScope.listaArea}" var="asig">
							<option value="<c:out value="${asig.codigo}"/>" <c:if test="${asig.codigo==sessionScope.filtroRecuperacionVO.filArea}">selected</c:if>><c:out value="${asig.nombre}"/></option>
						</c:forEach>
					</select>
				</td>
	...
	<form action='<c:url value="/recuperacion/SaveArea.jsp"/>' method="post" name="frmNuevo">
		<input TYPE="hidden" NAME="tipo" VALUE='<c:out value="${paramsVO.FICHA_RECUPERACION_AREA}"/>'>
		<input type="hidden" name="cmd" value=''>
		<input type="hidden" name="GUARDAR" value='<c:out value="${paramsVO.CMD_GUARDAR}"/>'>
		<input type="hidden" name="CANCELAR" value='<c:out value="${paramsVO.CMD_CANCELAR}"/>'>
		<table border="1" align="center" width="95%" cellpadding="0" cellspacing="0" bordercolor="gray">
		 	<caption>Lista de resultados de área</caption>
