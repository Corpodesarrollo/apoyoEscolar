...
<!--
	...
	<c:choose>
	  ...
	  <c:when test="${requestScope.ajaxParam==paramsVO.CMD_AJAX_GRUPO}">
		  	...
		  	<c:forEach begin="0" items="${requestScope.listaAsignatura}" var="doc" varStatus="st">
					parent.document.frmFiltro.filAsignatura.options[<c:out value="${st.count}"/>] = new Option('<c:out value="${doc.nombre}"/>','<c:out value="${doc.codigo}"/>');
				</c:forEach>
		</c:when>
	</c:choose>
//-->
