...
<!--
	...
	<c:choose>
	  ...
	  <c:when test="${requestScope.var==1}">
	  	borrar_combo(parent.document.<c:out value="${requestScope.forma}"/>.localidad);
		  	<c:forEach begin="0" items="${requestScope.col1}" var="fila" varStatus="st">
					parent.document.<c:out value="${requestScope.forma}"/>.localidad.options[<c:out value="${st.count}"/>] = new Option("<c:out value="${fila[1]}"/>","<c:out value="${fila[0]}"/>");
				</c:forEach>
		</c:when>
