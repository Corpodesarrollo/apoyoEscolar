...
<%@include file="../../parametros.jsp"%><c:set var="tip" value="1" scope="page"/>
		...
		<!--
			...
			function lista(forma){
				...
				forma.action='<c:url value="/ruta/reportes/FiltroGuardarReporte2.jsp"/>';
				forma.submit();
			}
					...
					<c:if test="${!empty requestScope.filtroSedeF && !empty requestScope.filtroJornadaF}">
						...
						<c:set var="z" value="${fila[0]}" scope="request"/>
							...
							<c:if test="${fila[0]==fila2[2]}">
									Sel_Hijos[k] = '<c:if test="${sessionScope.filtroNut.jornada== fila2[0]}">SELECTED</c:if>';id_Hijos[k] = '<c:out value="${fila2[0]}"/>'; Hijos[k] = '<c:out value="${fila2[1]}"/>'; id_Padre[k] = '<c:out value="${fila2[2]}"/>'; k++;
							</c:if></c:forEach>
			...
			function filtro2(combo_padre,combo_padre2,combo_padre3,combo_hijo,combo_hijo2){
					...
					<c:if test="${!empty requestScope.filtroSedeF && !empty requestScope.filtroJornadaF && !empty requestScope.filtroGradoF2 && !empty requestScope.filtroMetodologiaF}">var Padres = new Array();
						...
						<c:if test="${fila2[2]==fila3[3] && fila2[0]==fila3[4] && fila3[5]==fila4[0]}">Sel_Hijos[k] = '<c:if test="${sessionScope.filtroNut.grado== fila3[0]}">SELECTED</c:if>';id_Hijos[k] = '<c:out value="${fila3[0]}"/>'; Hijos[k] = '<c:out value="${fila3[1]}"/>'; id_Padre[k] = '<c:out value="${fila3[3]}"/>|<c:out value="${fila3[4]}"/>|<c:out value="${fila3[5]}"/>'; k++;</c:if></c:forEach>
						...
						<c:forEach begin="0" items="${requestScope.filtroGradoF2}" var="fila2"  varStatus="st">
							...
							<c:forEach begin="0" items="${requestScope.filtroGrupoF}" var="fila3">
								<c:if test="${fila2[0]==fila3[4] && fila2[3]==fila3[2] && fila2[4]==fila3[3] && fila2[5]==fila3[5]}">
									Sel_Hijos[k] = '<c:if test="${sessionScope.filtroNut.grupo== fila3[0]}">SELECTED</c:if>';id_Hijos[k] = '<c:out value="${fila3[0]}"/>'; Hijos[k] = '<c:out value="${fila3[1]}"/>'; id_Padre[k] = '<c:out value="${fila3[2]}"/>|<c:out value="${fila3[3]}"/>|<c:out value="${fila3[4]}"/>'; k++;
								</c:if>
							</c:forEach>
						Padres[<c:out value="${st.index}"/>] = new Padre(id_Hijos, Hijos, Sel_Hijos,id_Padre);
						</c:forEach>
...
<%@include file="../../mensaje.jsp"%>
	<font size="1">
	<form method="post" name="frm"  onSubmit="return validarForma(frm)" action='<c:url value="/ruta/reportes/FiltroGuardarReporte.jsp"/>'>
	<table border="1" align="center" bordercolor="#FFFFFF" width="90%">
	<caption>Reportes - Nutrici&oacute;n</caption>
		<tr>
		...
		<tr height="1">
			<td rowspan="2" width="90%" bgcolor="#FFFFFF">
			<img src="../../etc/img/tabs/nutricion_1.gif" width="84"  height="26" border="0">
			<a href="javaScript:lanzar(2)"><img src="../../etc/img/tabs/gestacion_0.gif" width="84" height="26" border="0"></a>
			</td>
		</tr>
	</table>
<!--//////////////////-->		
	...
	<input type="hidden" name="periodonom" value='<c:out value="${sessionScope.filtroNut.periodonom}"/>'>
										    ...
										    <td>
												<select name="sede" onChange='filtro(document.frm.sede, document.frm.jornada,document.frm.grado,document.frm.grupo,document.frm.metodologia)' style='width:300px;'>
													...
													<c:forEach begin="0" items="${requestScope.filtroSedeF}" var="fila">
														<option value="<c:out value="${fila[0]}"/>" <c:if test="${sessionScope.filtroNut.sede==fila[0]}">SELECTED</c:if>>
										...
										<tr>
										  ...
										  <td>
											<select name="estudiante" style='width:300px;'>
											  <option value='-9'>-- seleccione uno --</option>
											   <c:forEach begin="0" items="${requestScope.ruta_estudiantes}" var="fila">
												  <option value='<c:out value="${fila[0]}"/>'<c:if test="${sessionScope.filtroNut.estudiante== fila[0]}">SELECTED</c:if>> <c:out value="${fila[1]}"/> <c:out value="${fila[2]}"/> <c:out value="${fila[3]}"/></option>
											   </c:forEach>
											</select>
										  </td>
										<td> <span class="style2" >*</span>Periodo:</td>
										  <td>
                                              <select name="periodo" style='width:120px;'>
                                                ...
                                                " <c:if test="${sessionScope.filtroNut.periodo== fila[0]}">SELECTED</c:if>> <c:out value="${fila[1]}"/> </option></c:forEach>
                                              </select>
										  </td>
										  <tr>
											<td>Ordenado por:</td>
										  <td>
	  								  	<select name="orden">
										  		<option value='-9' <c:if test="${sessionScope.filtroNut.orden== '-9'}">SELECTED</c:if>>Apellidos</option>
										  		<option value='0' <c:if test="${sessionScope.filtroNut.orden== '0'}">SELECTED</c:if>>Número
										  		de identificación</option>
										  		<option value='1' <c:if test="${sessionScope.filtroNut.orden== '1'}">SELECTED</c:if>>Nombres</option>
										  		<option value='2' <c:if test="${sessionScope.filtroNut.orden== '2'}">SELECTED</c:if>>Código
										  		interno</option>
										  	</select>
										  </td>
										  <td></td>
