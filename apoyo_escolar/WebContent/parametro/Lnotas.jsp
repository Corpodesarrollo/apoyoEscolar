...
<%@include file="../parametros.jsp"%>
	...
	<!--
		...
		function hacerValidaciones_frmLnotas(frmLnotas){

		}

		function lanzar(tipo){			
			document.frmLnotas.tipo.value=tipo;
			document.frmLnotas.submit();
		}

		function guardar(tipo){
			if(validarForma(document.frmLnotas)){
				document.frmLnotas.tipo.value=tipo;
				document.frmLnotas.cmd.value="Guardar";
				document.frmLnotas.submit();
			}
		}
	...
	<font size='1'>
		<form method="post" name="frmLnotas" onSubmit="return validarForma(frmLnotas)" action='<c:url value="/parametro/NuevoGuardar.jsp"/>'>
			<input type="hidden" name="tipo" value="1">
			<input type="hidden" name="cmd" value="Cancelar">
			<input type="hidden" name="g_parlibro" value="<c:out value="${sessionScope.parametros.g_parlibro}"/>">

			<table border="0" align="center" width="100%">
				<caption>Libro de Notas</caption>
				<tr>
		  		<td width="45%">
            <input class="boton" name="cmd1" type="button" value="Guardar" onClick="guardar(-7)">
			...
			<table border="0" align="center" width="80%" cellpadding="0" cellspacing="0">
				<tr>
					<td>Habilitado:</td>
					<td>
						<INPUT type='checkbox' name='g_parlibro_' value="1" onClick='valorar(document.frmLnotas);' <c:if test="${sessionScope.parametros.g_parlibro==1}">checked</c:if>>
					</td>
				</tr>
			</table>
		</form>
