...
<%@include file="../parametros.jsp"%>
		<script languaje='javaScript'>
			...
			function hacerValidaciones_frm(forma){
				...
				validarCampo(forma.nombre1, "- Primer Nombre", 1,100);
				validarCampo(forma.apellido1, "- Primer Apellido", 1,100);
			}
   ...
   function ajaxGrupo(){
	  ...
	  document.frmAjaxNuevo.submit();
	   ...
	   <form method="post" name="frmAjaxNuevo" action='<c:url value="/AjaxCommon.do"/>'>
		...
		<FORM  METHOD="POST" name='frm' onSubmit=" return validarForma(frm)" action='<c:url value="/estudiante/FiltroGuardar.jsp?val=1"/>'>
			...
			<caption>Adicionar Estudiante</caption>
				<tr>
				  <td>
			...
			<table border="0" align="center" bordercolor="#FFFFFF" width="90%" cellpadding="2" cellSpacing="0">
				<tr><th colspan='4' class='Fila0'>Datos de la ubicación</th></tr>
				<tr>
				...
				<td><span class="style2">*</span> Grado:</td>
					...
					<td>
						<select name="filgrupo" id="filgrupo" style='width:120px;'>
							<option value='-9'>-- Seleccione uno --</option>
							 <c:forEach begin="0" items="${sessionScope.listaGrupo}" var="fila">
				            <option value='<c:out value="${fila.codigo}"/>' <c:if test="${sessionScope.filtroEst.filgrupo ==fila.codigo}">SELECTED</c:if>><c:out value="${fila.nombre}"/></option>
	                     </c:forEach>
						</select>							
					</td>
				</tr>
				...
				<table border="0" align="center" bordercolor="#FFFFFF" width="90%" cellpadding="2" cellSpacing="0">
				<tr><th colspan='4' class='Fila0'>Datos del alumno</th></tr>
				<tr>
					<td><span class="style2" >*</span>Tipo Documento:</td>
						<td>
			      	<select name="tipoDocumento" style='width:60px;'>
				...
				<tr>
					...
					<td><span class="style2">*</span>Segundo nombre:</td>
					<td>
