...
var locked=<c:out value="${sessionScope.isLockedRotacion}"/>;
	...
	function buscar(){
		document.frmLista.tipo.value='29';
	...
	<font size="1">
		<form action="" method="post" name="frmLista" onSubmit="return validarForma(frmLista)">
			...
			<table>
				<tr>
					<td rowspan="2" width="600">
						...
						<img src="../etc/img/tabs/espacios_1.gif" alt="Fijar Espacios Físicos" height="26" border="0">
		  ...
		  <table border="1" align="center" width="95%" cellpadding="0" cellspacing="0">
			  ...
			  <c:if test="${empty requestScope.espacio}">
					<tr>
						<th class="Fila1" colspan='6'>LA BUSQUEDA NO ARROJO RESULTADOS</th>
					</tr>
				</c:if>
		  	<c:if test="${!empty requestScope.espacio}">
			  	<tr><th  width='35' class="EncabezadoColumna">&nbsp;</th>
					...
					<th class="EncabezadoColumna">Exclusivo</th></tr>
					<c:forEach begin="0" items="${requestScope.espacio}" var="fila" varStatus="st">
						<tr>
							<c:set var="guiab"><c:out value="${fila.fijJerarquia}"/>|<c:out value="${fila.fijEspacio}"/>|<c:out value="${fila.fijAsignatura}"/>|<c:out value="${fila.fijVigencia}"/>|<c:out value="${fila.fijMetodologia}"/></c:set>
							...
							<td class='Fila<c:out value="${st.count%2}"/>' align="center"><c:out value="${fila.fijNomJornada}"/></td>
							...
							<td class='Fila<c:out value="${st.count%2}"/>' align="center"><c:out value="${fila.fijExclusivo}"/></td>
						</tr>
					</c:forEach>
				</c:if>
		  </table>
