...
<!--
	...
	function editar(n){
		if(document.frmLista.id){
				...
				document.frmLista.tipo.value='12';
	...
	function eliminar(n){
		if(document.frmLista.id){
			...
			if(confirm('¿DESEA ELIMINAR ESTE REGISTRO?')){
				document.frmLista.id.value=n;
				document.frmLista.tipo.value='13';
	...
	function buscar(){
		document.frmLista.tipo.value='16';
...
<%@include file="../mensaje.jsp" %>
		<form action="" method="post" name="frmLista" onSubmit="return validarForma(frmLista)">
			...
			<table>
				<tr>
					<td rowspan="2" width="600">
						<img src='<c:url value="/etc/img/tabs/estructura_1.gif"/>' alt='Estructura' height="26" border="0">
		  ...
		  <table border="1" align="center" width="95%" cellpadding="0" cellspacing="0">
			  ...
			  <c:if test="${empty requestScope.estructura}">
					<tr>
		  	...
		  	<c:if test="${!empty requestScope.estructura}">
			  	<tr>
						...
						<th class="EncabezadoColumna">Duraci&oacute;n Bloque</th>
						<th class="EncabezadoColumna">Bloque por Jornada</th>
						<th class="EncabezadoColumna">Semanas por Ciclo</th>
					</tr>
					<c:forEach begin="0" items="${requestScope.estructura}" var="fila" varStatus="st">
						<tr>
							<th class='Fila<c:out value="${st.count%2}"/>'>
							<a href='javaScript:editar(<c:out value="${fila.estCodigo}"/>);'><img border='0' src='../etc/img/<c:if test="${requestScope.guia==fila.estCodigo}">X</c:if>editar.png' width='15' height='15'></a>
							<a href='javaScript:eliminar(<c:out value="${fila.estCodigo}"/>);'><img border='0' src='../etc/img/eliminar.png' width='15' height='15'></a>
							</th>
							<td class='Fila<c:out value="${st.count%2}"/>' align="center"><c:out value="${fila.estNombre}"/></td>
							<td class='Fila<c:out value="${st.count%2}"/>' align="center"><c:out value="${fila.estDurBloque}"/></td>
							<td class='Fila<c:out value="${st.count%2}"/>' align="center"><c:out value="${fila.estBloqueXJornada}"/></td>
							<td class='Fila<c:out value="${st.count%2}"/>' align="center"><c:out value="${fila.estSemanaXCiclo}"/></td>
						</tr>
					</c:forEach>
				</c:if>
		  </table>
