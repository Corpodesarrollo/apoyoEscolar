...
var locked=<c:out value="${sessionScope.isLockedRotacion}"/>;
	...
	function editar(n){
		if(document.frmLista.id){
				...
				document.frmLista.tipo.value='122';
		...
		if(document.frmLista.id){
			if(locked!=0){
				...
				if(confirm('¿DESEA ELIMINAR ESTE REGISTRO?')){
					document.frmLista.id.value=n;
					document.frmLista.tipo.value='123';
	...
	function buscar(){
		document.frmLista.tipo.value='129';
...
<%@include file="../mensaje.jsp" %>
		<form action="" method="post" name="frmLista" onSubmit="return validarForma(frmLista)">
			...
			<table>
				<tr>
					<td rowspan="2" width="600">
						...
						<img src='<c:url value="/etc/img/tabs/inhabilitar_horas_1.gif"/>' alt='Inhabilitar Horas' height="26" border="0">
		  ...
		  <table border="1" align="center" width="95%" cellpadding="0" cellspacing="0">
			  ...
			  <c:if test="${empty requestScope.inhHora}">
					<tr>
		  	...
		  	<c:if test="${!empty requestScope.inhHora}">
			  	<tr>
					...
					<c:forEach begin="0" items="${requestScope.inhHora}" var="fila" varStatus="st">
						<tr>
							<c:set var="guiab"><c:out value="${fila.inhEstructura}"/>|<c:out value="${fila.inhDia}"/>|<c:out value="${fila.inhHoraIni}"/>|<c:out value="${fila.inhHoraFin}"/></c:set>
