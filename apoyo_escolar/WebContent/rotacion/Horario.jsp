...
<%@include file="../parametros.jsp"%><c:set var="tip" value="102" scope="page"/>
...
<script languaje='javaScript'>
		...
		var locked=<c:out value="${sessionScope.isLockedRotacion}"/>;
			...
			function ajaxAsignatura(forma,forma2){
				...
				//alert('cargaDocente'+forma2.vig.value);
				forma2.combo.value="2";
  			forma2.target="frame";
				forma2.submit();
			}

			function cargaDocente(forma,forma2){
				borrar_combo(forma.docente);
				forma2.inst.value='<c:out value="${sessionScope.login.instId}"/>';
				forma2.sede.value=forma.sede.options[forma.sede.selectedIndex].value;
				forma2.jor.value=forma.jornada.options[forma.jornada.selectedIndex].value;
				forma2.met.value=forma.metodologia.value;
				forma2.gra.value=forma.grado.options[forma.grado.selectedIndex].value;
				forma2.asi.value=forma.asignatura.options[forma.asignatura.selectedIndex].value;
				forma2.vig.value=forma.vigencia.options[forma.vigencia.selectedIndex].value;
				//alert('cargaDocente'+forma2.vig.value+forma.value+forma2.value);
				forma2.combo.value="1";
  			forma2.target="frame";
				forma2.submit();
			}
		...
		switch(tipo){
			...
			function guardar(tipo){
				if(validarForma(document.frm)){
					//alert(tipo);
					document.frm.docenteescogido.value=document.frm.docente.options[document.frm.docente.selectedIndex].value;
					document.frm.tipo.value=tipo;
			...
			function filtro(combo_padre,combo_hijo,combo_hijo2,combo_hijo3,combo_hijo4,combo_hijo6){
				...
				<c:forEach begin="0" items="${requestScope.filtroJornadaF}" var="fila2"><c:if test="${fila[0]==fila2[2]}">
				<c:if test="${requestScope.horarioEntraref== '1'}">
				Sel_Hijos[k] = '<c:if test="${sessionScope.horario.jornada== fila2[0]}">SELECTED</c:if>';
				</c:if><c:if test="${requestScope.horarioEntraref!= '1'}">
				Sel_Hijos[k] ='';
				</c:if>
				id_Hijos[k] = '<c:out value="${fila2[0]}"/>'; Hijos[k] = '<c:out value="${fila2[1]}"/>'; id_Padre[k] = '<c:out value="${fila2[2]}"/>'; k++;</c:if></c:forEach>
			...
			function filtro2(combo_padre,combo_padre2,combo_padre3,combo_hijo,combo_hijo2,combo_hijo3){
				...
				<c:forEach begin="0" items="${requestScope.filtroJornadaF}" var="fila2"  varStatus="st"><c:forEach begin="0" items="${requestScope.filtroMetodologiaF}" var="fila4" varStatus="st2">id_Hijos=new Array(); Hijos= new Array(); Sel_Hijos= new Array(); id_Padre= new Array(); var k=0;<c:forEach begin="0" items="${requestScope.filtroGradoF2}" var="fila3"><c:if test="${fila2[2]==fila3[3] && fila2[0]==fila3[4] && fila3[5]==fila4[0]}">
				<c:if test="${requestScope.horarioEntraref== '1'}">
				Sel_Hijos[k] = '<c:if test="${sessionScope.horario.grado== fila3[0]}">SELECTED</c:if>';
				</c:if><c:if test="${requestScope.horarioEntraref!= '1'}">
				Sel_Hijos[k] ='';
				</c:if>
				id_Hijos[k] = '<c:out value="${fila3[0]}"/>'; Hijos[k] = '<c:out value="${fila3[1]}"/>'; id_Padre[k] = '<c:out value="${fila3[3]}"/>|<c:out value="${fila3[4]}"/>|<c:out value="${fila3[5]}"/>'; k++;</c:if></c:forEach>
			...
			function filtro3(combo_padre,combo_padre2,combo_padre4,combo_padre3,combo_hijo){
				...
				<c:forEach begin="0" items="${requestScope.filtroGrupoF}" var="fila3"><c:if test="${fila2[0]==fila3[4] && fila2[3]==fila3[2] && fila2[4]==fila3[3] && fila2[5]==fila3[5]}">
				<c:if test="${requestScope.horarioEntraref== '1'}">
				Sel_Hijos[k] = '<c:if test="${sessionScope.horario.grupo== fila3[0]}">SELECTED</c:if>';
				</c:if><c:if test="${requestScope.horarioEntraref!= '1'}">
				Sel_Hijos[k] ='';
				</c:if>
				id_Hijos[k] = '<c:out value="${fila3[0]}"/>'; Hijos[k] = '<c:out value="${fila3[1]}"/>'; id_Padre[k] = '<c:out value="${fila3[2]}"/>|<c:out value="${fila3[3]}"/>|<c:out value="${fila3[4]}"/>|<c:out value="${fila3[5]}"/>'; k++;</c:if></c:forEach>
				Padres[<c:out value="${st.index}"/>] = new Padre(id_Hijos, Hijos, Sel_Hijos,id_Padre);</c:forEach>
			...
			function cargarHorasDispDocente(forma,forma2){
				//borrar_combo(forma.horasdisp);
				forma2.docente.value=forma.docente.options[forma.docente.selectedIndex].value;
				forma2.vig.value=forma.vigencia.options[forma.vigencia.selectedIndex].value;
				//alert('docente escogido es'+forma2.docente.value);
				forma2.combo.value="4";
  			    forma2.target="frame";
				forma2.submit();
			}
...
<!--//////////////////-->		
	...
	<input type="hidden" name="docenteescogido" value="">
	...
	<table cellpadding="0" cellspacing="0" border="0" ALIGN="CENTER" width="100%" >
		<tr height="1">
			<td width="10" bgcolor="#FFFFFF">&nbsp;</td>							
				<td rowspan="2" width="600" bgcolor="#FFFFFF">
						...
						<img src='<c:url value="/etc/img/tabs/fijar_horario_1.gif"/>' alt='Fijar horario' height="26" border="0">				
		...
		<caption>Formulario de Horario</caption>
		<tr>
		  <td width="45%" bgcolor="#FFFFFF">
			...
			<tr>
				    ...
				    <td colspan='3'>
						<select name="sede" onChange='filtro(document.frm.sede, document.frm.jornada,document.frm.grado,document.frm.grupo,document.frm.metodologia,document.frm.asignatura)' style='width:300px;'>
							<option value='-1'>-- Seleccione uno --</option>
							<c:forEach begin="0" items="${requestScope.filtroSedeF}" var="fila">
								<option value="<c:out value="${fila[0]}"/>" <c:if test="${requestScope.horarioEntraref== '1'}"><c:if test="${sessionScope.horario.sede== fila[0]}">selected</c:if></c:if> ><c:out value="${fila[1]}"/></option></c:forEach>
						</select>											
				   </td>
			...
			<tr>
			  ...
			  <td>
           <select name="metodologia" style='width:150px;' onChange='filtro2(document.frm.sede, document.frm.jornada,document.frm.metodologia,document.frm.grado,document.frm.grupo,document.frm.asignatura);'>
           	<option value='-1'>-- Seleccione uno --</option>
						<c:forEach begin="0" items="${requestScope.filtroMetodologiaF}" var="fila"><option value="<c:out value="${fila[0]}"/>" <c:if test="${requestScope.horarioEntraref== '1'}"><c:if test="${sessionScope.horario.metodologia==fila[0]}">selected</c:if></c:if> ><c:out value="${fila[1]}"/></option></c:forEach>
           </select>
        </td>
						...
						<td>
							<select name="vigencia" style='width:50px;' onChange='setVisible();filtroVig(document.frm.grado,document.frm.grupo,document.frm.asignatura);'>
								...
								<c:forEach begin="0" items="${requestScope.listaVigencia}" var="vig">
									<option value="<c:out value="${vig}"/>" <c:if test="${requestScope.horarioEntraref== '1'}"><c:if test="${vig==sessionScope.horario.vigencia}">selected</c:if></c:if> ><c:out value="${vig}"/></option>
								</c:forEach>
							</select>
						</td>	
				  <td><span class="style2" >*</span>Grado:</td>
				  <td>
            <select name="grado" onChange='filtro3(document.frm.sede, document.frm.jornada,document.frm.metodologia,document.frm.grado,document.frm.grupo);ajaxAsignatura(document.frm,document.frmAux)' style='width:90px;'>
            <option value='-1'>-- Seleccione uno --</option>
            </select>
          </td>
				</tr>
				<tr>				
				  ...
				  <td>
            <select name="grupo" style='width:90px;' onChange='setVisible();'>
            <option value='-1'>-- Seleccione uno --</option>
            </select>
          </td>
			  <td><span class="style2" >*</span>Asignatura:</td>
			  <td colspan="3">
           <select name="asignatura" style='width:150px;' onChange='setVisible();cargaDocente(document.frm,document.frmAux)'>
           	<option value='-1'>-- Seleccione uno --</option>
						<c:forEach begin="0" items="${requestScope.filtroAsignaturaF}" var="fila"><option value="<c:out value="${fila[0]}"/>" ><c:out value="${fila[1]}"/></option></c:forEach>
           </select>
        </td>
			</tr>
			<tr>
				...
				<td colspan=3>
					<select name="docente" style='width:300px;' onChange='setVisible();cargarHorasDispDocente(document.frm,document.frmAux)'>
						<option value='-1'>-- Seleccione uno --</option>
							<c:forEach begin="0" items="${requestScope.filtroDocenteF}" var="fila"><option value="<c:out value="${fila[0]}"/>" ><c:out value="${fila[1]}"/></option></c:forEach>
					</select>
				</td>
			</tr>
			<tr>
				...
				<td colspan=3>
					...
					<input  name="horasdisp" value="<c:if test="${!empty requestScope.horasdisp}"><c:out value="${requestScope.horasdisp}"/></c:if><c:if test="${empty requestScope.horasdisp}">No ha escogido docente</c:if>" disabled="disabled" />
...
<form method="post" name="frmAux" action="<c:url value="/rotacion/FiltroAux.do"/>"><input type="hidden" name="combo" value=""><input type="hidden" name="inst" value=""><input type="hidden" name="sede" value=""><input type="hidden" name="jor" value=""><input type="hidden" name="met" value=""><input type="hidden" name="gra" value=""><input type="hidden" name="asi" value=""><input type="hidden" name="vig" value=""><input type="hidden" name="ext" value="1"><input type="hidden" name="docente" value=""></form>