...
var locked=<c:out value="${sessionScope.isLockedRotacion}"/>;
	...
	function editar(n){
		if(document.frmLista.id){
				...
				document.frmLista.tipo.value='42';
		...
		if(document.frmLista.id){
			if(locked!=0){
				...
				if(confirm('¿DESEA ELIMINAR ESTE REGISTRO?')){
					document.frmLista.id.value=n;
					document.frmLista.tipo.value='43';
	...
	function buscar(){
		document.frmLista.tipo.value='49';
...
<%@include file="../mensaje.jsp" %>
		<form action="" method="post" name="frmLista" onSubmit="return validarForma(frmLista)">
			...
			<INPUT TYPE="hidden" NAME="fsede" VALUE="<c:out value="${requestScope.fsede}"/>">
			<input type="hidden" name="fespacio" value="<c:out value="${requestScope.fespacio}"/>">
		  ...
		  <table border="1" align="center" width="95%" cellpadding="0" cellspacing="0">
			  ...
			  <c:if test="${empty requestScope.espfisjor}">
					<tr>
		  	...
		  	<c:if test="${!empty requestScope.espfisjor}">
			  	<tr>
					...
					<c:forEach begin="0" items="${requestScope.espfisjor}" var="fila" varStatus="st">
						<tr>
							<c:set var="guiab"><c:out value="${fila.inhEstructura}"/>|<c:out value="${fila.inhEspacio}"/>|<c:out value="${fila.inhDia}"/>|<c:out value="${fila.inhHoraIni}"/>|<c:out value="${fila.inhHoraFin}"/></c:set>
							...
							<td class='Fila<c:out value="${st.count%2}"/>' align="center"><c:out value="${fila.inhNomEspacio}"/></td>
