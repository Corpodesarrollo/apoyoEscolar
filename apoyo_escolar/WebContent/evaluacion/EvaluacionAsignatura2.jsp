...
<%System.out.println("INICIO CARGA PAGINA EVAL ASIG APOYO: "+new java.util.Date()); %>
				...
				if(document.frmNuevo.nota){
					if(document.frmNuevo.nota.length){
						for(j = 0;j< document.frmNuevo.notaEst.length;j++){
							if(trim(document.frmNuevo.notaEst[j].value)!=""){
								...
								if(trim(forma.recEst[j].value)!=''){
									validarFloat(document.frmNuevo.recEst[j], "- Recuperación: No es número o está fuera del rango.", notaMin, notaMax);									
								}
	...
	<FORM ACTION='<c:url value="/evaluacion/EvaluacionGuardar.jsp"/>' METHOD="POST" name='frmNuevo' onSubmit="return validarForma(frmNuevo)">
	    ...
	    <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
			...
			<table style="display:" id='t1' name='t1' 	width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
				...
				<td>
				 ...
				 <a href="javascript:void(0);" title="Abrir PopUp" id="abrirPop">Consultar estudiantes que están perdiendo</a>
	    ...
	    <table style="display:" id='t2' name='t2' width="100%" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#CCCCCC">
				...
				<tr><c:if test="${st2.last}"><script>est=<c:out value="${st2.count}"/>;</script></c:if>
					...
					<th class='Fila<c:out value="${st2.count%2}"/>'><input type='text' name='ausjus' id='ausjus' class='Fila<c:out value="${st2.count%2}"/>'  onKeyPress='return acepteNumeros2(event,document.frmNuevo.ausjus,document.frmNuevo.ausjus,<c:out value="${st2.count}"/>,1)' size='3' maxlength='3' value='<c:if test="${fila[7]!=0}"><c:out value="${fila[7]}"/></c:if>'></th>
				</tr>
				</c:forEach>
			</table></c:if>
		</form>
            ...
            <div>
             <a href="javascript:void(0);" title="Cerrar" id="cerrar">[X]</a>
               <c:if test="${sessionScope.tipoEval==1  || sessionScope.tipoEval==3}">
				...
				<c:set var="minimaina" value="${sessionScope.minimoinasistenciaasignatura}" scope="page" />
              ...
              <tr>
              <td>El número maximo de inasistencias permitido es:<c:out value="${sessionScope.minimoinasistenciaasignatura}"/></td>
              </tr>
              </table>
              <table style="display:" id='t2' name='t2' width="100%" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#CCCCCC">
				<tr>					
