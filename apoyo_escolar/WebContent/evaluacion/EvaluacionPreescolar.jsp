...
<script languaje='javaScript'>
	<!--
		...
		function hacerValidaciones_frmFiltro(forma){
			...
			validarLista(forma.filDimension,"- Dimension",1)
		...
		function buscar(tipo){
			if(validarForma(document.frmFiltro)){
				...
				document.frmFiltro.ext2.value='/evaluacion/EvaluacionGuardar2.jsp?tipo=6';  //peticion
				document.frmFiltro.submit();
			}
		}
...
<form method="post" name="frmFiltro" class="formulario-evaluacion-general" onSubmit="return validarForma(frmFiltro)" action='<c:url value="/evaluacion/Save.jsp"/>'>
	<table cellpadding="0" cellspacing="0" border="0" align="center" bordercolor="#FFFFFF" width="95%">
		<caption>Evaluación por Dimensiones</caption>
  	</table>
	...
	<table cellpadding="0" cellspacing="0" border="0" ALIGN="CENTER" width="100%" class="menu_top">
		<tr height="1">
			<script languaje='javaScript'>
				...
				if(fichaPreescolar==1)document.write('<td width="14%" height="26"><a class="active" href="javaScript:lanzar(6)">Evaluación por Dimensiones</a></td>');
	...
	<table width="100%" cellpadding="3" cellSpacing="0" class="tabla-filtros">
		...
		<tr>
			<td><span class="style2">*</span>Dimensión:</td>
			<td>
				<select name="filDimension" style='width:120px;' onChange='setVisible()'>
					<option value='-99'>-- Seleccione uno --</option>
					<c:forEach begin="0" items="${requestScope.lDimension}" var="dim">
						<option value="<c:out value="${dim.codigo}"/>" <c:if test="${sessionScope.filtroComportamiento.filDimension==dim.codigo}">SELECTED</c:if>><c:out value="${dim.nombre}"/></option>
					</c:forEach>
				</select>
			</td>
			<td><span class="style2">*</span>Periodo:</td>
			<td>
				<select name="filPeriodo" style='width:120px;' onChange='setVisible()'>
					<option value='-99'>-- Seleccione uno --</option>
...
<script>
	...
	filtroDimension(document.frmFiltro.filGrado,document.frmFiltro.filDimension);
</script>
