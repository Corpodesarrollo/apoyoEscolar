...
<link type="text/css" href="http://jquery-ui.googlecode.com/svn/tags/1.7/themes/redmond/jquery-ui.css" rel="stylesheet" />
  ...
  <script type="text/javascript" src="jquery-ui.min.js"></script>
	    ...
	    function MostrarDialogasi() {
	    	...
	    	var areasel;
	    	var pescheck;
	    	var txt = document.createElement('br');//document.createElement('<br>');
		...
		appendErrorMessage("- Seleccione por lo menos una Asignatura de detalle")
			...
			function guardar(tipo){
				...
				if(validarForma(document.frm)){
					...
					document.frm.action='<c:url value="/boletines/FiltroGuardarAsignaturas.jsp"/>';
...
<form method="post" name="frm" onSubmit="return validarForma(frm)" action='<c:url value="/boletines/FiltroGuardarAsignaturas.jsp"/>'>
	<table border="0" align="center" width="100%">
	<caption>Generar Resumen de Asignaturas</caption>
		<tr>
...
<!--//////////////////-->	
	...
	<input type="hidden" name="sedenom" value='<c:out value="${sessionScope.filtroResumenAsignaturas.sedenom}"/>'>
	<input type="hidden" name="jornadanom" value='<c:out value="${sessionScope.filtroResumenAsignaturas.jornadanom}"/>'>
	<input type="hidden" name="metodologianom" value='<c:out value="${sessionScope.filtroResumenAsignaturas.metodologianom}"/>'>
	<input type="hidden" name="gradonom" value='<c:out value="${sessionScope.filtroResumenAsignaturas.gradonom}"/>'>
	<input type="hidden" name="gruponom" value='<c:out value="${sessionScope.filtroResumenAsignaturas.gruponom}"/>'>
	<input type="hidden" name="periodonom" value='<c:out value="${sessionScope.filtroResumenAsignaturas.periodonom}"/>'>
	<table border="0" align="center" width="100%">
		<tr height="1">
	...
	<TABLE width="100%" cellpadding="3" cellSpacing="0">
		...
		<tr style='display:none;'>
			...
			<td>
				<input type="radio" name="formato" value="1"  <c:if test="${sessionScope.filtroResumenAreas.formato== '1'}"></c:if>>
			</td>
			<td>Reporte en formato PDF:</td>
			<td>
				<input type="radio" name="formato" value="2" <c:if test="${sessionScope.filtroResumenAreas.formato== '2'}"></c:if>>
			</td>
		</tr>
	</TABLE>
<!--//////////////////////////////-->
