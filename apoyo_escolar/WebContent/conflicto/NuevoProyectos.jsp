<%@ page contentType="text/html; charset=iso-8859-1" language="java" errorPage="" %><%@ page import="siges.util.Recursos" %> 
	...
	<jsp:useBean id="nuevoProyectos" class="siges.conflicto.beans.Proyectos" scope="session"/>
...
<%pageContext.setAttribute("filtroPeriodo",Recursos.recursoEstatico[Recursos.PERIODO]);%>
	<script languaje='javaScript'>			
		  ...
		  function acepteNumeros(eve){
			...
			function ver(forma){
				if(forma.proy_.length){
					for(var i=0;i<forma.proy_.length;i++){
						if(forma.proy_[i].checked==true){
							forma.proy[i].value=forma.proy_[i].value;
						}
					}
				}else{
					if(forma.proy_.checked==true){
						forma.proy.value=forma.proy_.value;
					}
				}
			}
		-->
	...
	<font size="1">
		...
		<table border="1" align="center" bordercolor="#FFFFFF" width="100%">
	        ...
	        <input class='boton' name="cmd1" type="button" value="Buscar" onClick="buscar(40,4)">
        <c:if test="${(sessionScope.nuevoConflicto.ceperiodo!='-1') && (sessionScope.nuevoConflicto.cesede!='-1') && (sessionScope.nuevoConflicto.cejornada!='-1')}">
  	    	<c:if test="${requestScope.sololectura!=1}">
		      	<input class='boton' name="cmd1" type="button" value="Guardar" onClick="guardar(41,4)">
					</c:if>
        </c:if>
				</td> 
			</tr>		
	...
	<!--<input type="hidden" name="ceperiodo" value='0'>-->
		<table cellpadding="0" cellspacing="0" border="0" ALIGN="CENTER" width="100%" >
			<tr height="1">
				...
				<td rowspan="2" width="600" bgcolor="#FFFFFF">
					...
					<img src="../etc/img/tabs/proyectos_1.gif" alt="Proyectos que apoyan la resolución de conflictos" border="0"  height="26">
		...
		<table align="center" border="0">
			<tr>
				...
				<td><select name="cesede"
					onChange='filtro(document.frmNuevo.cesede, document.frmNuevo.cejornada)'
					style='width: 300px;'>
	...
	<c:if test="${(sessionScope.nuevoConflicto.ceperiodo!='-1') && (sessionScope.nuevoConflicto.cesede!='-1') && (sessionScope.nuevoConflicto.cejornada!='-1')}">
		<hr>
		<c:set scope="page" var="cont10" value="0"/>
		<c:set scope="page" var="islleno10" value="0"/>
		
			<c:set scope="page" var="cont10" value="${cont10+1}"/>
			<TABLE id='tbl<c:out value="${cont10}"/>' width="98%" align="center" cellpadding="0" cellSpacing="0" border="1">
			<caption><c:out value="${fila[1]}"/></caption>
				<tr>
					<th width="333px">Proyectos que apoyan la resolución de conflictos</th>
					<c:set scope="page" var="cont20" value="0"/>
					
					<th width="80px" align="center" colspan="4">
						Proyectos que en general apoyan la resolución de conflictos
					</th>
					
					<c:set scope="page" var="cont20" value="${cont20+1}"/>
					<c:set scope="page" var="islleno10" value="1"/>
					
				</tr>
				<c:forEach begin="0" items="${requestScope.filtroCat}" var="fila2">
					<tr>
						<td width="333px" <c:if test="${fila2[4]==2}">title='<c:out value="${fila2[5]}"/>'</c:if>>
							<c:out value="${fila2[1]}"/>
						</td>
						<td width="80px" align="center" <c:if test="${fila3[4]==2}">title='<c:out value="${fila2[5]}"/>'</c:if>>
							<input type="hidden" name="proy" value='-1'>
							<input type="checkbox" name="proy_" value='<c:out value="${fila2[0]}"/>'/>
						</td>
					</tr>
				</c:forEach>
				<tr><td></td></tr>
			</table>
			<c:if test="${cont20!=0}"><hr></c:if>
		
		<c:if test="${islleno10==0}"><table border="1" align="center" width="80%" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF"><tr><th colspan='6'>DEBE SELECCIONAR PRIMERO LOS TIPOS DE CONFLICTO</th></tr></table></c:if>
	</c:if>
<!--//////////////////////////////-->
</form>
<script>
	...
	<c:if test="${(sessionScope.nuevoConflicto.ceperiodo!='-1') && (sessionScope.nuevoConflicto.cesede!='-1') && (sessionScope.nuevoConflicto.cejornada!='-1')}">
		<c:forEach begin="0" items="${sessionScope.nuevoProyectos.proyvalor}" var="fila">
			if(document.frmNuevo.proy_.length){
				for(var i=0;i<document.frmNuevo.proy_.length;i++){
					if(document.frmNuevo.proy_[i].value=='<c:out value="${fila[1]}"/>'){
						document.frmNuevo.proy_[i].checked=true
					}
				}
			}
		</c:forEach>
