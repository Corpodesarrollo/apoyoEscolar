		...
		if(validarForma(document.frmFiltro)){
			if(confirm("¿Confirma la devolución del POA? \n Nota: Una vez devuelto no se podrá ver o editar actividades")){
				...
				while(trim(motivo).length==0){
					motivo=prompt('Indique el motivo de devolución:');
				}	
				document.frmFiltro.frmMotivo.value=motivo;
...
<body onLoad="mensaje(document.getElementById('msg'));">
	<form method="post" name="frmAjax0" action='<c:url value="/poa/aprobacion/Ajax.do"/>'><INPUT TYPE="hidden" NAME="tipo" VALUE='<c:out value="${paramsVO.FICHA_ACTIVIDAD_SIN}"/>'><input type="hidden" name="cmd" value='<c:out value="${paramsVO.CMD_AJAX}"/>'><input type="hidden" name="ajax"><input type="hidden" name="ajax"></form>
	<form action='<c:url value="/poa/aprobacion/SaveSin.jsp"/>' method="post" name="frmFiltro" onSubmit="return validarForma(frmFiltro)">
