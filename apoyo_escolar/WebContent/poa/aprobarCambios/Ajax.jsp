		...
		<c:choose>	  
			...
			<c:when test="${requestScope.ajaxParam==paramsVO.CMD_AJAX_TIPOGASTO}">
	  		...
	  		<c:when test="${requestScope.ajaxParam==paramsVO.CMD_AJAX_COLEGIO}">
	  			borrar_combo(parent.document.frmFiltro.filEntidad);
		  		<c:forEach begin="0" items="${requestScope.listaColegio}" var="linea" varStatus="st">
					parent.document.frmFiltro.filEntidad.options[<c:out value="${st.count}"/>] = new Option('<c:out value="${linea.nombre}"/>','<c:out value="${linea.codigo}"/>');
				</c:forEach>
			</c:when>
		</c:choose>
	//-->
