...
<!--
	...
	<c:choose>
	  <c:when test="${requestScope.ajaxParam==paramsVO.CMD_AJAX_AREA}">
	  		...
	  		borrar_combo(parent.document.frmNuevo.plaPonderadores);
		  	<c:forEach begin="0" items="${requestScope.listaAreaGestion}" var="linea" varStatus="st">
					...
					parent.document.frmNuevo.plaPonderadores.options[<c:out value="${st.count}"/>] = new Option('<c:out value="${linea.padre}"/>','<c:out value="${linea.padre}"/>');
				</c:forEach>
		</c:when>
