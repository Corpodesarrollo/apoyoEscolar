...
<body onLoad="mensaje(document.getElementById('msg'));">
	<form action='<c:url value="/poa/parametro/SaveProgramacionFechas.jsp"/>' method="post" name="frmFiltro">
		...
		<table cellpadding="0" cellspacing="0" border="0" ALIGN="CENTER" width="100%" >
			<tr height="1">
				...
				<td rowspan="2" width="469">
					...
					<img src='<c:url value="/etc/img/tabs/programacion_fechas_1.gif"/>' alt="Programación de fechas" height="26" border="0">
		...
		<table border="1" align="center" width="95%" cellpadding="0" cellspacing="0" bordercolor="gray">
		 	...
		 	<c:if test="${empty requestScope.listaProgramacionFechas}">
				<tr>
			...
			<c:if test="${!empty requestScope.listaProgramacionFechas}">
				<tr>
					...
					<td class="EncabezadoColumna" align="center">Vigencia</td>
					<td class="EncabezadoColumna" align="center">Fecha Planeación</td>
					<td class="EncabezadoColumna" align="center">Fecha Seguimiento primer trimestre</td>
					<td class="EncabezadoColumna" align="center">Fecha Seguimiento segundo trimestre</td>
					<td class="EncabezadoColumna" align="center">Fecha Seguimiento tercer trimestre</td>
					<td class="EncabezadoColumna" align="center">Fecha Seguimiento cuarto trimestre</td>
				</tr>
				<c:forEach begin="0" items="${requestScope.listaProgramacionFechas}" var="lista" varStatus="st">
					<tr>
						<th class='Fila<c:out value="${st.count%2}"/>'>
							<a href='javaScript:editar(<c:out value="${lista.progVigencia}"/>);'><img border='0' src='<c:url value="/etc/img/editar.png"/>' width='15' height='15'></a>
							<c:if test="${sessionScope.NivelPermiso==2}"><a href='javaScript:eliminar(<c:out value="${lista.progVigencia}"/>);'><img border='0' src='<c:url value="/etc/img/eliminar.png"/>' width='15' height='15'></a></c:if>
						</th>
						<td class='Fila<c:out value="${st.count%2}"/>' align="center"><c:out value="${lista.progVigencia}"/></td>
						<td class='Fila<c:out value="${st.count%2}"/>' align="center"><c:out value="${lista.progPlaneacionIni}"/> - <c:out value="${lista.progPlaneacionFin}"/></td>
						<td class='Fila<c:out value="${st.count%2}"/>' align="center"><c:out value="${lista.progSeguimiento1Ini}"/> - <c:out value="${lista.progSeguimiento1Fin}"/></td>
						<td class='Fila<c:out value="${st.count%2}"/>' align="center"><c:out value="${lista.progSeguimiento2Ini}"/> - <c:out value="${lista.progSeguimiento2Fin}"/></td>
						<td class='Fila<c:out value="${st.count%2}"/>' align="center"><c:out value="${lista.progSeguimiento3Ini}"/> - <c:out value="${lista.progSeguimiento3Fin}"/></td>
						<td class='Fila<c:out value="${st.count%2}"/>' align="center"><c:out value="${lista.progSeguimiento4Ini}"/> - <c:out value="${lista.progSeguimiento4Fin}"/></td>
					</tr>
				</c:forEach>
			</c:if>
		</table>
