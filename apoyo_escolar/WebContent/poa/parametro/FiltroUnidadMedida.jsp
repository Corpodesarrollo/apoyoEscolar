...
<body onLoad="mensaje(document.getElementById('msg'));">
	<form action='<c:url value="/poa/parametro/SaveUnidadMedida.jsp"/>' method="post" name="frmFiltro">
		...
		<table cellpadding="0" cellspacing="0" border="0" ALIGN="CENTER" width="100%" >
			<tr height="1">
				...
				<td rowspan="2" width="469">
					...
					<img src='<c:url value="/etc/img/tabs/unidad_medida_1.gif"/>' alt="Unidad de medida" height="26" border="0">
		...
		<table border="1" align="center" width="95%" cellpadding="0" cellspacing="0" bordercolor="gray">
		 	...
		 	<c:if test="${empty requestScope.listaUnidadMedida}">
				<tr>
			...
			<c:if test="${!empty requestScope.listaUnidadMedida}">
				...
				<c:forEach begin="0" items="${requestScope.listaUnidadMedida}" var="lista" varStatus="st">
					<tr>
						<th class='Fila<c:out value="${st.count%2}"/>'>
							<a href='javaScript:editar(<c:out value="${lista.uniCodigo}"/>);'><img border='0' src='<c:url value="/etc/img/editar.png"/>' width='15' height='15'></a>
							<c:if test="${sessionScope.NivelPermiso==2}"><a href='javaScript:eliminar(<c:out value="${lista.uniCodigo}"/>);'><img border='0' src='<c:url value="/etc/img/eliminar.png"/>' width='15' height='15'></a></c:if>
						</th>
						<td class='Fila<c:out value="${st.count%2}"/>' align="center"><c:out value="${lista.uniCodigo}"/></td>
						<td class='Fila<c:out value="${st.count%2}"/>' align="center"><c:out value="${lista.uniNombre}"/></td>
					</tr>
				</c:forEach>
			</c:if>
		</table>
