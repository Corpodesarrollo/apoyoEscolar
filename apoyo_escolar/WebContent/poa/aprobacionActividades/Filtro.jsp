...
<html>
		...
		<!--
			...
			function aprobar(n) {
				...
				if(validarForma(document.frmFiltro)) {
					...
					if(confirm("¿Confirma la aprobación del POA? \n Nota: Una vez aprobado no se podrá devolver")) {
			...
			function validarObservacion(forma) {
				...
				if(forma.filMotivo) {
					...
					if(trim(forma.filMotivo.value).length==0 || trim(forma.filMotivo.value).length>500) {
						...
						alert('- Observación por devolución (hasta 500 caracteres)');
						return false;
					}
			...
			function rechazar(n) {
				...
				if(validarForma(document.frmFiltro)) {
					...
					if(confirm("¿Confirma la devolución del POA? \n Nota: Una vez devuelto no se podrá ver o editar actividades")) {
			...
			function inhabilitar() {
				...
				if(document.frmFiltro.cmdPOA) {
				
					if(document.frmFiltro.cmdPOA.length) {
					
						for(var i = 0; i < document.frmFiltro.cmdPOA.length; i++) {
						
							document.frmFiltro.cmdPOA[i].style.display = 'none';
							
						}
						
					} else {
					
						document.frmFiltro.cmdPOA.style.display = 'none';
						
					}
				}
				
				if(document.frmFiltro.cmdPOA2) {
				
					if(document.frmFiltro.cmdPOA2.length) {
					
						for(var i = 0; i < document.frmFiltro.cmdPOA2.length; i++) {
							document.frmFiltro.cmdPOA2[i].style.display = 'none';
						}
						
					} else {
					
						document.frmFiltro.cmdPOA2.style.display = 'none';
			...
			<c:forEach begin="0" items="${requestScope.listaColegio}" var="lista" varStatus="st">
				obs[j++] = '<c:out value="${lista.colObservacion}"/>';
			</c:forEach>
			
			function verObservacion(n) {
				alert("Observación por devolución: \n \t" + obs[n]);
			}
		//-->
		</script>
	...
	<body onLoad="mensaje(document.getElementById('msg'));">
		...
		<form action='<c:url value="/poa/consulta/NuevoImprimir.do"/>' method="post" name="frmReporte">
		...
		<form action='<c:url value="/poa/aprobacionActividades/Save.jsp"/>' method="post" name="frmFiltro" onSubmit="return validarForma(frmFiltro)">
			...
			<table border="0" align="center" width="95%">
				<tr>
					...
					<td>
						<select name="filVigencia" style="width:220px">
					...
					<td>
						<select name="filEstatoPOA" style="width:220px">
				...
				<tr>
					...
					<td>
						<select name="filLocalidad" style="width:220px" onchange="javaScript:inhabilitar();" <c:if test="${sessionScope.filtroAprobacionActividadesVO.filTieneLocalidad}">disabled="disabled"</c:if>>
							<option value="-99" selected>-Seleccione uno-</option>
				...
				<tr>
					<td>Observación por devolución:</td>
					<td colspan="3"><textarea name="filMotivo" cols="120" rows="3" onKeyDown="textCounter(this,500,500);" onKeyUp="textCounter(this,500,500);" ></textarea></td>
				</tr>
			</table>
			<br>
			...
			<table border="1" align="center" width="95%" cellpadding="0" cellspacing="0" bordercolor="gray">
				...
				<c:if test="${!empty requestScope.listaColegio}">
					<tr>
						...
						<td class="EncabezadoColumna" align="center">Devolver</td>
					...
					<c:forEach begin="0" items="${requestScope.listaColegio}" var="lista" varStatus="st">
						<tr>
							...
							<td class='Fila<c:out value="${st.count%2}"/>' align="center" valign="middle">&nbsp;<c:if test="${lista.colEstado==0}"><input name="cmdPOA2" id='cmdPOA2' type="button" value="Devolver" style="width:70px;" onClick='rechazar(<c:out value="${lista.colColegio}"/>)' class="boton"></c:if>&nbsp;</td>
