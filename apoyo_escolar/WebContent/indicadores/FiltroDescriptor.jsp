...
<!--
	...
	function hacerValidaciones_frmFiltro(forma){
		...
		validarLista(forma.filTipo, "- Tipo de descriptor");
	...
	function ajaxArea0(){
		...
		if(parseInt(document.frmAjax0.ajax[3].value)!=-99){
			document.frmAjax0.cmd.value=document.frmFiltro.AJAX_AREA.value;
	 		document.frmAjax0.target="frameAjax0";
			document.frmAjax0.submit();
		}	
	}
	...
	function ajaxDocente0(){
		...
		document.frmAjax0.ajax[2].value=document.frmFiltro.filVigencia.value;
		document.frmAjax0.ajax[3].value=document.frmFiltro.filGrado.value;
		document.frmAjax0.ajax[4].value=document.frmFiltro.filArea.value;
...
<body onLoad="mensaje(document.getElementById('msg'));">
	...
	<form action='<c:url value="/indicadores/SaveDescriptor.jsp"/>' method="post" name="frmFiltro" onSubmit="return validarForma(frmFiltro)">
		<input TYPE="hidden" NAME="tipo" VALUE='<c:out value="${paramsVO.FICHA_DESCRIPTOR}"/>'>
		<input type="hidden" name="cmd" value=''>
		<c:forEach begin="0" end="5" var="i"><input type="hidden" name="id"></c:forEach>
		...
		<input type="hidden" name="filInstitucion" value='<c:out value="${sessionScope.filtroDescriptorVO.filInstitucion}"/>'>
		<input type="hidden" name="filVigencia" value='<c:out value="${sessionScope.filtroDescriptorVO.filVigencia}"/>'>
		<table border="0" ALIGN="CENTER" width="95%">
			<tr align="left" colspan="2">
			<c:forEach begin="0" items="${sessionScope.logroanddesc}" var="logdes">
				<td width="15%" height="26" class="norepeat2" onclick='javaScript:lanzar(<c:out value="${paramsVO.FICHA_LOGRO}"/>)'>
				...
				<td>
					<select name="filMetodologia" style="width:120px" onchange="javaScript:ajaxGrado0()">
						...
						<c:forEach begin="0" items="${requestScope.listaMetodologia}" var="metod">
							<option value="<c:out value="${metod.codigo}"/>" <c:if test="${metod.codigo==sessionScope.filtroDescriptorVO.filMetodologia}">selected</c:if>><c:out value="${metod.nombre}"/></option>
						</c:forEach>
					</select>
				</td>
				<td><span class="style2">*</span> Grado:</td>
				<td>
					<select name="filGrado" style="width:120px" onchange="javaScript:ajaxArea0()">
						<option value="-99" selected>--Seleccione uno--</option>
						<c:forEach begin="0" items="${requestScope.listaGrado}" var="grado">
							<option value="<c:out value="${grado.codigo}"/>" <c:if test="${grado.codigo==sessionScope.filtroDescriptorVO.filGrado}">selected</c:if>><c:out value="${grado.nombre}"/></option>
						</c:forEach>
					</select>
				</td>
			</tr>
			<tr>
				<td><span class="style2">*</span> Area:</td>
				<td>
					<select name="filArea" style="width:200px" onchange="javaScript:ajaxDocente0()">
						<option value="-99" selected>--Seleccione uno--</option>
						<c:forEach begin="0" items="${requestScope.listaArea}" var="area">
							<option value="<c:out value="${area.codigo}"/>" <c:if test="${area.codigo==sessionScope.filtroDescriptorVO.filArea}">selected</c:if>><c:out value="${area.nombre}"/></option>
						</c:forEach>
					</select>
				</td>
				<td><span class="style2">*</span> Tipo de descriptor:</td>
				<td>
					<select name="filTipo" style="width:100px">
						<option value="-99" selected>--Seleccione uno--</option>
						<c:forEach begin="0" items="${requestScope.listaTipo}" var="area">
							<option value="<c:out value="${area.codigo}"/>" <c:if test="${area.codigo==sessionScope.filtroDescriptorVO.filTipo}">selected</c:if>><c:out value="${area.nombre}"/></option>
						</c:forEach>
					</select>
				</td>
			</tr>
			<c:if test="${sessionScope.filtroDescriptorVO.filPlanEstudios==1}">
			<tr>
				...
				<td>
					<select name="filDocente" style="width:200px">
						...
						<c:forEach begin="0" items="${requestScope.listaDocente}" var="doc">
							<option value="<c:out value="${doc.codigo}"/>" <c:if test="${doc.codigo==sessionScope.filtroDescriptorVO.filDocente}">selected</c:if>><c:out value="${doc.nombre}"/></option>
						</c:forEach>
					</select>
				</td>
			</tr>
		...
		<table border="1" align="center" width="95%" cellpadding="0" cellspacing="0" bordercolor="gray">
		 	<caption>Listado de descriptores</caption>
		 	<c:if test="${empty requestScope.listaDescriptor}"><tr><th class="Fila1" colspan='6'>No hay descriptores</th></tr></c:if>
			<c:if test="${!empty requestScope.listaDescriptor}">
				<tr>
				...
				<c:forEach begin="0" items="${requestScope.listaDescriptor}" var="lista" varStatus="st">
					<tr>
						<td class='Fila<c:out value="${st.count%2}"/>' align="center"><c:out value="${lista.desConsecutivo}"/></td>
						<th class='Fila<c:out value="${st.count%2}"/>'>
							<a href='javaScript:editar(<c:out value="${lista.desCodigoJerarquia}"/>,<c:out value="${lista.desCodigo}"/>,<c:out value="${lista.desVigencia}"/>);'><img border='0' src='<c:url value="/etc/img/editar.png"/>' width='15' height='15'></a>
							<c:if test="${sessionScope.NivelPermiso==2 && sessionScope.filtroDescriptorVO.filInHabilitado==0}"><a href='javaScript:eliminar(<c:out value="${lista.desCodigoJerarquia}"/>,<c:out value="${lista.desCodigo}"/>,<c:out value="${lista.desVigencia}"/>);'><img border='0' src='<c:url value="/etc/img/eliminar.png"/>' width='15' height='15'></a></c:if>
						</th>
						<td class='Fila<c:out value="${st.count%2}"/>' align="center"><c:out value="${lista.desNombre}"/></td>
						<td class='Fila<c:out value="${st.count%2}"/>' align="center"><c:out value="${lista.desAbreviatura}"/></td>
						<td class='Fila<c:out value="${st.count%2}"/>' align="center"><c:out value="${lista.desOrden}"/></td>
					</tr>
				</c:forEach>
			</c:if>
		</table>
