...
<!--
	...
	function agregar(){
		...
		remote.opener.name = "frameProgramaSENA";
		remote.focus();
	}
...
<body onLoad="mensaje(document.getElementById('msg'));">
	<form action='<c:url value="/pei/registro/SaveProgramaSENA.jsp"/>' method="post" name="frmFiltro" onSubmit="return validarForma(frmFiltro)">
		...
		<input type="hidden" name="proInstitucion" value='<c:out value="${sessionScope.programaSenaPEIVO.proInstitucion}"/>'>
		<input type="hidden" name="proCodigo" value='<c:out value="${sessionScope.programaSenaPEIVO.proCodigo}"/>'>
		...
		<table border="1" align="center" width="95%" cellpadding="0" cellspacing="0" bordercolor="gray">
				...
				<c:forEach begin="0" items="${requestScope.listaProgramaSena}" var="lista" varStatus="st">
					<tr>
						<th class='Fila<c:out value="${st.count%2}"/>' title='<c:out value="${lista.proDescripcion}"/>'>
							<c:if test="${sessionScope.NivelPermiso==2 && sessionScope.desarrolloCurricularPEIVO.desDisabled==false}"><a href='javaScript:eliminar(<c:out value="${lista.proInstitucion}"/>,<c:out value="${lista.proCodigo}"/>);'><img border='0' src='<c:url value="/etc/img/eliminar.png"/>' width='15' height='15'></a></c:if>
						</th>
						<td class='Fila<c:out value="${st.count%2}"/>' align="center" title='<c:out value="${lista.proDescripcion}"/>'><c:out value="${lista.proNombre}"/></td>
						<td class='Fila<c:out value="${st.count%2}"/>' align="center" title='<c:out value="${lista.proDescripcion}"/>'><c:out value="${lista.proEstudiantes}"/></td>
					</tr>
				</c:forEach>
		</table>
	</form>
