...
<html>
<head>
<SCRIPT language=javascript src='<c:url value="/pei/registro/NuevoProgramaUniversidad.js"/>'></SCRIPT>
<script language="javaScript">var nav4=window.Event ? true : false;</script>
</head>
<c:import url="/mensaje.jsp"/>
<body onLoad="mensaje(document.getElementById('msg'));">
	<form method="post" name="frmNuevo" onSubmit="return validarForma(frmNuevo)" action='<c:url value="/pei/registro/SaveProgramaUniversidad.jsp"/>'>
		...
		<table border="0" align="center" bordercolor="#FFFFFF" width="95%">
				<caption>AGREGAR PROGRAMA DE EDUCACIÓN SUPERIOR</caption>
				<tr>
		  ...
		  <table border="0" align="center" width="95%" cellpadding="1" cellspacing="0">
			<tr>
				...
				<td>
					<select name="proUniversidad">
						<option value='-99'>--Seleccione uno--</option>
						<c:forEach begin="0" items="${requestScope.listaUniversidad}" var="item">
							<option value='<c:out value="${item.codigo}"/>' <c:if test="${item.codigo==sessionScope.programaUniversidadPEIVO.proUniversidad}">selected</c:if> title='<c:out value="${item.descripcion}"/>'><c:out value="${item.nombre}"/></option>
						</c:forEach>
					</select>
				</td>
			</tr>
				...
				<td>
					<select name="proCodigo">
						...
						<c:forEach begin="0" items="${requestScope.listaPrograma}" var="item">
							<option value='<c:out value="${item.codigo}"/>' <c:if test="${item.codigo==sessionScope.programaUniversidadPEIVO.proCodigo}">selected</c:if> title='<c:out value="${item.descripcion}"/>'><c:out value="${item.nombre}"/></option>
						</c:forEach>
					</select>
				</td>
			</tr>
			<tr>
				...
				<td>
					<input type="text" name="proEstudiantes" maxlength="10" size="3" value='<c:out value="${sessionScope.programaUniversidadPEIVO.proEstudiantes}"/>' onKeyPress='return acepteNumeros(event)'></input>
				</td>
			</tr>	
		</table>	
	</form>
