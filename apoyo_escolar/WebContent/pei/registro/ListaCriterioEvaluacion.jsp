...
<!--
	function eliminar(inst,cod){
		if(confirm('¿Confirma la eliminación del Criterio de Evaluacion?')){
			document.frmFiltro.criCodigoInstitucion.value=inst;
			document.frmFiltro.criConsecutivo.value=cod;
	...
	function agregar(){
		...
		remote.opener.name = "frameCriterioEvaluacion";
		remote.focus();
	}
...
<body onLoad="mensaje(document.getElementById('msg'));">
	<form action='<c:url value="/pei/registro/SaveCriterioEvaluacion.jsp"/>' method="post" id="frmFiltro" name="frmFiltro" onSubmit="return validarForma(frmFiltro)">
		...
		<table border="1" align="center" width="95%" cellpadding="0" cellspacing="0" bordercolor="gray">
				...
				<c:forEach begin="0" items="${requestScope.listaCriterioEvaluacion}" var="lista" varStatus="st">
					<tr>
						<th class='Fila<c:out value="${st.count%2}"/>' title='Eliminar'>
							<c:if test="${sessionScope.NivelPermiso==2 && sessionScope.desarrolloCurricularPEIVO.desDisabled==false}">
								<a href='javaScript:eliminar(<c:out value="${lista.criCodigoInstitucion}"/>,<c:out value="${lista.criConsecutivo}"/>);'>
									<img border='0' src='<c:url value="/etc/img/eliminar.png"/>' width='15' height='15'>
								</a>
							</c:if>
						</th>
						<td class='Fila<c:out value="${st.count%2}"/>' align="center">
							<textarea rows="2" cols="40" disabled><c:out value="${lista.criAlcance}"/></textarea>
						</td>
						<td class='Fila<c:out value="${st.count%2}"/>' align="center">
							<textarea rows="2" cols="40" disabled><c:out value="${lista.criDificultad}"/></textarea>
						</td>
					</tr>
				</c:forEach>
		</table>
