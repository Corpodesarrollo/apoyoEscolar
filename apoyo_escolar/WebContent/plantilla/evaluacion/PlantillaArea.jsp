...
<c:set var="tip" value="2" scope="page"/>
		...
		<!--
			...
			function hacerValidaciones_frmNuevo(forma){
				...
				if(forma.area.selectedIndex>0 && forma.docente){
					validarLista(forma.docente,"- Docente (obligatorio si se selecciona area)",1)
				}
			} 
			...
			function guardar(tipo){
				if(validarForma(document.frmNuevo)){
					...
					document.frmNuevo.area_.value=document.frmNuevo.area.options[document.frmNuevo.area.selectedIndex].text;
			...
			function filtro(combo_padre0,combo_padre,combo_hijo){
				...
				<c:if test="${!empty requestScope.filtroGrado2 && !empty requestScope.filtroGradoArea}">var Padres = new Array();
					...
					<c:forEach begin="0" items="${requestScope.filtroGradoArea}" var="fila2">
					<c:if test="${fila[0]==fila2[2] && fila[2]==fila2[3]}">Sel_Hijos[k] = '<c:if test="${sessionScope.filtroPlantilla.area== fila2[0]}">SELECTED</c:if>'; id_Hijos[k] = '<c:out value="${fila2[0]}"/>'; Hijos[k] = '<c:out value="${fila2[1]}"/>'; id_Padre[k] = '<c:out value="${fila2[4]}"/>'; k++;</c:if>
	...
	function ajaxDocente(){
		if(document.frmNuevo.docente){
			...
			document.frmAjax.ajax[4].value=document.frmNuevo.area.value;
...
<c:import url="/mensaje.jsp"/>
	<form method="post" name="frmAjax" action='<c:url value="/plantilla/evaluacion/ControllerPlantillaAjax.do"/>'>
		<INPUT TYPE="hidden" NAME="tipo" VALUE='<c:out value="${propertiesVO.PLANTILLAAREADESC}"/>'>
	...
	<caption>Generar plantillas de Evaluación de Área </caption>
		<tr>		
...
<!--//////////////////-->		
	...
	<input type="hidden" name="area_" value='<c:out value="${sessionScope.filtroPlantilla.area_}"/>'>
	...
	<table cellpadding="0" cellspacing="0" border="0" ALIGN="CENTER" width="100%" >
		<tr height="1">
			...
			<td rowspan="2" width="588" bgcolor="#FFFFFF"><a href="javaScript:lanzar(1)"><img src='<c:url value="/etc/img/tabs/Evaluacion_Asignaturas_0.gif"/>' alt="Asignatura" width="84"  height="26" border="0"></a><img src='<c:url value="/etc/img/tabs/Evaluacion_Areas_1.gif"/>' alt='-Area-' width="84"  height="26" border="0"><a href="javaScript:lanzar(3)"><img src='<c:url value="/etc/img/tabs/Evaluacion_Preescolar_0.gif"/>' alt="Preescolar" width="84"  height="26" border="0"></a><a href="javaScript:lanzar(6)"><img src='<c:url value="/etc/img/tabs/Recuperacion_0.gif"/>' alt="Recuperacion" width="84"  height="26" border="0"></a><a href="javaScript:lanzar(10)"><img src='<c:url value="/etc/img/tabs/Evaluacion_Comportamiento_0.gif"/>' alt="Comportamiento" width="84"  height="26" border="0"></a></td>		</tr>
	</table>
		<!-- FIN DE FICHAS A MOSTRAR AL USUARIO -->
								<TABLE width="100%" cellpadding="3" cellSpacing="0">
										<tr>
											...
											<td>
												<select name="metodologia" style='width:120px;' onChange='filtro3(document.frmNuevo.metodologia,document.frmNuevo.grado, document.frmNuevo.grupo, document.frmNuevo.area);'>
											...
											<td><select name="grado" style='width:120px;' onChange='filtro2(document.frmNuevo.metodologia,document.frmNuevo.grado, document.frmNuevo.grupo);filtro(document.frmNuevo.metodologia,document.frmNuevo.grado, document.frmNuevo.area)'><option value='-3'>-- Todos --</option></select></td>
											<th>Grupo:</th>
											<td><select name="grupo" style='width:100px;'><option value='-3'>-- Todos --</option></select></td>
										</tr>
										<tr>
											<th>Area:</th>
											<td><select name="area" style='width:120px;' onchange="javaScript:ajaxDocente()"><option value='-3'>-- Todos --</option></select></td>	
...
<!--//////////////////////////////-->
<script>filtro3(document.frmNuevo.metodologia,document.frmNuevo.grado, document.frmNuevo.grupo, document.frmNuevo.area);
