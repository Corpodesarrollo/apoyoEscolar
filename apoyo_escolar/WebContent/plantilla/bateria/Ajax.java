...
public class Ajax extends Service{
	...
	public void logroDocente(HttpServletRequest request, HttpSession session, Login usuVO,int cmd) throws ServletException {
		try {
			String params[]=request.getParameterValues("ajax");
			if(params==null || params.length<5 || params[0]==null || params[1]==null || params[2]==null || params[3]==null || params[4]==null) return;
			String []val=params[4].replace('|',':').split(":");
			if(val!=null && val.length==3){	
				long asig=Long.parseLong(val[2]);
				request.setAttribute("listaDocente", plantillaDAO.getListaDocenteAsignatura(Long.parseLong(params[0]),Integer.parseInt(params[1]),Integer.parseInt(params[2]),Integer.parseInt(params[3]),asig));
	...
	public void descriptorDocente(HttpServletRequest request, HttpSession session, Login usuVO,int cmd) throws ServletException {
		try {
			String params[]=request.getParameterValues("ajax");
			if(params==null || params.length<5 || params[0]==null || params[1]==null || params[2]==null || params[3]==null || params[4]==null) return;
			String []val=params[4].replace('|',':').split(":");
			...
			if(val!=null && val.length==2){	
				...
				request.setAttribute("listaDocente", plantillaDAO.getListaDocenteArea(Long.parseLong(params[0]),Integer.parseInt(params[1]),Integer.parseInt(params[2]),Integer.parseInt(params[3]),asig));
				request.setAttribute("ajaxParam",String.valueOf(cmd));
			}	
		} catch (Exception e) {
			e.printStackTrace();
