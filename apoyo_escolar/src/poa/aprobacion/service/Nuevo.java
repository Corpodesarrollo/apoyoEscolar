...
public class Nuevo extends Service{
	private static final long serialVersionUID = 6210013753285143307L;
	/**
	...
	public String[] process(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		PlaneacionVO planeacion=(PlaneacionVO)session.getAttribute("aprobacionVO");
		FiltroPlaneacionVO filtro=(FiltroPlaneacionVO)session.getAttribute("filtroAprobacionVO");
		PlaneacionVO planeacionSin=(PlaneacionVO)session.getAttribute("aprobacionSinVO");
		switch (TIPO){
		case ParamsVO.FICHA_ACTIVIDAD:
			...
			switch (CMD){
				...
				case ParamsVO.CMD_EDITAR:
					...
					planeacionBuscar(request, session, usuVO,filtro);
					planeacion=(PlaneacionVO)session.getAttribute("aprobacionVO");
				break;
		...
		case ParamsVO.FICHA_ACTIVIDAD_SIN:
			...
			switch (CMD){
				case ParamsVO.CMD_NUEVO:
					session.removeAttribute("aprobacionSinVO");
					planeacionSinBuscar(request, session, usuVO,filtro);
				break;
				case ParamsVO.CMD_EDITAR:
					planeacionSinEditar(request, session, usuVO,filtro);
					planeacionSinBuscar(request, session, usuVO,filtro);
					planeacionSin=(PlaneacionVO)session.getAttribute("aprobacionSinVO");
				break;
				case ParamsVO.CMD_BUSCAR:
					planeacionSinBuscar(request, session, usuVO,filtro);
				break;
	...
	public void planeacionEditar(HttpServletRequest request, HttpSession session, Login usuVO,FiltroPlaneacionVO filtro) throws ServletException {
		try {
			...
			if(id!=null && id2!=null && id3!=null){
				session.setAttribute("aprobacionVO",planeacionDAO.getActividad(Integer.parseInt(id),Long.parseLong(id2),Long.parseLong(id3),filtro.getFilEstatoPOA()));
	...
	public void planeacionSinEditar(HttpServletRequest request, HttpSession session, Login usuVO,FiltroPlaneacionVO filtro) throws ServletException {
		try {
			...
			String id3=request.getParameter("id3");
			if(id!=null && id2!=null && id3!=null){
				session.setAttribute("aprobacionSinVO",planeacionDAO.getActividadSin(Integer.parseInt(id),Long.parseLong(id2),Long.parseLong(id3),filtro.getFilEstatoPOA()));
