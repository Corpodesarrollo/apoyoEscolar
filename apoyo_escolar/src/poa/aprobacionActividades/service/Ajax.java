...
public class Ajax extends Service{
		...
		switch (TIPO){
			...
			case ParamsVO.FICHA_ACTIVIDAD_SIN:
				...
				switch (CMD){
					...
					break;	
					case ParamsVO.CMD_AJAX_COLEGIO:
						planeacionSinColegio(request, session, usuVO);
					break;	
				}
			break;
		}
	...
	public void planeacionTipoGasto(HttpServletRequest request, HttpSession session, Login usuVO) throws ServletException {
		try {
			...
			poa.aprobacionActividades.vo.PlaneacionVO planeacionVO = (poa.aprobacionActividades.vo.PlaneacionVO) session.getAttribute("cambioActividadesVO");
	...
	public void planeacionColegio(HttpServletRequest request, HttpSession session, Login usuVO) throws ServletException {
		try {
			String params[]=request.getParameterValues("ajax");
			if(params==null || params.length<1 || params[0]==null) return;
			//request.setAttribute("listaColegio", planeacionDAO.getListaColegio(Integer.parseInt(params[0])));
	 ...
	 * @throws ServletException
	 */
	public void planeacionSinColegio(HttpServletRequest request, HttpSession session, Login usuVO) throws ServletException {
		try {
			String params[]=request.getParameterValues("ajax");
			if(params==null || params.length<1 || params[0]==null) return;
			//request.setAttribute("listaColegio", planeacionDAO.getListaColegio(Integer.parseInt(params[0])));
