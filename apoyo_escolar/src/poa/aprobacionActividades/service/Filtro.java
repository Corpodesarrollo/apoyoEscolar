...
public class Filtro extends Service{
	private static final long serialVersionUID = 6230013753285143307L;
	/**
	...
	public void planeacionNuevo(HttpServletRequest request, HttpSession session, Login usuVO,FiltroPlaneacionVO filtro) throws ServletException {
		try {
			request.setAttribute("listaEstado", planeacionDAO.getListaEstado());
			...
			if(filtro==null || filtro.getLblVigencia()==null){
				...
				session.setAttribute("filtroAprobacionActividadesVO",filtro);
			}
		} catch (Exception e) {
			e.printStackTrace();
