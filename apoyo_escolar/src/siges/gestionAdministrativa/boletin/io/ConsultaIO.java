...
public class ConsultaIO {
	 ...
	 */
	public ConsultaIO(BoletinDAO boletinDAO) {
	 ...
	 * @throws Exception
	 */
	public ResultadoConsultaVO buscarPlantillaBoletion(
			PlantillaBoletionVO plantillaBoletionVO, String path2)
			throws Exception {
		// System.out.println(formaFecha.format(new Date())
		// + "  buscarPlantillaBoletion");
		...
		try {
			...
			String rutaJasper = Ruta2.get(plantillaBoletionVO.getFilRutaBase(),
					rb.getString("consulta.rutaJasper"));
			String archivoJasper = rb.getString("consulta.jasper");

			String rutaExcel = Ruta.get(
					plantillaBoletionVO.getFilRutaBase(),
					rb.getString("consulta.path")
							+ plantillaBoletionVO.getFilUsuario());
			String archivoPdf = dane + "_VIGENCIA_"
					+ plantillaBoletionVO.getPlabolvigencia() + "_DOC_"
					+ plantillaBoletionVO.getPlabolnumdoc2() + "_FECHA_"
					+ fecha + ".pdf";
			String archivoZip = dane + "_VIGENCIA_"
					+ plantillaBoletionVO.getPlabolvigencia() + "_DOC_"
					+ plantillaBoletionVO.getPlabolnumdoc2() + "_FECHA_"
					+ fecha + ".zip";
			String rutaExcelRelativo = Ruta.getRelativo(
					...
					rb.getString("consulta.path")
							+ plantillaBoletionVO.getFilUsuario());
			String rutaImagen = Ruta2.get(plantillaBoletionVO.getFilRutaBase(),
					rb.getString("consulta.rutaImagen"));
			String archivoImagen = Ruta2.get(rutaImagen, "")
					+ rb.getString("consulta.imagen");

			File reportFile = new File(rutaJasper + archivoJasper);

			Map map = new HashMap();
			map.put("SUBREPORT_DIR", rutaJasper);
			// System.out.println("rutaImagen + archivoImagen " + rutaImagen
			// + archivoImagen);
			...
			map.put("GRADO_GRUPO", "0" + plantillaBoletionVO.getPlabolgrado()
					+ "" + plantillaBoletionVO.getPlabolgrupoNomb());
			map.put("PERIODO",
					new Integer((int) plantillaBoletionVO.getPlabolperido()));
			map.put("VIGENCIA",
					new Integer((int) plantillaBoletionVO.getPlabolvigencia()));
			map.put("RESOLUCION",
					"" + plantillaBoletionVO.getPlabolinstResolucion());
			map.put("DOCUMENTO_ESTUDIANTE",
					"" + plantillaBoletionVO.getPlabolnumdoc2());
			map.put("NOMBRE_ESTUDIANTE",
					"" + plantillaBoletionVO.getPlabolnomb());
			map.put("NOMBRE_DIRECTOR",
					"" + plantillaBoletionVO.getPlabolcoordNom());
			map.put("ESCUDO_SED", "" + archivoImagen);
			map.put("NUM_PERIODOS",
					new Integer(plantillaBoletionVO.getPlabolperidoNum()));
			map.put("SUBTITULO", "LOGROS Y DESCRIPTORES");
			map.put("NOMBRE_PER_FINAL",
					"" + plantillaBoletionVO.getPlabolperidoNomb());
			...
			escudo = new File(path2 + "e"
					+ plantillaBoletionVO.getPlabolinstDane().trim() + ".gif");
			// System.out.println("escudo: "+escudo);
			if (escudo.exists()) {
				// System.out.println("EXISTE");
				map.put("ESCUDO_COLEGIO", path2 + "e"
						+ plantillaBoletionVO.getPlabolinstDane().trim()
						+ ".gif");
			...
			if (zip.ponerZip(rutaExcel, archivoZip, 100000, l)) {
				...
				// System.out.println("rutaExcelRelativo+archivoZip "
				// + rutaExcelRelativo + archivoZip);
				resultado.setRuta(rutaExcelRelativo + archivoZip);
				// System.out.println("rutaExcel+archivoZip " + rutaExcel
				// + archivoZip);
