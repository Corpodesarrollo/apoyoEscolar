...
public class Ajax extends Service {
	...
	private ComparativosDAO comDAO = null;
	...
	public String[] process(HttpServletRequest request,
		...
		comDAO = new ComparativosDAO(cursor);
		usuVO = (Login) request.getSession().getAttribute("login");
		FiltroReportesVO filtroVO = (FiltroReportesVO) request.getSession()
		...
		try {
			...
			case ParamsVO.FICHA_REPORTES:
				...
				case ParamsVO.CMD_AJAX_JORD:
					getJornada(request, usuVO, filtroVO);
					break;
				case ParamsVO.CMD_AJAX_METD:
					getMetodologia(request, usuVO, filtroVO);
					break;
				case ParamsVO.CMD_AJAX_GRAD:
					getGrado(request, usuVO, filtroVO);
					break;
				case ParamsVO.CMD_AJAX_GRUP:
					getGrupo(request, usuVO, filtroVO);
					break;
	 ...
	 */
	private void getMunicipios(HttpServletRequest request, Login usuVO,
			FiltroReportesVO filtroVO) throws ServletException {
		// System.out.println("APOYO: " + formaFecha.format(new Date())
		// + " Ajax getCOLEGIOS");
		try {
			String params[] = null;
			...
			if (params != null && GenericValidator.isLong(params[0])) {
				...
				request.getSession().setAttribute("listaLocalidad",
						comDAO.getLocalidades());
				request.setAttribute("listaMetodo",
						comDAO.getMetodologiaGlobal());
				...
				request.getSession().setAttribute("ajaxParam",
						String.valueOf(ParamsVO.CMD_AJAX_MUN));
				filtroVO.setFechaProm(comDAO.getFechaPromedios(
						ParamsVO.NIVEL_PROV, codProv));
				...
				request.getSession().setAttribute("fechaProm",
						filtroVO.getFechaProm());
				request.getSession().setAttribute("fechaPromValida",
						new Integer(filtroVO.getFechaPromValida()));
	 ...
	 * @throws ServletException
	 */
	private void getColegios(HttpServletRequest request, Login usuVO,
			FiltroReportesVO filtroVO) throws ServletException {
		...
		try {
			...
			if (params != null && GenericValidator.isLong(params[0])) {
				...
				request.getSession().setAttribute("ajaxParam",
						String.valueOf(ParamsVO.CMD_AJAX_INST));

				filtroVO.setFechaProm(comDAO.getFechaPromedios(
						ParamsVO.NIVEL_MUN, codLoc));
	 ...
	 * @throws ServletException
	 */
	private void getSedes(HttpServletRequest request, Login usuVO,
			FiltroReportesVO filtroVO) throws ServletException {
		// System.out.println(formaFecha.format(new Date()) +
		// " Ajax getSEDES ");
		try {
			...
			if (params != null && GenericValidator.isLong(params[0])) {
				...
				long codZona = Long.parseLong(params[1]);
				request.getSession().setAttribute("listaSede",
						comDAO.getSede(codInst, codZona));

				request.setAttribute("listaMetodo",
						comDAO.getMetodologia(codInst));
				request.setAttribute("listaJornada",
						comDAO.getJornada(codInst, -99));
				// System.out.println(formaFecha.format(new Date())
				// + " Ajax getSEDES INST " + codInst);
				if (filtroVO == null)
				...
				filtroVO.setMetodo(-99);
				filtroVO.setGrado(-99);

				request.setAttribute("listaGrado", comDAO.getGrado(filtroVO));

				request.setAttribute(
						"listaPeriodo",
						getListaPeriodo(nivelEval.getNumPer(),
								nivelEval.getNomPerFinal()));

				request.getSession().setAttribute("ajaxParam",
						String.valueOf(ParamsVO.CMD_AJAX_SEDE));

				filtroVO.setFechaProm(comDAO.getFechaPromedios(
						ParamsVO.NIVEL_INST, codInst));
				// System.out.println("AJAX INST FECHA PROM: "
				// + filtroVO.getFechaProm());
				if (filtroVO.getFechaProm() != null
						&& !filtroVO.getFechaProm().equals(""))
					filtroVO.setFechaPromValida(1);
	 ...
	 * @throws ServletException
	 */
	private void getJornada(HttpServletRequest request, Login usuVO,
			FiltroReportesVO filtroVO) throws ServletException {
		// System.out.println(formaFecha.format(new Date()) +
		// " Ajax getJornada");
		try {
			...
			if (params != null && GenericValidator.isLong(params[0])) {
				...
				request.getSession().setAttribute("listaJornada",
						comDAO.getJornada(codInst, codSede));
	...
	private void getMetodologia(HttpServletRequest request, Login usuVO,
		...
		try {
			...
			if (params != null && GenericValidator.isLong(params[0])
					...
					&& GenericValidator.isLong(params[2])) {
				long codInst = Long.parseLong(params[0]);
				long codSede = Long.parseLong(params[1]);
	 ...
	 * @throws ServletException
	 */
	private void getGrado(HttpServletRequest request, Login usuVO,
			FiltroReportesVO filtroVO) throws ServletException {
		// System.out.println(formaFecha.format(new Date()) +
		// "  Ajax getGrado");
		try {
	 ...
	 * @throws ServletException
	 */
	private void getGrupo(HttpServletRequest request, Login usuVO,
			FiltroReportesVO filtroVO) throws ServletException {
		// System.out.println(formaFecha.format(new Date()) +
		// "  Ajax getGrupo");
		try {
			...
			if (params != null && GenericValidator.isLong(params[0])) {
				...
				// FiltroReportesVO plantillaBoletionVO = new
				// FiltroReportesVO();
				filtroVO.setInst(codInst);
				filtroVO.setSede(codSede);
				filtroVO.setJornd(codJord);
				filtroVO.setMetodo(codMetodo);
				filtroVO.setGrado(codGrado);
				...
				request.getSession().setAttribute("listaGrupo",
						comDAO.getGrupo(filtroVO));

				request.getSession().setAttribute("ajaxParam",
						String.valueOf(ParamsVO.CMD_AJAX_GRUP));
	 ...
	 * @throws ServletException
	 */
	private void getEst(HttpServletRequest request, Login usuVO)
			throws ServletException {
		...
		try {
			...
			if (params != null && GenericValidator.isLong(params[0])) {
				...
				// request.getSession().setAttribute("nomEst",comDAO.getGrado(codInst,
				// codMetodo));
				request.getSession().setAttribute("ajaxParam",
						String.valueOf(ParamsVO.CMD_AJAX_GRAD));
	 ...
	 * @throws ServletException
	 */
	private void getAsignaturas(HttpServletRequest request, Login usuVO)
			throws ServletException {
		// System.out.println(formaFecha.format(new Date())
		// + "  Ajax getASIGNATURAS");
		try {
			...
			if (conAreAsig_ == 1) {
				...
				if (codInst > 0)
					areAsig = comDAO.getAsignaturas(codInst, codMetodo,
							codGrado, asig, vig_);
				else
			...
			request.setAttribute("ajaxParam",
					String.valueOf(ParamsVO.CMD_AJAX_ASIG));
			// System.out.println("AJAX ASIG COMPARATIVOS, CON AREASIG: "
			// + conAreAsig_ + ", TIPOREP:" + tipoRep_);
			request.setAttribute("ajaxConAreAsig", new Integer(
					(int) conAreAsig_));
			request.setAttribute("ajaxTipoRep", new Integer((int) tipoRep_));
	 ...
	 * @throws ServletException
	 */
	private void getRango(HttpServletRequest request, Login usuVO,
			FiltroReportesVO filtro) throws ServletException {
		// System.out.println(formaFecha.format(new Date())
		// + "  Ajax getTIPO EVAL RANGOS");
		try {
			NivelEvalVO nivelVO = new NivelEvalVO();
			nivelVO.setCod_nivel_eval(1);
			TipoEvalVO tipoEval = null;
			...
			if (params != null && GenericValidator.isLong(params[0])
					&& GenericValidator.isLong(params[1])
					&& GenericValidator.isLong(params[2])
					&& GenericValidator.isLong(params[3])
					&& GenericValidator.isLong(params[4])) {
				inst = params[0];
				sede = params[1];
				jornada = params[2];
				metod = params[3];
			...
			request.setAttribute("tipoEval", tipoEval);
			request.setAttribute("ajaxParam",
					String.valueOf(ParamsVO.CMD_AJAX_RANGO));
