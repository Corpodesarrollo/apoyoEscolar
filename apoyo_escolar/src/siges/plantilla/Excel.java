...
import siges.util.beans.Recuperacion;

/**
 * siges.plantilla<br>
 * Funcinn: Clase que genera las plantillas fisicamente en disco a partir de las
 * plantillas y de los datos proporcionados por la clase que lo instancia <br>
 */
public class Excel {
	...
	private HSSFCellStyle cellstyleGREEN;
	private HSSFCellStyle cellstyleLABEL_GREEAN;
	private HSSFCellStyle cellstyleYELLOW;
	private HSSFCellStyle cellstyleBLUE;
	 ...
	 *             Version 1.1.<br>
	 */
	public boolean plantillaPreescolar(String[] config, Collection[] c,
			FiltroPlantilla filtro, List listaDimen) {
		...
		try {
			...
			/*
			 * String tipo[] = { "", "Corporal", "Comunicativa", "Cognitiva",
			 * "ntica", "Estntica" }; if (filtro.getGrado().equals("30") ||
			 * filtro.getGrado().equals("31")) { tipo[1] =
			 * "Comportamiento Social"; tipo[2] = "Comunicativa"; tipo[3] =
			 * "Cientnfica"; tipo[4] = "Convivencia"; tipo[5] = "Lndica"; }
			 */

			// hoja de escalas
			sheet = workbook.getSheetAt(1);// OBTENER HOJA DE ESCALAS
			setEscalaPree(c, filtro);
			// filtro.setJerarquiaLogro(jerarquia);

			pintarColores();
			for (int y = 0; y < listaDimen.size(); y++) {

				// sheet = workbook.getSheetAt(y);// OBTENER HOJA DE PLANTILLA
				DimensionesVO dimensionesVO = (DimensionesVO) listaDimen.get(y);
				sheet = workbook.createSheet(dimensionesVO.getDimnombre());
				...
				filtro.setArea("" + y);
				filtro.setArea_(dimensionesVO.getDimnombre());

				setEncabezadoPreescolar(filtro);
				setEstudiantesPreescolar(c[0], 0);
				setNotasPreescolar(c[0], c[2], "" + (y + 1));

			}
			sheet = workbook.getSheetAt(0);// OBTENER HOJA OCULTA
	 ...
	 *             Version 1.1.<br>
	 */
	public boolean plantillaLogroAsignatura(String[] config, Collection[] c,
			FiltroPlantilla filtro) {
		// 0=Estudiantes
		// 1=Escala asignatura
		...
		try {
			...
			setEscalaLogroAsignatura(c, filtro);
			filtro.setJerarquiaLogro(jerarquia);
			// hoja de plantilla
			...
			setLogros(c[4], 1);
			setNotasAsignatura(c[0], c[2]);
			setNotasLogro(c[0], c[4], c[6]);
			// hoja oculta
			...
			if (!f.exists())
				FileUtils.forceMkdir(f);
			fileOut = new FileOutputStream(config[2] + config[3]);// CREAR UN
			// ARCHIVO
			workbook.write(fileOut);// ESCRIBIR EL ARCHIVO A DISCO
			fileOut.flush();// CERRAR
			fileOut.close();
			...
			setMensaje(e.toString());
			System.out.println("PLANTILLA ASIG :IO Error Asignatura: "
					+ e.toString());
			return false;
		} catch (Exception e) {
			setMensaje(e.toString());
			...
			System.out.println("PLANTILLA ASIG : Error Asignatura: "
					+ e.toString());
			return false;
		} finally {
			try {
	...
	public boolean plantillaRecuperacionLogro(String[] config, Collection[] c,
			FiltroPlantilla filtro) {
		// 0=Estudiantes
		// 1=Logros
		// 2=Escala de logros
	 ...
	 *             Version 1.1.<br>
	 */
	public boolean plantillaAreaDescriptor(String[] config, Collection[] c,
			FiltroPlantilla filtro) throws Exception {
//		System.out.println("plantillaAreaDescriptor ");
		/*
		...
		try {
			...
			// hoja de escalas
			sheet = workbook.getSheetAt(0);// OBTENER HOJA DE ESCALAS
			setEscalaAreaDescriptor(c[1], filtro);
			// filtro.setJerarquiaLogro(jerarquia);
	 ...
	 *             Version 1.1.<br>
	 */
	public boolean plantillaBateriaLogro(String[] config, Collection[] c,
			Logros filtro) {
		java.sql.Date d = new java.sql.Date(new java.util.Date().getTime());
		int numFila = 2;
		int numCelda = -2;
		...
		try {
			...
			// setEncabezadoBateriaLogro(filtro);//PINTAR LOS LOGROS QUE ESTA EN
			// BD
	 ...
	 *             Version 1.1.<br>
	 */
	public boolean plantillaBateriaDescriptor(String[] config, Collection[] c,
			Logros filtro) {
		java.sql.Date d = new java.sql.Date(new java.util.Date().getTime());
		int numFila = 2;
		int numCelda = -2;
	 ...
	 *            Version 1.1.<br>
	 */
	public void setNotasLogro(Collection c1, Collection c2, Collection c4) {
		int numFila;
		int numCelda;
		try {
			if (!c1.isEmpty()) {
				...
				while (iterator.hasNext()) {
					...
					if (!c2.isEmpty()) {
						numCelda = Asignatura.Asig[Asignatura.EvaluacionLogro][Asignatura.COL];
						...
						while (iterator2.hasNext()) {
							...
							if (!c4.isEmpty()) {
								...
								while (iterator4.hasNext()) {
									...
									String vlr4 = (String) o2[3];// cod nota
									// evaluacion nota
									if (vlr.equals(id) && vlr2.equals(log)
											&& vlr3.equals(jer)) {
										cell = row.createCell((short) numCelda);
										cell.setCellValue((String) vlr4);
										break;
									}
								}
							}
			...
			System.out.println("PLANTILLA " + e.getMessage());
			e.printStackTrace();
		}
	}

	public void setNotasRecuperacion(Collection c1, Collection c2,
			Collection c3, Collection c4) {
		int numFila;
		int numCelda;
		if (!c1.isEmpty()) {
							...
							while (iterator4.hasNext()) {
								...
								if (!c3.isEmpty()) {
									...
									while (iterator3.hasNext()) {
										...
										String not2 = (String) o2[1];// nombre
										// nota
										if (vlr.equals(id) && vlr2.equals(log)
												&& vlr3.equals(jer)
												&& vlr4.equals(not)) {
											cell = row
													.createCell((short) numCelda);
	...
	/*
	 * Pone los descriptores que tienen los estudiantes en el archivo separados
	 * por comas @param: Collection c1 Lista de estudiantes @param: Collection
	 * c2 Lista de notas que ya estan {codest,tipodesc,coddesc}
	 */
	/**
	 * @param c1
	 * @param c2
	...
	public void setNotasDescriptor(Collection c1, Collection c2) {
		...
		String id;
		String z;
		int celdas[] = { 0, Area.Area[Area.EvalFortaleza][Area.COL],
		...
		if (!c1.isEmpty()) {
			...
			while (iterator.hasNext()) {
				...
				if (!c2.isEmpty()) {
					...
					while (iterator2.hasNext()) {
						o = ((Object[]) iterator2.next());
						String[] id2 = { (String) o[0], (String) o[1],
								(String) o[2] };
						if (id2[0].equals(id)) {
							numCelda = celdas[Integer.parseInt(id2[1])] + 1;
...
//		System.out.println("setEscalaLogroAsignatura ");
		try {
			if (c != null) {
				...
				if (!c[1].isEmpty()) {
					 ...
					 * MODIFICADO 20-03-10
					 * */
					// SI ES DIFERENTE A LA ESCALA CONCEPTUAL DEBE PINTAR EN EL
					// ENCABEZADO "MAXIMO Y MINIMO"
					if (filtroPlantilla.getFilCodTipoEval() != ParamsVO.ESCALA_CONCEPTUAL) {
						int numFilaEnc = Asignatura.Asig[Asignatura.EscalaAsignatura1][Asignatura.FIL] - 1;
						row = sheet.createRow(numFilaEnc);
					...
					while (iterator.hasNext()) {
						...
						int[] pos = {
								0,
								Asignatura.Asig[Asignatura.EscalaAsignatura1][Asignatura.COL],
								Asignatura.Asig[Asignatura.EscalaAsignatura2][Asignatura.COL] };
				...
				if (!c[5].isEmpty()) {
					...
					while (iterator.hasNext()) {
						...
						int[] pos = {
								0,
								Asignatura.Asig[Asignatura.EscalaLogro1][Asignatura.COL],
								Asignatura.Asig[Asignatura.EscalaLogro2][Asignatura.COL] };
						for (int j = 1; j < o.length; j++) {
							cell = row.createCell((short) pos[j]);
							cell.setCellValue((String) (o[j] != null ? o[j]
									: ""));
				...
				/*
				 * //MOTIVOS DE AUSENCIA
				 * if(!Recursos.getRecurso(Recursos.AUSENCIA).isEmpty()){ int
				 * numFila
				 * =Asignatura.Asig[Asignatura.Ausencia1][Asignatura.FIL];
				 * iterator =Recursos.getRecurso(Recursos.AUSENCIA).iterator();
				 * while(iterator.hasNext()){ row=sheet.createRow(numFila++);
				 * o=(Object[])iterator.next(); int[]
				 * pos={Asignatura.Asig[Asignatura
				 * .Ausencia1][Asignatura.COL],Asignatura
				 * .Asig[Asignatura.Ausencia2][Asignatura.COL]}; for(int
				 * j=0;j<o.length-1;j++){ cell=row.createCell((short)pos[j]);
				 * cell.setCellValue((String)(o[j]!=null?o[j]:"")); } } }
				 */
				// LOGROS
				if (!c[4].isEmpty()) {
					...
					while (iterator.hasNext()) {
						...
						int[] pos = {
								0,
								Asignatura.Asig[Asignatura.ListaLogro1][Asignatura.COL],
								Asignatura.Asig[Asignatura.ListaLogro2][Asignatura.COL] };
						// int[] pos_={0,0,1};
			...
			System.out.println("PLANTILLA LOGROS:" + e.getMessage());
			e.printStackTrace();
	 ...
	 *            Version 1.1.<br>
	 */
	public void setEscalaPree(Collection[] c, FiltroPlantilla filtroPlantilla) {
//		System.out.println("setEscalaLogroPree ");
		...
		if (c != null) {
			...
			if (!c[2].isEmpty()) {
				...
				while (iterator.hasNext()) {
					...
					int[] pos = {
							0,
							Recuperacion.Rec[Recuperacion.EscalaLogro1][Recuperacion.COL],
			...
			if (!c[1].isEmpty()) {
				...
				while (iterator.hasNext()) {
					...
					int[] pos = {
							0,
							Recuperacion.Rec[Recuperacion.ListaLogro1][Recuperacion.COL],
	...
	public void setEscalaAreaDescriptor(Collection c,
		...
		if (c != null) {
			...
			if (filtroPlantilla.getFilCodTipoEval() != ParamsVO.ESCALA_CONCEPTUAL) {
				...
				cell.setCellValue((String) ("VALOR MINIMO"));
				HSSFCellStyle cellstyle1 = workbook.createCellStyle();
				// cellstyle1.setFillBackgroundColor(HSSFColor.LIGHT_TURQUOISE.index);
				cellstyle1.setBorderBottom(cellstyle1.BORDER_THIN);
				HSSFFont cellfont = workbook.createFont();
				cellfont.setColor(HSSFColor.DARK_TEAL.index);
				cellfont.setBoldweight(HSSFFont.BOLDWEIGHT_BOLD);
			...
			if (!c.isEmpty()) {
				...
				while (iterator.hasNext()) {
					row = sheet.createRow(numFila++);
					o = (Object[]) iterator.next();
					int[] pos = { 0, Area.Area[Area.EscalaArea1][Area.COL],
							Area.Area[Area.EscalaArea2][Area.COL] };
					for (int j = 1; j < o.length; j++) {
						cell = row.createCell((short) pos[j]);
						cell.setCellValue((String) (o[j] != null ? o[j] : ""));
					}
			 ...
			 * //MOTIVOS DE AUSENCIA
			 * if(!Recursos.getRecurso(Recursos.AUSENCIA).isEmpty()){ int
			 * numFila=Area.Area[Area.Ausencia1][Area.FIL]; iterator
			 * =Recursos.getRecurso(Recursos.AUSENCIA).iterator();
			 * while(iterator.hasNext()){ row=sheet.createRow(numFila++);
			 * o=(Object[])iterator.next(); int[]
			 * pos={Area.Area[Area.Ausencia1][
			 * Area.COL],Area.Area[Area.Ausencia2][Area.COL]}; for(int
			 * j=0;j<o.length-1;j++){ cell=row.createCell((short)pos[j]);
			 * cell.setCellValue((String)(o[j]!=null?o[j]:"")); } } }
			 */
		}
	...
	public void setEncabezadoBateriaLogro(Logros f) {
		...
		if (f != null) {
			...
			cell = row
					.getCell((short) BateriaLogro.BatLog[BateriaLogro.Institucion2][BateriaLogro.COL]);
			cell.setCellValue((String) f.getPlantillaInstitucion_());
			// FECHA
			numFila = BateriaLogro.BatLog[BateriaLogro.Fecha2][BateriaLogro.FIL];
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) BateriaLogro.BatLog[BateriaLogro.Fecha2][BateriaLogro.COL]);
			cell.setCellValue(d.toString());
			// grado
			numFila = BateriaLogro.BatLog[BateriaLogro.Grado2][BateriaLogro.FIL];
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) BateriaLogro.BatLog[BateriaLogro.Grado2][BateriaLogro.COL]);
			cell.setCellValue((String) f.getPlantillaGrado_());
			// ASIGNATURA
			numFila = BateriaLogro.BatLog[BateriaLogro.Asignatura2][BateriaLogro.FIL];
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) BateriaLogro.BatLog[BateriaLogro.Asignatura2][BateriaLogro.COL]);
			cell.setCellValue((String) f.getPlantillaAsignatura_());
			// metodologia
			numFila = BateriaLogro.BatLog[BateriaLogro.Metodologia2][BateriaLogro.FIL];
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) BateriaLogro.BatLog[BateriaLogro.Metodologia2][BateriaLogro.COL]);
			cell.setCellValue((String) f.getPlantillaMetodologia_());
			// Docente
			numFila = BateriaLogro.BatLog[BateriaLogro.Docente2][BateriaLogro.FIL];
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) BateriaLogro.BatLog[BateriaLogro.Docente2][BateriaLogro.COL]);
			cell.setCellValue((String) f.getPlantillaDocente_());
		}
	}
	...
	public void setEncabezadoBateriaDescriptor(Logros f) {
		...
		if (f != null) {
			...
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) BateriaDescriptor.BatDes[BateriaDescriptor.Institucion2][BateriaDescriptor.COL]);
			cell.setCellValue((String) f.getPlantillaInstitucion_());
			// FECHA
			numFila = BateriaDescriptor.BatDes[BateriaDescriptor.Fecha2][BateriaDescriptor.FIL];
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) BateriaDescriptor.BatDes[BateriaDescriptor.Fecha2][BateriaDescriptor.COL]);
			cell.setCellValue(d.toString());
			// GRADO
			numFila = BateriaDescriptor.BatDes[BateriaDescriptor.Grado2][BateriaDescriptor.FIL];
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) BateriaDescriptor.BatDes[BateriaDescriptor.Grado2][BateriaDescriptor.COL]);
			cell.setCellValue((String) f.getPlantillaGrado_());
			// ASIGNATURA
			numFila = BateriaDescriptor.BatDes[BateriaDescriptor.Area2][BateriaDescriptor.FIL];
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) BateriaDescriptor.BatDes[BateriaDescriptor.Area2][BateriaDescriptor.COL]);
			cell.setCellValue((String) f.getPlantillaAsignatura_());
			// metodologia
			numFila = BateriaDescriptor.BatDes[BateriaDescriptor.Metodologia2][BateriaDescriptor.FIL];
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) BateriaDescriptor.BatDes[BateriaDescriptor.Metodologia2][BateriaDescriptor.COL]);
			cell.setCellValue((String) f.getPlantillaMetodologia_());
			// docente
			numFila = BateriaDescriptor.BatDes[BateriaDescriptor.Docente2][BateriaDescriptor.FIL];
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) BateriaDescriptor.BatDes[BateriaDescriptor.Docente2][BateriaDescriptor.COL]);
			cell.setCellValue((String) f.getPlantillaDocente_());
		}
	}
	...
	public void setEncabezadoBateriaLogroOculto(Logros f) {
		...
		if (f != null) {
			...
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) BateriaLogro.BatLog[BateriaLogro.Institucion1][BateriaLogro.COL]);
			...
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) BateriaLogro.BatLog[BateriaLogro.Grado1][BateriaLogro.COL]);
			...
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) BateriaLogro.BatLog[BateriaLogro.Asignatura1][BateriaLogro.COL]);
			...
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) BateriaLogro.BatLog[BateriaLogro.Metodologia1][BateriaLogro.COL]);
			...
			numFila = BateriaLogro.BatLog[BateriaLogro.Docente1][BateriaLogro.FIL];
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) BateriaLogro.BatLog[BateriaLogro.Docente1][BateriaLogro.COL]);
			cell.setCellValue(String.valueOf(f.getPlantillaDocente()));

			// tipo
			numFila = BateriaLogro.BatLog[BateriaLogro.Tipo][BateriaLogro.FIL];
			row = sheet.createRow(numFila);
			cell = row
					.createCell((short) BateriaLogro.BatLog[BateriaLogro.Tipo][BateriaLogro.COL]);
			cell.setCellValue(f.getTipoPlantilla());
			// nivel de logro
			numFila = BateriaLogro.BatLog[BateriaLogro.Nivel][BateriaLogro.COL];
			cell = row
					.createCell((short) BateriaLogro.BatLog[BateriaLogro.Nivel][BateriaLogro.COL]);
			cell.setCellValue(f.getNivelLogro());
			// plan estudios
			numFila = BateriaLogro.BatLog[BateriaLogro.PlanEstudios][BateriaLogro.COL];
			cell = row
					.createCell((short) BateriaLogro.BatLog[BateriaLogro.PlanEstudios][BateriaLogro.COL]);
			cell.setCellValue(f.getEstadoPlanEstudios());
		}
	}
	...
	public void setEncabezadoBateriaDescriptorOculto(Logros f) {
		...
		if (f != null) {
			...
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) BateriaDescriptor.BatDes[BateriaDescriptor.Institucion1][BateriaDescriptor.COL]);
			cell.setCellValue((String) f.getPlantillaInstitucion());
			// FECHA
			numFila = BateriaDescriptor.BatDes[BateriaDescriptor.Fecha2][BateriaDescriptor.FIL];
			row = sheet.getRow(numFila);
			...
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) BateriaDescriptor.BatDes[BateriaDescriptor.Grado1][BateriaDescriptor.COL]);
			cell.setCellValue((String) f.getPlantillaGrado());
			// ASIGNATURA
			numFila = BateriaDescriptor.BatDes[BateriaDescriptor.Area2][BateriaDescriptor.FIL];
			row = sheet.getRow(numFila);
			...
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) BateriaDescriptor.BatDes[BateriaDescriptor.Area1][BateriaDescriptor.COL]);
			cell.setCellValue((String) f.getPlantillaAsignatura());
			// metodologia
			numFila = BateriaDescriptor.BatDes[BateriaDescriptor.Metodologia2][BateriaDescriptor.FIL];
			row = sheet.getRow(numFila);
			...
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) BateriaDescriptor.BatDes[BateriaDescriptor.Metodologia1][BateriaDescriptor.COL]);
			cell.setCellValue((String) f.getPlantillaMetodologia());
			...
			numFila = BateriaDescriptor.BatDes[BateriaDescriptor.Docente1][BateriaDescriptor.FIL];
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) BateriaDescriptor.BatDes[BateriaDescriptor.Docente1][BateriaDescriptor.COL]);
			cell.setCellValue(String.valueOf(f.getPlantillaDocente()));
			...
			row = sheet.createRow(numFila);
			cell = row
					.createCell((short) BateriaDescriptor.BatDes[BateriaDescriptor.Tipo][BateriaDescriptor.COL]);
			cell.setCellValue(f.getTipoPlantilla());
			// nivel de logro
			numFila = BateriaDescriptor.BatDes[BateriaDescriptor.Nivel][BateriaDescriptor.COL];
			cell = row
					.createCell((short) BateriaDescriptor.BatDes[BateriaDescriptor.Nivel][BateriaDescriptor.COL]);
			cell.setCellValue(f.getNivelLogro());
			// plan estudios
			numFila = BateriaDescriptor.BatDes[BateriaDescriptor.PlanEstudios][BateriaDescriptor.FIL];
			row = sheet.createRow(numFila);
			cell = row
					.createCell((short) BateriaDescriptor.BatDes[BateriaDescriptor.PlanEstudios][BateriaDescriptor.COL]);
			cell.setCellValue(f.getEstadoPlanEstudios());
		}
	}
...
//		System.out.println("setEncabezadoLogroAsignatura");
		java.sql.Date d = new java.sql.Date(new java.util.Date().getTime());
		int numFila = 1;
		try {
			if (f != null) {
				...
				if (f.getCerrar().equals("1")) {
					...
					if (row == null)
						row = sheet.createRow(numFila);
					cell = row
							.getCell((short) Asignatura.Asig[Asignatura.TxtCerrado][Asignatura.COL]);
					if (cell == null)
			...
			System.out.println("PLANTILLA LOGRO " + e.getMessage());
			e.printStackTrace();
		}
	}
	...
	public void setEncabezadoRecuperacion(FiltroPlantilla f) {
		...
		if (f != null) {
			...
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) Recuperacion.Rec[Recuperacion.Institucion2][Recuperacion.COL]);
			cell.setCellValue((String) f.getInstitucion_());
			// sede
			numFila = Recuperacion.Rec[Recuperacion.Sede2][Recuperacion.FIL];
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) Recuperacion.Rec[Recuperacion.Sede2][Recuperacion.COL]);
			cell.setCellValue((String) f.getSede_());
			// jornada
			numFila = Recuperacion.Rec[Recuperacion.Jornada2][Recuperacion.FIL];
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) Recuperacion.Rec[Recuperacion.Jornada2][Recuperacion.COL]);
			cell.setCellValue((String) f.getJornada_());
			// grado
			numFila = Recuperacion.Rec[Recuperacion.Grado2][Recuperacion.FIL];
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) Recuperacion.Rec[Recuperacion.Grado2][Recuperacion.COL]);
			cell.setCellValue((String) f.getGrado_());
			// GRUPO
			numFila = Recuperacion.Rec[Recuperacion.Grupo2][Recuperacion.FIL];
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) Recuperacion.Rec[Recuperacion.Grupo2][Recuperacion.COL]);
			cell.setCellValue((String) f.getGrupo_());
			// ASIGNATURA
			numFila = Recuperacion.Rec[Recuperacion.Asignatura2][Recuperacion.FIL];
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) Recuperacion.Rec[Recuperacion.Asignatura2][Recuperacion.COL]);
			cell.setCellValue((String) f.getAsignatura_());
			// PERIODO
			numFila = Recuperacion.Rec[Recuperacion.Periodo2][Recuperacion.FIL];
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) Recuperacion.Rec[Recuperacion.Periodo2][Recuperacion.COL]);
			cell.setCellValue((String) f.getPeriodo_());
			// PERIODO actual
			numFila = Recuperacion.Rec[Recuperacion.PeriodoActual2][Recuperacion.FIL];
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) Recuperacion.Rec[Recuperacion.PeriodoActual2][Recuperacion.COL]);
			cell.setCellValue((String) f.getPeriodo2_());
			// DOCENTE
			numFila = Recuperacion.Rec[Recuperacion.Docente2][Recuperacion.FIL];
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) Recuperacion.Rec[Recuperacion.Docente2][Recuperacion.COL]);
			cell.setCellValue((String) f.getDocente_());
			// FECHA
			numFila = Recuperacion.Rec[Recuperacion.Fecha2][Recuperacion.FIL];
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) Recuperacion.Rec[Recuperacion.Fecha2][Recuperacion.COL]);
			cell.setCellValue(d.toString());
			// METODOLOGIA
			numFila = Recuperacion.Rec[Recuperacion.Metodologia2][Recuperacion.FIL];
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) Recuperacion.Rec[Recuperacion.Metodologia2][Recuperacion.COL]);
			cell.setCellValue((String) f.getMetodologia_());
			// cerrado o no
			if (f.getCerrar().equals("1")) {
				...
				if (row == null)
					row = sheet.createRow(numFila);
				cell = row
						.getCell((short) Recuperacion.Rec[Recuperacion.TxtCerrado][Recuperacion.COL]);
				if (cell == null)
					cell = row.createCell((short) 2);
		...
		if (f != null) {
			...
			if (f.getCerrar().equals("1")) {
				...
				if (row == null)
					row = sheet.createRow(numFila);
				cell = row
						.getCell((short) Area.Area[Area.TxtCerrado][Area.COL]);
				if (cell == null)
					cell = row.createCell((short) 2);
				cell.setCellValue("LA EVALUACION ESTA CERRADA");
			}
	...
	public void setEncabezadoPreescolar(FiltroPlantilla f) {
		...
		if (f != null) {
			...
			sheet.setColumnWidth(
					(short) Preescolar.Pree[Preescolar.Institucion1][Preescolar.COL],
					(short) 3000);
			sheet.setColumnWidth(
					(short) Preescolar.Pree[Preescolar.Institucion2][Preescolar.COL],
					(short) 3000);
			sheet.setColumnWidth(
					(short) Preescolar.Pree[Preescolar.Evaluacion][Preescolar.COL],
					(short) 10500);
			sheet.setColumnWidth(
					(short) Preescolar.Pree[Preescolar.Metodologia1][Preescolar.COL],
					(short) 3000);
			// sheet.setColumnWidth((short)Preescolar.Pree[Preescolar.Nombre][Preescolar.COL],(short)6500);
			// sheet.setColumnWidth((short)Preescolar.Pree[Preescolar.Apellido][Preescolar.COL],(short)6500);
			...
			// titulo
			cell = row
					.getCell((short) Preescolar.Pree[Preescolar.Institucion1][Preescolar.COL]);

			if (cell == null) {
				cell = row
						.createCell((short) Preescolar.Pree[Preescolar.Institucion1][Preescolar.COL]);
			}

			cell.setCellValue((String) Preescolar.labelINSTITUCION);
			cell.setCellStyle(cellstyleLABEL_GREEAN);

			// dato
			cell = row
					.getCell((short) Preescolar.Pree[Preescolar.Institucion2][Preescolar.COL]);

			if (cell == null) {
				cell = row
						.createCell((short) Preescolar.Pree[Preescolar.Institucion2][Preescolar.COL]);
			}
			...
			// label
			cell = row
					.getCell((short) Preescolar.Pree[Preescolar.Sede1][Preescolar.COL]);
			if (cell == null) {
				cell = row
						.createCell((short) Preescolar.Pree[Preescolar.Sede1][Preescolar.COL]);
			}
			cell.setCellValue((String) Preescolar.labelSEDE);
			cell.setCellStyle(cellstyleLABEL_GREEAN);

			// dato
			cell = row
					.getCell((short) Preescolar.Pree[Preescolar.Sede2][Preescolar.COL]);
			if (cell == null) {
				cell = row
						.createCell((short) Preescolar.Pree[Preescolar.Sede2][Preescolar.COL]);
			}
			cell.setCellValue((String) f.getSede_());
			cell.setCellStyle(cellstyleYELLOW);
			...
			cell = row
					.getCell((short) Preescolar.Pree[Preescolar.Jornada1][Preescolar.COL]);
			if (cell == null) {
				cell = row
						.createCell((short) Preescolar.Pree[Preescolar.Jornada1][Preescolar.COL]);
			}
			cell.setCellValue((String) Preescolar.labelJORD);
			cell.setCellStyle(cellstyleLABEL_GREEAN);

			// DATO
			cell = row
					.getCell((short) Preescolar.Pree[Preescolar.Jornada2][Preescolar.COL]);
			if (cell == null) {
				cell = row
						.createCell((short) Preescolar.Pree[Preescolar.Jornada2][Preescolar.COL]);
			}
			cell.setCellValue((String) f.getJornada_());
			cell.setCellStyle(cellstyleYELLOW);
			...
			// LABEL
			cell = row
					.getCell((short) Preescolar.Pree[Preescolar.Grado1][Preescolar.COL]);
			if (cell == null) {
				cell = row
						.createCell((short) Preescolar.Pree[Preescolar.Grado1][Preescolar.COL]);
			}

			cell.setCellValue((String) Preescolar.labelGRADO);
			cell.setCellStyle(cellstyleLABEL_GREEAN);

			// DATO
			cell = row
					.getCell((short) Preescolar.Pree[Preescolar.Grado2][Preescolar.COL]);
			if (cell == null) {
				cell = row
						.createCell((short) Preescolar.Pree[Preescolar.Grado2][Preescolar.COL]);
			}
			...
			// TITULO
			cell = row
					.getCell((short) Preescolar.Pree[Preescolar.Grupo1][Preescolar.COL]);
			if (cell == null) {
				cell = row
						.createCell((short) Preescolar.Pree[Preescolar.Grupo1][Preescolar.COL]);
			}
			cell.setCellValue((String) Preescolar.labelGRUPO);
			cell.setCellStyle(cellstyleLABEL_GREEAN);

			// DATO
			cell = row
					.getCell((short) Preescolar.Pree[Preescolar.Grupo2][Preescolar.COL]);
			if (cell == null) {
				cell = row
						.createCell((short) Preescolar.Pree[Preescolar.Grupo2][Preescolar.COL]);
			}
			cell.setCellValue((String) f.getGrupo_());
			...
			// TITULO
			cell = row
					.getCell((short) Preescolar.Pree[Preescolar.Dimension1][Preescolar.COL]);
			if (cell == null) {
				cell = row
						.createCell((short) Preescolar.Pree[Preescolar.Dimension1][Preescolar.COL]);
			}

			cell.setCellValue(Preescolar.labelDIMENSION);
			cell.setCellStyle(cellstyleLABEL_GREEAN);
			// DATO
			cell = row
					.getCell((short) Preescolar.Pree[Preescolar.Dimension2][Preescolar.COL]);
			if (cell == null) {
				cell = row
						.createCell((short) Preescolar.Pree[Preescolar.Dimension2][Preescolar.COL]);
			}
			...
			// LABEL
			cell = row
					.getCell((short) Preescolar.Pree[Preescolar.Periodo1][Preescolar.COL]);
			if (cell == null) {
				cell = row
						.createCell((short) Preescolar.Pree[Preescolar.Periodo1][Preescolar.COL]);
			}

			cell.setCellValue((String) Preescolar.labelPERIODO);
			cell.setCellStyle(cellstyleLABEL_GREEAN);

			// DATO
			cell = row
					.getCell((short) Preescolar.Pree[Preescolar.Periodo2][Preescolar.COL]);
			if (cell == null) {
				cell = row
						.createCell((short) Preescolar.Pree[Preescolar.Periodo2][Preescolar.COL]);
			}
			...
			// label
			cell = row
					.getCell((short) Preescolar.Pree[Preescolar.Docente1][Preescolar.COL]);
			if (cell == null) {
				cell = row
						.createCell((short) Preescolar.Pree[Preescolar.Docente1][Preescolar.COL]);
			}
			cell.setCellValue((String) Preescolar.labelDOCENTE);
			cell.setCellStyle(cellstyleLABEL_GREEAN);

			cell = row
					.getCell((short) Preescolar.Pree[Preescolar.Docente2][Preescolar.COL]);
			if (cell == null) {
				cell = row
						.createCell((short) Preescolar.Pree[Preescolar.Docente2][Preescolar.COL]);
			}
			cell.setCellValue((String) f.getDocente_());
			cell.setCellStyle(cellstyleGREEN);
			...
			// label
			cell = row
					.getCell((short) Preescolar.Pree[Preescolar.Fecha1][Preescolar.COL]);
			if (cell == null) {
				cell = row
						.createCell((short) Preescolar.Pree[Preescolar.Fecha1][Preescolar.COL]);
			}
			cell.setCellValue(Preescolar.labelFECHA);
			cell.setCellStyle(cellstyleLABEL_GREEAN);

			// datos
			cell = row
					.getCell((short) Preescolar.Pree[Preescolar.Fecha2][Preescolar.COL]);
			if (cell == null) {
				cell = row
						.createCell((short) Preescolar.Pree[Preescolar.Fecha2][Preescolar.COL]);
			}
			cell.setCellValue(d.toString());
			cell.setCellStyle(cellstyleGREEN);
			...
			// label
			cell = row
					.getCell((short) Preescolar.Pree[Preescolar.Metodologia1][Preescolar.COL]);
			if (cell == null) {
				cell = row
						.createCell((short) Preescolar.Pree[Preescolar.Metodologia1][Preescolar.COL]);
			}
			cell.setCellValue((String) Preescolar.labelMETOD);
			cell.setCellStyle(cellstyleLABEL_GREEAN);

			// DATOS
			cell = row
					.getCell((short) Preescolar.Pree[Preescolar.Metodologia2][Preescolar.COL]);
			if (cell == null) {
				cell = row
						.createCell((short) Preescolar.Pree[Preescolar.Metodologia2][Preescolar.COL]);
			}
			cell.setCellValue((String) f.getMetodologia_());
			cell.setCellStyle(cellstyleGREEN);
			...
			if (f.getCerrar().equals("1")) {
				...
				cell = row
						.getCell((short) Preescolar.Pree[Preescolar.TxtCerrado][Preescolar.COL]);
				if (cell == null) {
					cell = row.createCell((short) 2);
				}
				cell.setCellValue("LA EVALUACION ESTA CERRADA");
	 ...
	 *            Version 1.1.<br>
	 */
	public void setEncabezadoPreescolarAntea(FiltroPlantilla f) {
		java.sql.Date d = new java.sql.Date(new java.util.Date().getTime());
		int numFila = 1;
		if (f != null) {
			// institucion
	...
	public void setEncabezadoLogroAsignaturaOculto(FiltroPlantilla f) {
		...
		try {
			if (f != null) {
				...
				row = sheet.getRow(numFila);
				cell = row
						.getCell((short) Asignatura.Asig[Asignatura.Institucion1][Asignatura.COL]);
				...
				row = sheet.getRow(numFila);
				cell = row
						.getCell((short) Asignatura.Asig[Asignatura.Sede1][Asignatura.COL]);
				...
				row = sheet.getRow(numFila);
				cell = row
						.getCell((short) Asignatura.Asig[Asignatura.Jornada1][Asignatura.COL]);
				...
				row = sheet.getRow(numFila);
				cell = row
						.getCell((short) Asignatura.Asig[Asignatura.Grado1][Asignatura.COL]);
				...
				row = sheet.getRow(numFila);
				cell = row
						.getCell((short) Asignatura.Asig[Asignatura.Grupo1][Asignatura.COL]);
				...
				row = sheet.getRow(numFila);
				cell = row
						.getCell((short) Asignatura.Asig[Asignatura.Asignatura1][Asignatura.COL]);
				...
				row = sheet.getRow(numFila);
				cell = row
						.getCell((short) Asignatura.Asig[Asignatura.Periodo1][Asignatura.COL]);
				...
				row = sheet.getRow(numFila);
				cell = row
						.getCell((short) Asignatura.Asig[Asignatura.Docente1][Asignatura.COL]);
				...
				row = sheet.getRow(numFila);
				cell = row
						.getCell((short) Asignatura.Asig[Asignatura.Metodologia1][Asignatura.COL]);
				cell.setCellValue((String) f.getMetodologia());
				// tipo
				numFila = Asignatura.Asig[Asignatura.Tipo][Asignatura.FIL];
				row = sheet.createRow(numFila);
				cell = row
						.createCell((short) Asignatura.Asig[Asignatura.Tipo][Asignatura.COL]);
				cell.setCellValue(f.getTipoPlantilla());
				// planestudios
				numFila = Asignatura.Asig[Asignatura.PlanEstudios][Asignatura.FIL];
				row = sheet.createRow(numFila);
				cell = row
						.createCell((short) Asignatura.Asig[Asignatura.PlanEstudios][Asignatura.COL]);
				cell.setCellValue(f.getFilPlanEstudios());
				// cerrado
				numFila = Asignatura.Asig[Asignatura.Cerrado][Asignatura.FIL];
				cell = row
						.createCell((short) Asignatura.Asig[Asignatura.Cerrado][Asignatura.COL]);
				cell.setCellValue(f.getCerrar());
				// nivel de los logros
				numFila = Asignatura.Asig[Asignatura.NivelLogro][Asignatura.FIL];
				cell = row
						.createCell((short) Asignatura.Asig[Asignatura.NivelLogro][Asignatura.COL]);
				cell.setCellValue(f.getNivelLogro());
				// jerarquia de grupo
				numFila = Asignatura.Asig[Asignatura.JerarquiaGrupo][Asignatura.FIL];
				cell = row
						.createCell((short) Asignatura.Asig[Asignatura.JerarquiaGrupo][Asignatura.COL]);
				cell.setCellValue(f.getJerarquiagrupo());
				// jerarquia de logro
				numFila = Asignatura.Asig[Asignatura.JerarquiaLogro][Asignatura.FIL];
				row = sheet.createRow(numFila);
				cell = row
						.createCell((short) Asignatura.Asig[Asignatura.JerarquiaLogro][Asignatura.COL]);
	...
	public void setEncabezadoRecuperacionOculto(FiltroPlantilla f) {
		...
		if (f != null) {
			...
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) Recuperacion.Rec[Recuperacion.Institucion1][Recuperacion.COL]);
			cell.setCellValue((String) f.getInstitucion());
			// sede
			numFila = Recuperacion.Rec[Recuperacion.Sede2][Recuperacion.FIL];
			row = sheet.getRow(numFila);
			...
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) Recuperacion.Rec[Recuperacion.Sede1][Recuperacion.COL]);
			cell.setCellValue((String) f.getSede());
			// jornada
			numFila = Recuperacion.Rec[Recuperacion.Jornada2][Recuperacion.FIL];
			row = sheet.getRow(numFila);
			...
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) Recuperacion.Rec[Recuperacion.Jornada1][Recuperacion.COL]);
			cell.setCellValue((String) f.getJornada());
			// grado
			numFila = Recuperacion.Rec[Recuperacion.Grado2][Recuperacion.FIL];
			row = sheet.getRow(numFila);
			...
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) Recuperacion.Rec[Recuperacion.Grado1][Recuperacion.COL]);
			cell.setCellValue((String) f.getGrado());
			// GRUPO
			numFila = Recuperacion.Rec[Recuperacion.Grupo2][Recuperacion.FIL];
			row = sheet.getRow(numFila);
			...
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) Recuperacion.Rec[Recuperacion.Grupo1][Recuperacion.COL]);
			cell.setCellValue((String) f.getGrupo());
			// ASIGNATURA
			numFila = Recuperacion.Rec[Recuperacion.Asignatura2][Recuperacion.FIL];
			row = sheet.getRow(numFila);
			...
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) Recuperacion.Rec[Recuperacion.Asignatura1][Recuperacion.COL]);
			cell.setCellValue((String) f.getAsignatura());
			// PERIODO
			numFila = Recuperacion.Rec[Recuperacion.Periodo2][Recuperacion.FIL];
			row = sheet.getRow(numFila);
			...
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) Recuperacion.Rec[Recuperacion.Periodo1][Recuperacion.COL]);
			cell.setCellValue((String) f.getPeriodo());
			// PERIODO ACTUAL
			numFila = Recuperacion.Rec[Recuperacion.PeriodoActual2][Recuperacion.FIL];
			row = sheet.getRow(numFila);
			...
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) Recuperacion.Rec[Recuperacion.PeriodoActual1][Recuperacion.COL]);
			...
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) Recuperacion.Rec[Recuperacion.Docente1][Recuperacion.COL]);
			cell.setCellValue((String) f.getDocente());
			// FECHA
			numFila = Recuperacion.Rec[Recuperacion.Fecha2][Recuperacion.FIL];
			row = sheet.getRow(numFila);
			...
			row = sheet.getRow(numFila);
			cell = row
					.getCell((short) Recuperacion.Rec[Recuperacion.Metodologia1][Recuperacion.COL]);
			cell.setCellValue((String) f.getMetodologia());
			// tipo
			numFila = Recuperacion.Rec[Recuperacion.Tipo][Recuperacion.FIL];
			row = sheet.createRow(numFila);
			cell = row
					.createCell((short) Recuperacion.Rec[Recuperacion.Tipo][Recuperacion.COL]);
			cell.setCellValue(f.getTipoPlantilla());
			// plan estudios
			numFila = Recuperacion.Rec[Recuperacion.PlanEstudios][Recuperacion.FIL];
			row = sheet.createRow(numFila);
			cell = row
					.createCell((short) Recuperacion.Rec[Recuperacion.PlanEstudios][Recuperacion.COL]);
			cell.setCellValue(f.getFilPlanEstudios());
			// cerrado
			numFila = Recuperacion.Rec[Recuperacion.Cerrado][Recuperacion.FIL];
			cell = row
					.createCell((short) Recuperacion.Rec[Recuperacion.Cerrado][Recuperacion.COL]);
			cell.setCellValue(f.getCerrar());
			// nivel de los logros
			numFila = Recuperacion.Rec[Recuperacion.NivelLogro][Recuperacion.FIL];
			cell = row
					.createCell((short) Recuperacion.Rec[Recuperacion.NivelLogro][Recuperacion.COL]);
			cell.setCellValue(f.getNivelLogro());
			// jerarquia de grupo
			numFila = Recuperacion.Rec[Recuperacion.JerarquiaGrupo][Recuperacion.FIL];
			cell = row
					.createCell((short) Recuperacion.Rec[Recuperacion.JerarquiaGrupo][Recuperacion.COL]);
			cell.setCellValue(f.getJerarquiagrupo());
			// jerarquia de logro
			numFila = Recuperacion.Rec[Recuperacion.JerarquiaLogro][Recuperacion.FIL];
			row = sheet.createRow(numFila);
			cell = row
					.createCell((short) Recuperacion.Rec[Recuperacion.JerarquiaLogro][Recuperacion.COL]);
			cell.setCellValue(f.getJerarquiaLogro());
		}
	}
	...
	public void setEncabezadoAreaDescriptorOculto(FiltroPlantilla f) {
		...
		if (f != null) {
			...
			cell.setCellValue(f.getTipoPlantilla());
			// plan estudios
			numFila = Area.Area[Area.PlanEstudios][Area.FIL];
			row = sheet.createRow(numFila);
			cell = row
					.createCell((short) Area.Area[Area.PlanEstudios][Area.COL]);
			cell.setCellValue(f.getFilPlanEstudios());
			// cerrado
			...
			cell = row
					.createCell((short) Area.Area[Area.JerarquiaGrupo][Area.COL]);
			cell.setCellValue(f.getJerarquiagrupo());
			// jerarquia de logro
			numFila = Area.Area[Area.JerarquiaArea][Area.FIL];
			row = sheet.createRow(numFila);
			cell = row
					.createCell((short) Area.Area[Area.JerarquiaArea][Area.COL]);
			cell.setCellValue(f.getJerarquiaLogro());
		}
	}
	...
	public void setEncabezadoPreescolarOculto(FiltroPlantilla f) {
		...
		if (f != null) {
			...
			row = sheet.createRow(numFila);
			cell = row
					.createCell((short) Preescolar.Pree[Preescolar.Tipo][Preescolar.COL]);
			cell.setCellValue(f.getTipoPlantilla());
			// cerrado
			numFila = Preescolar.Pree[Preescolar.Cerrado][Preescolar.FIL];
			cell = row
					.createCell((short) Preescolar.Pree[Preescolar.Cerrado][Preescolar.COL]);
			cell.setCellValue(f.getCerrar());
			// nivel de los logros
			numFila = Preescolar.Pree[Preescolar.NivelLogro][Preescolar.FIL];
			cell = row
					.createCell((short) Preescolar.Pree[Preescolar.NivelLogro][Preescolar.COL]);
			cell.setCellValue(f.getNivelLogro());
			// jerarquia de grupo
			numFila = Preescolar.Pree[Preescolar.JerarquiaGrupo][Preescolar.FIL];
			cell = row
					.createCell((short) Preescolar.Pree[Preescolar.JerarquiaGrupo][Preescolar.COL]);
			cell.setCellValue(f.getJerarquiagrupo());
		}
	}
...
//		System.out.println("setEstudiantesLogroAsignatura ");
		try {
			if (!c.isEmpty()) {
				...
				while (iterator.hasNext()) {
					...
					for (int j = 0; j < o.length - 1; j++) {
						...
						case 0:// codigo o consecutivo
							if (n == 0) {
								cell = row
										.createCell((short) Asignatura.Asig[Asignatura.Codigo][Asignatura.COL]);
						...
						case 2:// Apellidos
							cell = row
									.createCell((short) Asignatura.Asig[Asignatura.Apellido][Asignatura.COL]);
							cell.setCellValue((String) (o[j] != null ? o[j]
									: "")
									+ " "
									+ (o[j + 1] != null ? o[j + 1] : ""));
						...
						case 4:// nombres
							cell = row
									.createCell((short) Asignatura.Asig[Asignatura.Nombre][Asignatura.COL]);
							cell.setCellValue((String) (o[j] != null ? o[j]
									: "")
									+ " "
									+ (o[j + 1] != null ? o[j + 1] : ""));
			...
			while (iterator.hasNext()) {
				...
				for (int j = 0; j < o.length - 1; j++) {
					...
					case 0:// codigo o consecutivo
						if (n == 0) {
							cell = row
									.createCell((short) Recuperacion.Rec[Recuperacion.Codigo][Recuperacion.COL]);
							cell.setCellValue("" + (contador++));
						} else {
							cell = row
									.createCell((short) Recuperacion.Rec[Recuperacion.Codigo][Recuperacion.COL]);
					...
					case 2:// Apellidos
						cell = row
								.createCell((short) Recuperacion.Rec[Recuperacion.Apellido][Recuperacion.COL]);
						cell.setCellValue((String) (o[j] != null ? o[j] : "")
								+ " " + (o[j + 1] != null ? o[j + 1] : ""));
						break;
					case 4:// nombres
						cell = row
								.createCell((short) Recuperacion.Rec[Recuperacion.Nombre][Recuperacion.COL]);
						cell.setCellValue((String) (o[j] != null ? o[j] : "")
								+ " " + (o[j + 1] != null ? o[j + 1] : ""));
						break;
					}
			...
			while (iterator.hasNext()) {
				...
				for (int j = 0; j < o.length - 1; j++) {
					...
					case 0:// codigo o consecutivo
						if (n == 0) {
							cell = row
									.createCell((short) Area.Area[Area.Codigo][Area.COL]);
							cell.setCellValue("" + (contador++));
						} else {
							cell = row
									.createCell((short) Area.Area[Area.Codigo][Area.COL]);
					...
					case 2:// Apellidos
						cell = row
								.createCell((short) Area.Area[Area.Apellido][Area.COL]);
						cell.setCellValue((String) (o[j] != null ? o[j] : "")
								+ " " + (o[j + 1] != null ? o[j + 1] : ""));
						break;
					case 4:// nombres
						cell = row
								.createCell((short) Area.Area[Area.Nombre][Area.COL]);
						cell.setCellValue((String) (o[j] != null ? o[j] : "")
								+ " " + (o[j + 1] != null ? o[j + 1] : ""));
						break;
					}
	...
	public void setEstudiantesPreescolar(Collection c, int n) {
		if (!c.isEmpty()) {
			...
			// ENCABEZADO DATOS DE ESTUDIANTES
			row = sheet.createRow(numFila - 2);
			cell = row
					.createCell((short) Preescolar.Pree[Preescolar.Apellido][Preescolar.COL]);
			cell.setCellValue((String) Preescolar.labelDATOEST);
			if (cellstyleLABEL_GREEAN == null) {
//				System.out.println("es null cellstyleLABEL_GREEAN");
			} else {
//				System.out.println("no es null cellstyleLABEL_GREEAN");
			}
			cell.setCellStyle(cellstyleLABEL_GREEAN);
			...
			cell = row
					.createCell((short) Preescolar.Pree[Preescolar.Codigo][Preescolar.COL]);
			cell.setCellValue((String) Preescolar.labelCONSECUTIVO);
			cell.setCellStyle(cellstyleLABEL_GREEAN);

			cell = row
					.createCell((short) Preescolar.Pree[Preescolar.Nombre][Preescolar.COL]);
			cell.setCellValue((String) Preescolar.labelNOMBRE);
			cell.setCellStyle(cellstyleLABEL_GREEAN);

			cell = row
					.createCell((short) Preescolar.Pree[Preescolar.Apellido][Preescolar.COL]);
			cell.setCellValue((String) Preescolar.labelAPELLIDO);
			cell.setCellStyle(cellstyleLABEL_GREEAN);
	...
	public void setLogros(Collection c, int n) {
		try {
			if (!c.isEmpty()) {
				...
				int numCelda = Asignatura.Asig[Asignatura.Logros2][Asignatura.COL] + 1;
				iterator = c.iterator();
				while (iterator.hasNext()) {
					row = sheet.getRow(numFila);
					if (row == null)
					...
					cell = row.getCell((short) numCelda++);
					if (cell == null)
						cell = row.createCell((short) (numCelda - 1));
	 ...
	 *            Version 1.1.<br>
	 */
	public void setNotasAsignatura(Collection c1, Collection c4) {
		String not, not2, not3, recu3;
		try {
			if (!c1.isEmpty()) {
				...
				while (iterator.hasNext()) {
					...
					if (!c4.isEmpty()) {
						...
						while (iterator2.hasNext()) {
							...
							not3 = (String) o[2];// ausencia
							recu3 = (String) o[3];// ausencia
							if (not2 != null) {
								if (not.equals(id)) {
									cell = row
											.createCell((short) Asignatura.Asig[Asignatura.Evaluacion][Asignatura.COL]);// nota
									cell.setCellValue((String) not2);

									cell = row
											.createCell((short) Asignatura.Asig[Asignatura.Recuperacion][Asignatura.COL]);// ausencia
									cell.setCellValue((String) recu3);

									cell = row
											.createCell((short) Asignatura.Asig[Asignatura.AusenciaJus][Asignatura.COL]);// ausencia
									cell.setCellValue((String) not3);
	...
	/*
	 * public void setNotasAsignatura(Collection c1, Collection c3, Collection
	 * c4) { String not; String not2; String vlr; String vlr2; if
	 * (!c1.isEmpty()) { int numFila =
	 * Asignatura.Asig[Asignatura.Evaluacion][Asignatura.FIL]; iterator =
	 * c1.iterator(); while (iterator.hasNext()) { row =
	 * sheet.createRow(numFila++); String id = (String) ((Object[])
	 * iterator.next())[0]; if (!c4.isEmpty()) { iterator2 = c4.iterator();
	 * while (iterator2.hasNext()) { o = ((Object[]) iterator2.next()); not =
	 * (String) o[0];// id Est not2 = (String) o[1];// nota if (!c3.isEmpty()) {
	 * iterator3 = c3.iterator(); while (iterator3.hasNext()) { o = ((Object[])
	 * iterator3.next()); vlr = (String) o[0];// codigo nota vlr2 = (String)
	 * o[1];// nombre nota if (not2 != null) { if (not.equals(id) &&
	 * not2.equals(vlr)) { cell = row.createCell((short)
	 * Asignatura.Asig[Asignatura.Evaluacion][Asignatura.COL]);// nota
	 * cell.setCellValue((String) vlr2); break; } } } } } } } } }
	 */
	/**
	 * @param c1
	 * @param c2
	 ...
	 *            Version 1.1.<br>
	 */
	public void setNotasArea(Collection c1, Collection c4) {
		String not;
		String not2;
		String not3;
		String recu3;
			...
			while (iterator.hasNext()) {
				...
				if (!c4.isEmpty()) {
					...
					while (iterator2.hasNext()) {
						...
						not2 = (String) o[1];// nota
						not3 = (String) o[2];// ausencia
						recu3 = (String) o[3];// recuperacion
						if (not2 != null) {
							if (not.equals(id)) {
								cell = row
										.createCell((short) Area.Area[Area.Evaluacion][Area.COL]);// nota
								cell.setCellValue((String) not2);
								cell = row
										.createCell((short) Area.Area[Area.Recuperacion][Area.COL]);// nota
								cell.setCellValue((String) recu3);
								cell = row
										.createCell((short) Area.Area[Area.AusenciaJus][Area.COL]);// nota
								cell.setCellValue((String) not3);
								break;
							}
						}
	...
	public void setNotasPreescolar(Collection c1, Collection c3, String tipo) {
		...
		if (!c1.isEmpty()) {
			...
			cell = row
					.createCell((short) Preescolar.Pree[Preescolar.Evaluacion][Preescolar.COL]);
			cell.setCellValue((String) Preescolar.labelDATO_EVAL);
			cell.setCellStyle(cellstyleLABEL_GREEAN);

			cell = row
					.createCell((short) Preescolar.Pree[Preescolar.EvaluacionConceptual][Preescolar.COL]);
			cell.setCellValue((String) Preescolar.labelDATO_FIJAR_EVAL);
			cell.setCellStyle(cellstyleLABEL_GREEAN);
				...
				if (!c3.isEmpty()) {
					...
					while (iterator2.hasNext()) {
						...
						if (est.equals(id) && dim.equals(tipo)) {
							cell = row
									.createCell((short) Preescolar.Pree[Preescolar.Evaluacion][Preescolar.COL]);
							cell.setCellValue(ev);
							cell = row
									.createCell((short) Preescolar.Pree[Preescolar.EvaluacionConceptual][Preescolar.COL]);
							cell.setCellValue(evCon);
						}
					}
				}
			}
	...
	/**
	 * Funcion que define los respectivos estilos
	 * 
	 */
	public void pintarColores() {
		...
		cellstyleGREEN.setFont(cellfont);
		cellstyleGREEN.setBorderBottom(cellstyle1.BORDER_THIN);
		cellstyleGREEN.setFillForegroundColor(HSSFColor.WHITE.index);
		cellstyleGREEN.setFillForegroundColor(HSSFColor.WHITE.index);
		cellstyleGREEN.setFillPattern(HSSFCellStyle.SOLID_FOREGROUND);

		cellstyleBLUE = workbook.createCellStyle();
		/*
		 * cellstyleBLUE.setBorderBottom(cellstyle1.BORDER_THIN);
		 * cellstyleBLUE.setBorderLeft(cellstyle1.BORDER_THIN);
		 * cellstyleBLUE.setBorderRight(cellstyle1.BORDER_THIN);
		 * cellstyleBLUE.setBorderTop(cellstyle1.BORDER_THIN);
		 * cellstyleBLUE.setFillForegroundColor(HSSFColor.WHITE.index);
		 * cellstyleBLUE
		 * .setFillForegroundColor(HSSFColor.LIGHT_TURQUOISE.index);
		...
		cellstyleLABEL_GREEAN = workbook.createCellStyle();

		cellstyleLABEL_GREEAN.setBorderBottom(cellstyle1.BORDER_THIN);
		cellstyleLABEL_GREEAN.setBorderLeft(cellstyle1.BORDER_THIN);
		cellstyleLABEL_GREEAN.setBorderRight(cellstyle1.BORDER_THIN);
		cellstyleLABEL_GREEAN.setBorderTop(cellstyle1.BORDER_THIN);
		cellstyleLABEL_GREEAN.setFillForegroundColor(HSSFColor.WHITE.index);
		cellstyleLABEL_GREEAN
				.setFillForegroundColor(HSSFColor.LIGHT_TURQUOISE.index);
		cellstyleLABEL_GREEAN.setFillPattern(HSSFCellStyle.SOLID_FOREGROUND);
		cellstyleLABEL_GREEAN.setFont(cellfont);

		// cellstyleYELLOW
		cellstyleYELLOW = workbook.createCellStyle();
		cellstyleYELLOW.setBorderBottom(cellstyle1.BORDER_THIN);
		cellstyleYELLOW.setBorderBottom(cellstyle1.BORDER_THIN);
		cellstyleYELLOW.setBorderLeft(cellstyle1.BORDER_THIN);
		cellstyleYELLOW.setBorderRight(cellstyle1.BORDER_THIN);
		cellstyleYELLOW.setBorderTop(cellstyle1.BORDER_THIN);

		cellstyleYELLOW.setFillForegroundColor(HSSFColor.WHITE.index);
		...
		cellstyleYELLOW.setFont(cellfont);
		cellstyleYELLOW.setWrapText(false);
