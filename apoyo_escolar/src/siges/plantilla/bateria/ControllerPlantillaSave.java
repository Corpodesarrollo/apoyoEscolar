 ...
 * Funcinn: Procesa la solicitud de plantillas y llama al servicio que la genera <br>
 */
public class ControllerPlantillaSave extends HttpServlet {
	...
	public synchronized String process(HttpServletRequest request,
		...
		if (boton.equals("Buscar")) {
			// String alert =
			// "Las plantillas se estan generando en este momento. \nPulse en el hipervinculo 'Listado de Reportes' para ver si el reporte ya fue generado. \nO vaya a la opción de menu 'Reportes generados'";
			// FiltroPlantilla filtro = (FiltroPlantilla)
			// filtroPlantilla.clone();
			// Login log = (Login) login.clone();
			ubicacion = getUbicacionZip(login.getUsuarioId(), f2, tipo,
					ubicacion);
			ubicacionPlantilla = getUbicacionPlantilla(login.getUsuarioId(),
					tipo, ubicacionPlantilla);
			switch (tipo) {
			case Properties.PLANTILLABATLOGRO:// bateria de logros
				asignarListaBateriaLogro(request, login, logros, tipo,
						ubicacion);
				request.setAttribute("listaDocente",
						(List) session.getAttribute("sessionDocente"));
				return (sig4 += "?tipo=" + tipo);
			case Properties.PLANTILLABATDESCRIPTOR:// bateria de descriptor
				asignarListaBateriaDescriptor(request, login, logros, tipo,
						ubicacion);
				request.setAttribute("listaDocente",
						(List) session.getAttribute("sessionDocente"));
	 ...
	 *             Version 1.1.<br>
	 */
	public void asignarListaBateriaLogro(HttpServletRequest request,
			Login login, Logros logros, int tipo, String[] ubicacion)
			throws ServletException, IOException {
		String asig;
		...
		Logros item = null;
		try {
			if (logros.getPlantillaDocente() != 0
					&& logros.getPlantillaDocente() != -99) {
				logros.setPlantillaDocente_(plantillaDAO
						.getNombreDocente(logros.getPlantillaDocente()));
			}else{
				
				if(login.getPerfil().equals("422")){
					
					logros.setPlantillaDocente_(login.getUsuario());
				}
				
			}
			// periodos
			// col[0] = Recursos.recursoEstatico[Recursos.PERIODO];
			col[0] = getListaPeriodo(login.getLogNumPer(),
					login.getLogNomPerDef());

			/* todo fue escogido en los combos */
			if (!logros.getPlantillaMetodologia().equals("-99")
					&& !logros.getPlantillaGrado().equals("-99")
					&& !logros.getPlantillaAsignatura().equals("-99")) {
				asig = logros.getPlantillaAsignatura().substring(
						logros.getPlantillaAsignatura().lastIndexOf("|") + 1,
						logros.getPlantillaAsignatura().length());
				request.setAttribute(
						"plantilla",
						plantillaBateriaLogroDesc(asig, login, logros, tipo,
								col));
				request.setAttribute("tipoArchivo", "zip");
				Logger.print(
						login.getUsuarioId(),
						"Bateria Logro Inst:"
								+ logros.getPlantillaInstitucion() + " Metod:"
								+ logros.getPlantillaMetodologia() + " Grado:"
								+ logros.getPlantillaGrado() + " Asig:" + asig,
						"Generacinn de Plantilla de Bateria de Logros: Metodologia '"
								+ logros.getPlantillaMetodologia_()
								+ "', Grado '" + logros.getPlantillaGrado_()
								+ "', Asignatura '"
								+ logros.getPlantillaAsignatura_() + "'", 3, 1,
						this.toString());
				return;
			}
			...
			if (!logros.getPlantillaMetodologia().equals("-99")
					&& !logros.getPlantillaGrado().equals("-99")
					&& logros.getPlantillaAsignatura().equals("-99")) {
				if (logros.getEstadoPlanEstudios() == 0) {// es plan de estudios
															// unificado
					lista = plantillaDAO.getListaAsignatura(
							Long.parseLong(logros.getPlantillaInstitucion()),
							Integer.parseInt(logros.getPlantillaMetodologia()),
							logros.getPlantillaVigencia(),
							Integer.parseInt(logros.getPlantillaGrado()));
				} else {// es plan de estudios por docente
					lista = plantillaDAO.getListaDocenteAsignatura(
							Long.parseLong(logros.getPlantillaInstitucion()),
							Integer.parseInt(logros.getPlantillaMetodologia()),
							logros.getPlantillaVigencia(),
							Integer.parseInt(logros.getPlantillaGrado()));
				...
				if (lista.size() > 0) {
					for (int i = 0; i < lista.size(); i++) {
						...
						logros.setPlantillaAsignatura(item
								.getPlantillaAsignatura());
						logros.setPlantillaAsignatura_(item
								.getPlantillaAsignatura_());
						if (logros.getEstadoPlanEstudios() == 1) {
							logros.setPlantillaDocente(item
									.getPlantillaDocente());
							logros.setPlantillaDocente_(item
									.getPlantillaDocente_());
						}
						files.add(plantillaBateriaZip(
								item.getPlantillaAsignatura(), login, logros,
								tipo, col));
					}
					request.setAttribute(
							"plantilla",
							ponerZip(files, login.getUsuarioId(), ubicacion,
									logros, tipo));
					request.setAttribute("tipoArchivo", "zip");
					Logger.print(login.getUsuarioId(), "Bateria Logro Inst:"
							+ logros.getPlantillaInstitucion() + " Metod:"
							+ logros.getPlantillaMetodologia() + " Grado:"
							+ logros.getPlantillaGrado(),
							"Generacinn de Plantilla de Bateria de Logros:", 3,
							1, this.toString());
				} else {
					request.setAttribute(
							"mensaje",
							setMensaje("No se pudo generar la plantilla de Excel.\nPorque no se ha establecido el plan de estudios.\nHasta que no se ingrese esta información no sera posible generar archivos"));
			...
			if (!logros.getPlantillaMetodologia().equals("-99")
				...
				if (logros.getEstadoPlanEstudios() == 0) {// es plan de estudios
															// unificado
					lista = plantillaDAO.getListaAsignaturaGrado(
							Long.parseLong(logros.getPlantillaInstitucion()),
							Integer.parseInt(logros.getPlantillaMetodologia()),
							logros.getPlantillaVigencia());
				} else {// es plan de estudios por docente
					lista = plantillaDAO.getListaDocenteAsignaturaGrado(
							Long.parseLong(logros.getPlantillaInstitucion()),
							Integer.parseInt(logros.getPlantillaMetodologia()),
							logros.getPlantillaVigencia());
				}
	 ...
	 *             Version 1.1.<br>
	 */
	public void asignarListaBateriaDescriptor(HttpServletRequest request,
			Login login, Logros logros, int tipo, String[] ubicacion)
			throws ServletException, IOException {
		String asig;
		Collection files = new ArrayList();
		...
		try {
			...
			if (!logros.getPlantillaMetodologia().equals("-99")
				...
				Logger.print(
						login.getUsuarioId(),
						"Bateria Descriptor Inst:"
								+ logros.getPlantillaInstitucion() + " Met:"
								+ logros.getPlantillaMetodologia() + " Gra:"
								+ logros.getPlantillaGrado() + " Area:" + asig,
						"Generacinn de Plantilla de Bateria de Descriptores: Metodologia '"
								+ logros.getPlantillaMetodologia_()
								+ "', Grado '" + logros.getPlantillaGrado_()
								+ "', Area '"
								+ logros.getPlantillaAsignatura_() + "'", 3, 1,
			...
			if (!logros.getPlantillaMetodologia().equals("-99")
				...
				if (logros.getEstadoPlanEstudios() == 0) {// es plan de estudios
															// unificado
					lista = plantillaDAO.getListaArea(
							Long.parseLong(logros.getPlantillaInstitucion()),
							Integer.parseInt(logros.getPlantillaMetodologia()),
							logros.getPlantillaVigencia(),
							Integer.parseInt(logros.getPlantillaGrado()));
				} else {// es plan de estudios por docente
					lista = plantillaDAO.getListaDocenteArea(
							Long.parseLong(logros.getPlantillaInstitucion()),
							Integer.parseInt(logros.getPlantillaMetodologia()),
							logros.getPlantillaVigencia(),
							Integer.parseInt(logros.getPlantillaGrado()));
				...
				if (lista.size() > 0) {
					...
					Logger.print(
							...
							"Bateria Descriptor Inst:"
									...
									+ " Gra:" + logros.getPlantillaGrado(),
							"Generacinn de Plantilla de Bateria de Descriptores:",
							3, 1, this.toString());
				} else {
					request.setAttribute(
							"mensaje",
				...
				return;
				/*
				 * list = new ArrayList(); o = new Object[2];// institucion o[0]
				 * = enterolargo; o[1] = login.getInstId(); list.add(o); o = new
				 * Object[2];// metodologia o[0] = entero; o[1] =
				 * logros.getPlantillaMetodologia(); list.add(o); o = new
				 * Object[2];// grado o[0] = entero; o[1] =
				 * logros.getPlantillaGrado(); list.add(o); o = new Object[2];//
				 * Vigencia o[0] = entero; o[1] = plantillaDAO.getVigencia();
				 * list.add(o); String[][] a =
				 * plantillaDAO.getFiltroMatriz(rb.getString
				 * ("listaPlantillaDescriptor1"), list); if (a != null) { for
				 * (int i = 0; i < a.length; i++) {
				 * logros.setPlantillaAsignatura(a[i][0]);
				 * logros.setPlantillaAsignatura_(a[i][1]); asig =
				 * logros.getPlantillaAsignatura();
				 * files.add(plantillaBateriaZip(asig, login, logros, tipo,
				 * col)); } request.setAttribute("plantilla", ponerZip(files,
				 * login.getUsuarioId(), ubicacion, logros, tipo));
				 * request.setAttribute("tipoArchivo", "zip");
				 * Logger.print(login.getUsuarioId(), "Bateria Descriptor Inst:"
				 * + logros.getPlantillaInstitucion() + " Met:" +
				 * logros.getPlantillaMetodologia() + " Gra:" +
				 * logros.getPlantillaGrado(),
				 * "Generacinn de Plantilla de Bateria de Descriptores:", 3, 1,
				 * this.toString()); } else { request.setAttribute("mensaje",
				 * setMensaje(
				 * "No se pudo generar la plantilla de Excel.\nPorque no se ha establecido el plan de estudios.\nHasta que no se ingrese esta información no sera posible generar archivos"
				 * )); request.setAttribute("plantilla", "--"); } return;
				 */
			}
			/* no selecciono grado y asignatura */
			if (!logros.getPlantillaMetodologia().equals("-99")
				...
				if (logros.getEstadoPlanEstudios() == 0) {// es plan de estudios
															// unificado
					lista = plantillaDAO.getListaAreaGrado(
							Long.parseLong(logros.getPlantillaInstitucion()),
							Integer.parseInt(logros.getPlantillaMetodologia()),
							logros.getPlantillaVigencia());
				} else {// es plan de estudios por docente
					lista = plantillaDAO.getListaDocenteAreaGrado(
							Long.parseLong(logros.getPlantillaInstitucion()),
							Integer.parseInt(logros.getPlantillaMetodologia()),
							logros.getPlantillaVigencia());
				}
				 ...
				 * Object[2];// metodologia o[0] = entero; o[1] =
				 * logros.getPlantillaMetodologia(); list.add(o); o = new
				 * Object[2];// Vigencia o[0] = entero; o[1] =
				 * plantillaDAO.getVigencia(); list.add(o); String[][] a =
				 * plantillaDAO
				 * .getFiltroMatriz(rb.getString("listaPlantillaDescriptor2"),
				 * list); if (a != null) { for (int i = 0; i < a.length; i++) {
				 * logros.setPlantillaGrado(a[i][0]);
				 * logros.setPlantillaGrado_(a[i][1]);
				 * logros.setPlantillaAsignatura(a[i][2]);
				 * logros.setPlantillaAsignatura_(a[i][3]); asig =
				 * logros.getPlantillaAsignatura();
				 * files.add(plantillaBateriaZip(asig, login, logros, tipo,
				 * col)); } request.setAttribute("plantilla", ponerZip(files,
				 * login.getUsuarioId(), ubicacion, logros, tipo));
			 ...
			 * if (logros.getPlantillaMetodologia().equals("-99") &&
			 * logros.getPlantillaGrado().equals("-99") &&
			 * logros.getPlantillaAsignatura().equals("-99")) { // traer la
			 * lista de metodologias, grados y asignaturas para esa //
			 * institucion list = new ArrayList(); o = new Object[2];//
			 * institucion o[0] = enterolargo; o[1] = login.getInstId();
			 * list.add(o); o = new Object[2];// Vigencia o[0] = entero; o[1] =
			 * plantillaDAO.getVigencia(); list.add(o); String[][] a =
			 * plantillaDAO
			 * .getFiltroMatriz(rb.getString("listaPlantillaDescriptor3"),
			 * list); if (a != null) { for (int i = 0; i < a.length; i++) {
			 ...
			 * logros.setPlantillaAsignatura_(a[i][5]); asig =
			 * logros.getPlantillaAsignatura();
			 * files.add(plantillaBateriaZip(asig, login, logros, tipo, col)); }
			 * request.setAttribute("plantilla", ponerZip(files,
			 * login.getUsuarioId(), ubicacion, logros, tipo));
			 * request.setAttribute("tipoArchivo", "zip");
			 * Logger.print(login.getUsuarioId(), "Bateria Descriptor Inst:" +
			 * logros.getPlantillaInstitucion() + " Met:" +
			...
			System.out
					.println("PLANTILLA Bateria Descriptor con errores:" + th);
			throw new ServletException(th);
		}
	}
	...
	public String[] getUbicacionPlantilla(String id, int tipo,
		...
		case Properties.PLANTILLABATLOGRO:
			path = rb.getString("plantillaLogro.PathPlantillaLogro") + "." + id;
			relativo = Ruta.getRelativo(getServletContext(), path);
			ubicacionPlantilla[i++] = Ruta2.get(getServletContext(),
					rb.getString("plantillaLogro.PathPlantilla"));// path de la
																	// plantilla
			ubicacionPlantilla[i++] = rb.getString("plantillaLogro.Plantilla");// nombre
																				// de
																				// la
																				// plantilla
			ubicacionPlantilla[i++] = Ruta.get(getServletContext(), path);// path
																			// del
																			// nuevo
																			// archivo
		...
		case Properties.PLANTILLABATDESCRIPTOR:
			...
			relativo = Ruta.getRelativo(getServletContext(), path);
			ubicacionPlantilla[i++] = Ruta2.get(getServletContext(),
					rb.getString("plantillaDescriptor.PathPlantilla"));// path
																		// de la
																		// plantilla
			ubicacionPlantilla[i++] = rb
					.getString("plantillaDescriptor.Plantilla");// nombre de la
																// plantilla
			ubicacionPlantilla[i++] = Ruta.get(getServletContext(), path);// path
																			// del
																			// nuevo
	 ...
	 *             Version 1.1.<br>
	 */
	public String ponerZip(Collection list, String id, String[] ubi,
			Logros logros, int tipo) {
		Zip zip = new Zip();
		String path;
		String archivo;
		String relativo;
		...
		case Properties.PLANTILLABATLOGRO:
			...
			if (zip.ponerZip(ruta, archivo, zise, list)) {
				...
				ponerReporte(
						...
						ubi[1] + ubi[0],
						"zip",
						"Plantillas de Importacinn de logros "
								+ f2.toString().replace(':', '-')
										.replace('.', '-').substring(0, 10)
								+ "_["
								+ f2.toString().replace(':', '-')
										.replace('.', '-').substring(11, 19)
								+ "]", 5);
		...
		case Properties.PLANTILLABATDESCRIPTOR:
			...
			if (zip.ponerZip(ruta, archivo, zise, list)) {
				ponerReporte(
						...
						ubi[1] + ubi[0],
						"zip",
						"Plantillas de Importacinn de Descriptores"
								+ f2.toString().replace(':', '-')
										.replace('.', '-').substring(0, 10)
								+ "_["
								+ f2.toString().replace(':', '-')
	 ...
	 *             Version 1.1.<br>
	 */
	public String plantillaBateriaLogroDesc(String asig, Login login,
			Logros logros, int tipo, Collection[] col) throws Exception {
		...
		case Properties.PLANTILLABATLOGRO:
			...
			archivo = getNombre("PlantillaLogro",
					logros.getPlantillaMetodologia(),
					logros.getPlantillaGrado(),
					logros.getPlantillaAsignatura_(),
					logros.getPlantillaDocente(),
			...
			config[i++] = Ruta2.get(getServletContext(),
					rb.getString("plantillaLogro.PathPlantilla"));// path de la
																	// plantilla
			config[i++] = rb.getString("plantillaLogro.Plantilla");// nombre de
																	// la
																	// plantilla
			...
			ponerReporte(login.getUsuarioId(), relativo + config[3], "xls",
					config[3], 5);
			break;
		case Properties.PLANTILLABATDESCRIPTOR:
			...
			path = rb.getString("plantillaDescriptor.PathPlantillaDescriptor")
					+ "." + login.getUsuarioId();
			/* calcular el nombre del archivo */
			archivo = getNombre("PlantillaDescriptor",
					logros.getPlantillaMetodologia(),
					logros.getPlantillaGrado(),
					logros.getPlantillaAsignatura_(),
					logros.getPlantillaDocente(),
			...
			config[i++] = Ruta2.get(getServletContext(),
					rb.getString("plantillaDescriptor.PathPlantilla"));// path
																		// de la
																		// plantilla
			config[i++] = rb.getString("plantillaDescriptor.Plantilla");// nombre
																		// de la
																		// plantilla
			config[i++] = Ruta.get(getServletContext(), path);// path del nuevo
																// archivo
	...
	public String getNombre(String ini, String meto, String grad, String asig,
		...
		String fe = f2.toString().replace(':', '-').replace('.', '-')
				...
				+ f2.toString().replace(':', '-').replace('.', '-')
						.substring(11, 19);
		if (estado == 0)
			return (ini + "_Met_" + met + "_Gra_" + gra + "_" + asi + "_(" + fe
					+ ")[" + (int) (Math.random() * 1000) + "].xls");
		else
			return (ini + "_Met_" + met + "_Gra_" + gra + "_" + asi + "_Doc_"
					+ doc + "(" + fe + ")[" + (int) (Math.random() * 1000) + "].xls");
	}
	 ...
	 *             Version 1.1.<br>
	 */
	public String plantillaBateriaZip(String asig, Login login, Logros logros,
			int tipo, Collection[] col) {
		Excel excel = new Excel();
		String relativo = "";
		String path = "", archivo = "";
		logros.setPlantillaAsignatura(asig);
	 ...
	 *            Version 1.1.<br>
	 */
	public void ponerReporteZip(String us, String rec, String tipo,
	 ...
	 *            Version 1.1.<br>
	 */
	public void actualizarReporte(String us, String rec, int estado,
			String mensaje, Cursor c) {
		Collection list;
		Object[] o;
		list = new ArrayList();
		o = new Object[2];
	...
	public Collection getListaPeriodo(long numPer, String nomPerDef) {
		...
		/*
		 * Object[] o=new Object[2]; o[0]= ""+(7); o[1]=nomPerDef;
		 * collection.add(o);
		 */
