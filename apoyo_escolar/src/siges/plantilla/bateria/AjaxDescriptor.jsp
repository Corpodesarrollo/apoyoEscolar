...
<!--
	...
	<c:choose>
	  <c:when test="${requestScope.ajaxParam==paramsVO.CMD_AJAX_DOCENTE}">
	  		borrar_combo(parent.document.frmNuevo.plantillaDocente);
		  	<c:forEach begin="0" items="${requestScope.listaDocente}" var="doc" varStatus="st">
					parent.document.frmNuevo.plantillaDocente.options[<c:out value="${st.count}"/>] = new Option('<c:out value="${doc.nombre}"/>','<c:out value="${doc.codigo}"/>');
				</c:forEach>
		</c:when>
