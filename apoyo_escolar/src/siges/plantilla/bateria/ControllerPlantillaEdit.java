 ...
 * Funcinn: Recibe la solicitud de filto de plantillas y llena los objetos que
 * mostrar la vista de formulario <br>
 */
public class ControllerPlantillaEdit extends HttpServlet {
	...
	public String process(HttpServletRequest request,
		...
		// System.out.println("EN PLANTILLA INDICADORES "
	...
	/**
	 * Pone en session los datos necesarios para la vista del formulario de
	 * generacinn de plantilla de bateria de logros<BR>
	 * 
	 * @param request
	 * @throws ServletException
	 * @throws IOException
	 ...
	 *             Version 1.1.<br>
	 */
	public void editarPlantillaLogro(HttpServletRequest request, Login login,
			Logros logros) throws ServletException, IOException {
		HttpSession session = request.getSession();
		Collection list;
		Object[] o;
		try {
			if (logros == null) {
				...
				logros.setPlantillaInstitucion_(login.getInstId());
				// logros.setPlantillaMetodologia(login.getMetodologiaId());
				// logros.setPlantillaMetodologia_(login.getMetodologia());
				logros.setPlantillaVigencia((int) plantillaDAO
						.getVigenciaInst(Long.parseLong(login.getInstId())));
				logros.setEstadoPlanEstudios(plantillaDAO.getEstadoPlan(Long
			...
			if (logros.getPlantillaAsignatura() != null
					&& !logros.getPlantillaAsignatura().equals("")
					&& !logros.getPlantillaAsignatura().equals("-99")) {
				String[] val = logros.getPlantillaAsignatura()
						.replace('|', ':').split(":");
				if (val != null && val.length == 3) {
					...
					request.setAttribute(
							...
							plantillaDAO.getListaDocenteAsignatura(
									Long.parseLong(logros
											.getPlantillaInstitucion()),
									Integer.parseInt(logros
											.getPlantillaMetodologia()), logros
											.getPlantillaVigencia(), Integer
											.parseInt(logros
													.getPlantillaGrado()), asig));
				}
			}
			session.removeAttribute("editar");
			...
			if (request.getAttribute("filtroGrado2") == null) {
				c = plantillaDAO.getFiltro(
						rb.getString("filtroGradoInstitucion"), list);
			...
			if (request.getAttribute("filtroGradoAsignatura2") == null) {
				c = plantillaDAO
						.getFiltro(
								rb.getString("filtroAsignaturaMetodologiaGradoInstitucion"),
								list);
	 ...
	 *             Version 1.1.<br>
	 */
	public void editarPlantillaDescriptor(HttpServletRequest request,
			Login login, Logros logros) throws ServletException, IOException {
		HttpSession session = request.getSession();
		Collection list;
		Object[] o;
		try {
			if (logros == null) {
				...
				logros.setPlantillaMetodologia_(login.getMetodologia());
				logros.setPlantillaVigencia((int) plantillaDAO
						.getVigenciaInst(Long.parseLong(login.getUsuarioId())));
				logros.setEstadoPlanEstudios(plantillaDAO.getEstadoPlan(Long
						.parseLong(login.getInstId())));
				session.setAttribute("logros", logros);
			}
			...
			// metodologias por institucion
			if (request.getAttribute("filtroMetodologia") == null)
				request.setAttribute("filtroMetodologia", plantillaDAO
						.getFiltro(rb.getString("listaMetodologiaInstitucion"),
								list));
			// grados por institucion
			if (request.getAttribute("filtroGrado2") == null)
				request.setAttribute(
						"filtroGrado2",
						plantillaDAO.getFiltro(
								rb.getString("filtroGradoInstitucion"), list));
			// areas por grado
			o = new Object[2];
			o[0] = Properties.ENTEROLARGO;
			...
			if (request.getAttribute("filtroGradoArea") == null) {
				request.setAttribute("filtroGradoArea", plantillaDAO.getFiltro(
						rb.getString("filtroAreaGradoInstitucion"), list));
			}
		} catch (Exception th) {
			throw new ServletException(th);
		}
