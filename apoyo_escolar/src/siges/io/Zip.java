...
public class Zip {
	...
	public boolean ponerZip(String ruta, String archivo, int BUFFER_SIZE, Collection files) {
		...
		try {
			...
			for (Iterator i = files.iterator(); i.hasNext();) {// Iterate over
																...
																// files in our
																// list
				filename = (String) i.next();// Get a BufferedInputStream
												// that we can use to read the
												// source file
	...
	public static void hacerZip(String ruta, String archivo, int BUFFER_SIZE, Collection files) throws Exception {
		String filename;
		int count;
		ZipOutputStream out = null;
		FileOutputStream dest = null;
		...
		try {
			...
			try {
				...
				System.out.println("Error Borrando archivos en Finally Zip" + ex);
			}
		}
	}
}
