    ...
    private String mensaje;//mensaje en caso de error
   	...
   	**/
   		public String process(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
   		  ...
   		  String sig="/conflictoGrupo/FiltroConflicto.jsp";
   		  String sig2="/conflictoGrupo/NuevoConflictoGrupo.jsp";
   		  String p_inte=this.getServletContext().getInitParameter("integrador");
	    		...
	    		try{
	    		    ...
	    		    //filtro
	    		    case 0:
	    		        cf=conflictoDAO.obtenerDirectorGrupo(login.getUsuarioId());
	    		        cargarConflicto(request,session,login);
	    		        if(cf!=null){
	    		            request.setAttribute("readonly","2");
   		    ...
   		    try{
   		        int z=0;
   		  		    ...
   		  		    request.setAttribute("filtroSedeF",conflictoDAO.getFiltro(rb.getString("filtroSedeInstitucion"),list));
   	  	  				...
   	  	  				request.setAttribute("filtroMetodologiaF",conflictoDAO.getFiltro(rb.getString("listaMetodologiaInstitucion"),list));		
   	  	  				request.setAttribute("filtroGradoF2",conflictoDAO.getFiltro(rb.getString("filtroSedeJornadaGradoInstitucion2"),list));
   	  	  				request.setAttribute("filtroGrupoF",conflictoDAO.getFiltro(rb.getString("filtroSedeJornadaGradoGrupoInstitucion"),list));
   		    }catch(Throwable th){
   		        th.printStackTrace();
