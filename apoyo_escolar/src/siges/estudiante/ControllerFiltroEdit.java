...
public class ControllerFiltroEdit extends HttpServlet {
	...
	public String process(HttpServletRequest request,
		...
		try {
			...
			Collection col = estudianteDAO.getFiltro(rb.getString("filtroSedeInstitucion"), list);
			request.getSession().setAttribute("filtroSedeF", col);
			//request.setAttribute("filtroJornadaF",estudianteDAO.getFiltro(rb.getString("filtroSedeJornadaInstitucion"), list));
			request.getSession().setAttribute("filtroJornadaF",estudianteDAO.getFiltro(rb.getString("filtroSedeJornadaInstitucion"), list));
			request.setAttribute("filtroMetodologiaF",estudianteDAO.getFiltro(rb.getString("listaMetodologiaInstitucion"), list));
			request.setAttribute("filtroGradoF2", estudianteDAO.getFiltro(rb.getString("filtroSedeJornadaGradoInstitucion2"), list));
			request.setAttribute("filtroGrupoF", estudianteDAO.getFiltro(rb.getString("filtroSedeJornadaGradoGrupoInstitucion"),list));
			Gson gson = new Gson();
			...
			try {
				...
				Logger.print(login.getUsuarioId(), "HOJAS DE VIDA (Bitacora), módulo de Adicionar Estudiante. ",
						7, 1, this.toString());
			}
		} catch (Exception e) {
			System.out.println("Error " + this + ":" + e.toString());
