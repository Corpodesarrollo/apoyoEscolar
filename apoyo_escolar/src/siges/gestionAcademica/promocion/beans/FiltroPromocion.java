...
*	@version v 1.0	<BR>
*/
public class FiltroPromocion implements Cloneable{
 ...
 */
	...
	/**
	 * asigna el campo fecha_estado
	 *	@param String s
	 */
		public void setfecha_estado(String[] s){
			this.fecha_estado=s;
		}	

	/**
	 * retorna el campo fecha_estado
	 *	@return String
	 */
...
public String seleccionPromocion(String s,int n){
	...
	try{
		...
		try{
			...
			if(band == 0 && ("|-1").equals(s.substring(s.lastIndexOf("|"),  s.length())) && ("|4").equals(s.substring(s.lastIndexOf("|"),  s.length()))){
				return "SELECTED";
			}
		}catch (Exception e) {
		...
		//	System.out.println("longitud de motivo"+motivo.length);
			for(int i=0;i<(motivo!=null?motivo.length:0);i++){
				//System.out.println("motivo :"+motivo[i]);
		...
		case 3: 
			for(int i=0;i<(semestres!=null?semestres.length:0);i++){
				if((""+n).equals(s)){
					return "CHECKED";
				}
			}
			break;
		case 4: 
				//System.out.println("longitud de fecha "+fecha_estado.length);
				for(int i=0;i<(fecha_estado!=null?fecha_estado.length:0);i++){
					//System.out.println("fecha : "+fecha_estado[i]);
					b=fecha_estado[i].replace('|',':').split(":");		
		...
		System.out.println(new Date() + ":: PROMOCIO " + e.getMessage());
		e.printStackTrace();
	}
	return "";
}
 ...
 */
	...
	public void setSemestres(String[] semestres_) {
		this.semestres = semestres_;
	}
	...
	public void setGradoNom(String gradoNom) {
		this.gradoNom = gradoNom;
	}
}