...
//      2.0		14/09/2023	LINKTIC			Se genera copia de la clase Acceso, y se modifica los metodos para recibir password md5 ya generado
...
public class AccesoPaginas {
	...
	static public void Cargar() {
		...
		String[] p = { rb.getString("loginUsuario1"), // 0
	...
	static public String[][] autorizado(String a, String b) throws Exception {
		...
		try {
			...
			pst = cn.prepareStatement(con);
			pst.setString(1, a);
			pst.setString(2, b); // Cambio 2.0 - Se retira metodo md5 ya que llega la contraseña en md5
			rs = pst.executeQuery();
	 ...
	 * @throws Exception
	 */
	static public String[][] autorizadoEstudiante(String a, String b) throws Exception {
		Connection cn = null;
		PreparedStatement pst = null;
		ResultSet rs = null;
		if (a == null || b == null) {
	 ...
	 * @throws Exception
	 */
	static public String[][] autorizado(String a, String b, String c, String d) throws Exception {
		Connection cn = null;
		PreparedStatement pst = null;
		ResultSet rs = null;
	...
	static public Collection getTipoDoc() {
		...
		try {
			...
			pst = cn.prepareStatement(
					"SELECT g_concodigo,g_conNombre FROM G_Constante WHERE G_ConTipo=10 ORDER BY G_ConOrden");
			rs = pst.executeQuery();
			int m = rs.getMetaData().getColumnCount();
			list = new ArrayList();
			Object[] o;
	...
	static public Collection getasistencia(String doc, String vig) {
		...
		try {
			cn = cursor.getConnection();
			pst = cn.prepareStatement(
					"select asimotivo,decode(asimotivo,null,'Sin Motivo',0,'Sin Justificar',(SELECT G_CONNOMBRE FROM G_CONSTANTE where asimotivo=G_CONCODIGO and G_CONTIPO=15 and rownum=1)) motivo,count(*) from asistencia,estudiante where estnumdoc=? and asicodperso=estcodigo and asivigencia=? group by asimotivo");
			pst.setString(1, doc);
	...
	static public String[][] getFiltroMatriz(Collection a) throws InternalErrorException {
		Object[] o;
		if (!a.isEmpty()) {
			int i = 0, j = -1;
	...
	/**
	 * Funcinn: Ejecuta un consulta en la base de datos y devuelte el
	 * resultado<br>
	 * 
	 * @param Collection
	 *            lista
	 * @return String[]
	 */
	static public String[] getFiltroArray(Collection a) throws InternalErrorException {
		if (a != null && !a.isEmpty()) {
		...
		try {
			...
			for (int i = 0; i < params.length; i++) {
				...
				case 0:
					...
					// System.out.println("ENTRO GET PERFILES POA: param1:
					// "+params[i][1]+" -param2: "+params[i][2]);
					break;
	...
	static public Collection getlogrosdesc(String ins, String vig) {
		...
		try {
			cn = cursor.getConnection();
			pst = cn.prepareStatement(
					"select nvl(insparsubtitbol,'Logros'),nvl(insparevaldescriptores,'Descriptores') from institucion_parametro where insparcodinst=? and insparvigencia=?");
			pst.setString(1, ins);
	...
	static public Collection[] ponerPerfilesTodos(String[][] params) {
		...
		try {
			...
			// ")");
			pst = cn.prepareStatement(consultas[21] + inst + ") order by INSNOMBRE ");
			...
			pst.close();
			// SEDES
			pst = cn.prepareStatement(consultas[22] + inst + ") order by SEDCODIGO ");
			pos = 1;
			for (int i = 0; i < params.length; i++) {
				jer = Long.parseLong(params[i][1]);
				niv = Long.parseLong(params[i][0]);
			...
			pst.close();
			// JORNADAS
			pst = cn.prepareStatement(consultas[23] + inst + ") ORDER BY SEDJORCODJOR");
			pos = 1;
			for (int i = 0; i < params.length; i++) {
				jer = Long.parseLong(params[i][1]);
				niv = Long.parseLong(params[i][0]);
	...
	static public Collection[] getGradoGrupo(String ins) {
		...
		try {
			...
			// JORANADAS
			pst = cn.prepareStatement(
					"select distinct SEDJORCODJOR,G_ConNombre from sede_jornada,g_constante where sedjorcodinst=? and G_ConTipo=5 and G_ConCodigo=SEDJORCODJOR order by 1");
			pst.setString(pos++, ins);
			rs = pst.executeQuery();
			list[1] = getCollection(rs);
			rs.close();
			...
			// GRADO
			pst = cn.prepareStatement(
					"SELECT distinct GRACODIGO,G_GRANOMBRE,GRACODINST FROM GRADO, G_GRADO WHERE GraCodInst=? and G_GRACODIGO=GraCodigo order by 1");
			pst.setString(pos++, ins);
			rs = pst.executeQuery();
			list[2] = getCollection(rs);
			rs.close();
			...
			pos = 1;
			pst = cn.prepareStatement(
					"select distinct GruCodigo,GruNombre,a.G_JerGrado,G_JerSede,G_JerJorn from grupo, g_jerarquia  a, estudiante e where a.G_JerTipo=1 and a.G_JerNivel=7 and a.G_JerInst=? and a.G_JerCodigo=GruCodJerar and e.estgrupo=grujergrupo order by GRUNOMBRE");
			...
			// JORNADA FILTRO
			pst = cn.prepareStatement(
					"select SEDJORCODJOR,G_ConNombre,sedjorcodsede from sede_jornada,g_constante where sedjorcodinst=? and G_ConTipo=5 and G_ConCodigo=SEDJORCODJOR order by 1");
			pst.setString(pos++, ins);
			rs = pst.executeQuery();
			...
			// GRUPO FILTRO
			pst = cn.prepareStatement(
					"select distinct GruCodigo,GruNombre,a.G_JerGrado from grupo, g_jerarquia  a where a.G_JerTipo=1 and a.G_JerNivel=7 and a.G_JerInst=? and a.G_JerCodigo=GruCodJerar order by 1");
			pst.setString(pos++, ins);
			rs = pst.executeQuery();
	 ...
	 * @return boolean
	 */
	static public Login getUsuario(String a, String b, String[][] param, String perf, String inst, String sed,
			String jor) {
		...
		try {
			...
			if (l != null) {
				...
				if (rs.next()) {
					...
					Logger.print(l.getUsuario(), "Error al traer los datos de Art_estudiante: " + l.getUsuario(), 0, 1,
							"Clase Acceso,Metodo getUsuarioEstudiante");
					return null;
				}
				rs.close();
				pst.close();
			...
			Logger.print(l.getUsuario(), "Error al traer los datos de Art_estudiante: " + e.getMessage(), 0, 1,
					"Clase Acceso,Metodo getUsuarioEstudiante");
			System.out.println("error" + e);
		} finally {
			try {
	 ...
	 * @return Login
	 */
	static public Login login4(String a, String b, String[][] param, String perf, String inst) {
		String[][] us = null;
		Connection cn = null;
		PreparedStatement pst = null;
		ResultSet rs = null;
		...
		try {
			...
			if (us != null) {
				...
				l = login4Params(l);
				l.setNivel(param[0][0]);
				// System.out.println("valor de mun:
				// "+l.getMunId()+"//"+l.getMun());
				// metodologia
				pst = cn.prepareStatement(consultas[24]);
	...
	static public Login login4Params(Login login) {
		...
		try {
			...
			long vigenciaActual = getVigenciaInst(Long.parseLong(login.getInstId()));
