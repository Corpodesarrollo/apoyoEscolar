...
public class Boletin_ extends Thread {
	...
	public Boletin_(Cursor c, String cont, String p, String p1, String p2,
			File reportF, File reportF1, int nn) {
		super("hilo_boletin " + nn);
		cursor = c;
		path = p;
		path1 = p1;
		path2 = p2;
	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				List boletines = boletin_Generar();
				Iterator bols = boletines.iterator();
				if (boletines != null && boletines.size() > 0) {
					// System.out
					// .println("BOLETINES: SI HAY BOLETINES POR GENERAR: CANTIDAD "
					// + boletines.size());
					while (bols.hasNext()) {
						DatosBoletinVO bdt = (DatosBoletinVO) bols.next();
						// System.out
						// .println("BOLETINES: BOLETIN A GENERAR CONSEC: "
						// + bdt.getDABOLCONSEC());
						...
						escudo = new File(path2 + "e" + bdt.getDANE().trim()
								+ ".gif");
						// System.out.println("escudo: " + escudo);
						if (escudo.exists()) {
							parameters.put("ESCUDO_COLEGIO", path2 + "e"
						...
						if (!process(bdt, parameters)) {
							// System.out.println("BOLETINES: PROCCES FALLO.");
							ponerReporteMensaje(
									"2",
									modulo,
									usuarioBol,
									rb3.getString("boletines.PathBoletines")
						...
						if (!updatePuestoBoletin(puest, nombreBol, usuarioBol,
								"update_puesto_boletin_2")) {
							// System.out
							// .println("BOLETINES: **NO Se actualizn el puesto del boletin en Datos_Boletin**");
							continue;
						}
						if (!update_cola_boletines()) {
							// System.out
							// .println("BOLETINES: NO Se actualizn la lista de los reportes en cola");
							continue;
						}
						// System.out.println("BOLETINES: " + getName()
						// + ":Sale While Boletines");
					}
				} else {
					Thread.sleep(dormir);
					// System.out.println("BOLETINES: NO HAY BOLETINES A GENERAR");
			...
			System.out
					.println(new Date()
							+ " - BOLETINES: EXECPCION EN HILO RUN,INTERRUPCION. SE CAYO HILO");
			ex.printStackTrace();
			ponerReporteMensaje("2", modulo, usuarioBol,
					rb3.getString("boletines.PathBoletines") + nombreBol + "",
			...
			System.out
					.println(new Date()
							+ " - BOLETINES: EXECPCION EN HILO RUN,EXCEPCION. SE CAYO HILO");
			ex.printStackTrace();
			ponerReporteMensaje("2", modulo, usuarioBol,
					rb3.getString("boletines.PathBoletines") + nombreBol + "",
					"zip", "" + nombreBol, "ReporteActualizarBoletin",
	...
	public List boletin_Generar() {
		...
		try {
			...
			pst = con.prepareStatement(rb3.getString("boletin_a_generar"));
			posicion = 1;
			pst.clearParameters();
			rs = pst.executeQuery();
			...
			// if (!listdb.isEmpty()) {
			// System.out.println("nnnnSI HAY BOLETINES POR GENERAR!!!!");
			// }
		} catch (Exception e) {
			e.printStackTrace();
			ponerReporteMensaje("2", modulo, usuarioBol,
	...
	public boolean process(DatosBoletinVO dbt, Map parameterscopy) {
		...
		try {
			...
			// System.out
			// .println("BOLETINES: NIVEL GRADO: " + dbt.getDABOLGRADO());
			...
			if (boletin == 1) {
				// System.out
				// .println("BOLETINES: ENTRO A GENERAR BOLETIN POR ASIGNATURA");
				if (!boletin(dbt.getDABOLCONSEC())) {
					// System.out
					// .println("BOLETINES: ERROR PROCEDIMIENTO BOLETIN");
					return false;
				}
				...
				if (rs.next()) {
					...
					// System.out
					// .println("BOLETINES: HAY DATOS SE MANDA EJECUTAR EL JASPER");
					long consecutivoConsultaExterna = reportesDAO.getConsecutivoConsultasExternas();
					reportesDAO.insertarConsultasExternas(consecutivoConsultaExterna,"", "", "", "BOL");
					...
					if ((reportFile.getPath() != null)
							...
							&& (con != null)) {
						// System.out
						// .println("BOLETINES:  SE MANDO EJECUTAR JASPER");
					...
					archivosalida = Ruta.get(context,
							r.getString("boletines.PathBoletin"));
					...
					if (zip.ponerZip(archivosalida, nombreBol, zise, list)) {
						...
						reportesDAO.updateConsultasExternas(consecutivoConsultaExterna, archivosalida, nombreBol, "zip", "BOL");
				...
				// System.out
				// .println("BOLETINES: ENTRO A GENERAR BOLETIN POR DIMENSIONES ** ");
				if (!boletin_dim(dbt.getDABOLCONSEC())) {
					// System.out
					// .println("BOLETINES: ERROR PROCEDIMIENTO DIMENSIONES **");
					return false;
				}
				// VALIDAR SI NO HUBO ERROR EN EL PROC
				if (validarEstadoBol(dbt.getDABOLCONSEC()) == 2)
				...
				if (rs.next()) {
					...
					// System.out
					// .println("BOLETINES: HAY DATOS SE MANDA EJECUTAR EL JASPER DIMENSIONES");
					long consecutivoConsultaExterna = reportesDAO.getConsecutivoConsultasExternas();
					reportesDAO.insertarConsultasExternas(consecutivoConsultaExterna,"", "", "", "BOL");
					String pinConsultaExterna = "BOL"+consecutivoConsultaExterna;
					parameterscopy.put("PINCONSULTAEXTERNA", pinConsultaExterna);
					if ((reportFile1.getPath() != null)
							...
							&& (con != null)) {
						// System.out
						// .println("BOLETINES:  SE MANDO EJECUTAR JASPER DIMENSIONES");
					...
					if (zip.ponerZip(archivosalida, nombreBol, zise, list)) {
						...
						updateReporte(nombreBol, usuarioBol, "1");
						reportesDAO.updateConsultasExternas(consecutivoConsultaExterna, archivosalida, nombreBol, "zip", "BOL");
						//siges.util.Logger.pint(usuarioBol,"Se genern el Boletin:_Institucion:_"+filtro.getInsitucion()+"_Usuario:_"+usuarioBol+"_NombreBoletin:_"+nombreBol+"",3,1,this.toString());
						limpiarTablas(consecBol);
						return true;
					}
				}// fin de rs
	...
	public boolean boletin(long consec) {
		...
		try {
			if (consec > 0) {
				...
				// System.out.println("BOLETINES: ASIGNATURA - consec: "
				// + consec
				...
				cstmt.executeUpdate();
				// System.out
				// .println("BOLETINES: ASIGNATURA - Fin procedimiento Hora: "
	...
	public boolean boletin_dim(long consec) {
		...
		try {
			if (consec > 0) {
				...
				cstmt.setLong(posicion++, consec);
				// System.out.println("BOLETINES: DIMENSIONES **  - consec "
				// + consec
				// + " Inicia procedimiento Hora: "
				// + new java.sql.Timestamp(System.currentTimeMillis())
				// .toString());
				cstmt.executeUpdate();
				// System.out
				// .println("BOLETINES: DIMENSIONES ** - Fin procedimiento Hora: "
				// + new java.sql.Timestamp(System
				// .currentTimeMillis()).toString());
				cstmt.close();
			}
	...
	public int validarEstadoBol(long consec) {
		...
		try {
			...
			e.printStackTrace();
			// ponerReporteMensaje("2",modulo,usuarioBol,rb3.getString("boletines.PathBoletines")+nombreBol+"","zip",""+nombreBol,"ReporteActualizarBoletinPaila","Ocurrin excepcinn prepareds:_"+e);
			// updateDatosBoletin("2",nombreBol,"0",usuarioBol);
			// updateReporte(nombreBol,usuarioBol,"2");
			limpiarTablas(consecBol);
			System.out
	...
	public boolean update_cola_boletines() {
		...
		try {
			con = cursor.getConnection();
			pst = con
					.prepareStatement(rb3.getString("update_puesto_boletin_1"));
			posicion = 1;
			pst.clearParameters();
			pst.executeUpdate();
			con.commit();
