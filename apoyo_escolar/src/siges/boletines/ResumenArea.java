...
public class ResumenArea extends Thread {
	...
	public ResumenArea(Cursor c, String cont, String p, String p1, String p2,
			File reportF, File reportF1, int nn) {
		super("hilo_ResumenArea " + nn);
		cursor = c;
		path = p;
		// path1=p1;
		path2 = p2;
		...
		try {
			...
			while (true) {
				...
				if (resumenes != null && resumenes.size() > 0) {
					// System.out
					// .println("RESUMEN AREA: SI HAY RESUMENES POR GENERAR: CANTIDAD "
					// + resumenes.size());
					while (bols.hasNext()) {
						DatosBoletinVO bdt = (DatosBoletinVO) bols.next();
						// System.out
						// .println("RESUMEN AREA: RESUMEN A GENERAR CONSEC: "
						// + bdt.getDABOLCONSEC());
						...
						//
						// System.out.println("ESTUDOS " + path2 + "e"
						...
						if (!process(bdt, parameters)) {
							// System.out.println("RESUMEN AREA: PROCCES FALLO.");
						...
						if (!updatePuestoBoletin(puest, nombreBol, usuarioBol,
							...
							// System.out
							// .println("RESUMEN AREA: **NO Se actualizn el puesto del resumen en Datos_Boletin**");
							continue;
						}
						if (!update_cola_boletines()) {
							// System.out
							// .println("RESUMEN AREA: NO Se actualizn la lista de los reportes en cola");
							continue;
						}
						System.out.println("RESUMEN AREA: " + getName()
								+ ":Sale While resumenes");
					}
				} else {
					Thread.sleep(dormir);
					// System.out
					// .println("RESUMEN AREA: NO HAY RESUMENES AREA A GENERAR");
			...
			System.out
					.println(new Date()
							+ " - RESUMEN AREA: EXECPCION EN HILO RUN,INTERRUPCION. SE CAYO HILO");
			ex.printStackTrace();
			ponerReporteMensaje("2", modulo, usuarioBol,
			...
			System.out
					.println(new Date()
							+ " - RESUMEN AREA: EXECPCION EN HILO RUN,EXCEPCION. SE CAYO HILO");
			ex.printStackTrace();
			ponerReporteMensaje("2", modulo, usuarioBol,
					rb3.getString("resumenes.PathResumenes") + nombreBol + "",
					"zip", "" + nombreBol, "ReporteActualizarBoletin",
	...
	public List resumen_Generar() {
		...
		try {
			...
			// if (!listdb.isEmpty()) {
			// System.out
			// .println("RESUMEN AREAS:nnnnSI HAY RESUMENES AREA POR GENERAR!!!!");
			// }
		} catch (Exception e) {
			e.printStackTrace();
			ponerReporteMensaje("2", modulo, usuarioBol,
	...
	public boolean process(DatosBoletinVO dbt, Map parameterscopy) {
		...
		try {
			...
			// System.out.println("RESUMEN AREA: NIVEL GRADO: "
			// + dbt.getDABOLGRADO());
			...
			if (boletin == 1) {
				// System.out
				// .println("RESUMEN AREA: ENTRO A GENERAR RESUMEN POR ASIGNATURA");
				if (!boletin(dbt.getDABOLCONSEC())) {
					// System.out
					// .println("RESUMEN AREA: ERROR PROCEDIMIENTO RESUMEN");
					return false;
				}
				...
				if (rs.next()) {
					...
					// System.out
					// .println("RESUMEN AREA: HAY DATOS SE MANDA EJECUTAR EL JASPER");
					String rutaXSL = "";
					...
					// System.out.println("RESUMEN AREA NOMBRE  ** ARCHIVO XLS: "
					// + rutaXSL);
				...
				// System.out
				// .println("RESUMEN AREA: ENTRO A GENERAR RESUMEN AREA POR DIMENSIONES ** ");
				if (!boletin_dim(dbt.getDABOLCONSEC())) {
					// System.out
					// .println("RESUMEN AREA: ERROR PROCEDIMIENTO DIMENSIONES **");
					return false;
				}
				// VALIDAR SI NO HUBO ERROR EN EL PROC
				if (validarEstadoBol(dbt.getDABOLCONSEC()) == 2)
				...
				if (rs.next()) {
					...
					// System.out
					// .println("RESUMEN AREA: HAY DATOS SE MANDA EJECUTAR EL JASPER DIMENSIONES");
					if ((reportFile1.getPath() != null)
							&& (parameterscopy != null)
							&& (!parameterscopy.values().equals("0"))
							&& (con != null)) {
						// System.out
						// .println("RESUMEN AREA:  SE MANDO EJECUTAR JASPER DIMENSIONES");
	...
	public boolean boletin_dim(long consec) {
		...
		try {
			if (consec > 0) {
				...
				cstmt.executeUpdate();
				// System.out
				// .println("RESUMEN AREA: DIMENSIONES ** - Fin procedimiento Hora: "
				// + new java.sql.Timestamp(System
				// .currentTimeMillis()).toString());
				cstmt.close();
			}
	...
	public void updateReporte(String nombreboletin, String user, String estado) {
		...
		try {
			...
			pst = con.prepareStatement(rb3
					.getString("update_reporte_resumen_area"));
			posicion = 1;
			pst.clearParameters();
			pst.setString(posicion++, (nombreboletin));
			pst.setString(posicion++, (user));
	...
	public boolean update_cola_boletines() {
		...
		try {
			con = cursor.getConnection();
			pst = con.prepareStatement(rb3
					.getString("update_puesto_resumenArea_1"));
			posicion = 1;
			pst.clearParameters();
			pst.executeUpdate();
			con.commit();
