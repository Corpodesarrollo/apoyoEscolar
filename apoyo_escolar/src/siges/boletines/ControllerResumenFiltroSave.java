...
public class ControllerResumenFiltroSave extends HttpServlet {
	...
	public void setMensaje(String s) {
		if (!err) {
			err = true;
			mensaje = "VERIFIQUE LA SIGUIENTE informaci髇: \n\n";
		}
		mensaje = "  - " + s + "\n";
	}
	...
	public boolean resumen_Area_Asig(HttpServletRequest request, String vigencia, String contextoTotal)
		...
		try {
			...
			if (!rs.next()) {
				...
				// System.out
				// .println("Se insertn el ZIP en Reporte con estado -1");
				siges.util.Logger.print(login.getUsuarioId(), "Petici髇 de Resumen_Area/Asignatura:_Institucion:_" + login.getInstId()+ "_Usuario:_" + login.getUsuarioId() + "_NombreReporte:_" + archivozip + "",3, 1, this.toString());
				...
				rs.close();
				pst.close();
				setMensaje("nYa existe una petici髇 de este reporte con los mismos par醡etros!");
				request.setAttribute("mensaje", mensaje);
				request.setAttribute("mensaje2", "Ya existe una petici髇 de este reporte con los mismos par醡etros!");
				return false;
			}
