...
public class ControllerResumenArea  extends HttpServlet{
...
public void init(ServletConfig config) throws ServletException{
    ...
    rb=ResourceBundle.getBundle("siges.boletines.bundle.boletines");
 	String contextoTotal=context.getRealPath("/");
  	...
  	reportFile=new File(path+rb.getString("repResumenArea"));	
  	reportFile1=new File(path+rb.getString("repreResumenArea"));
  	/*reportFile5=new File(path+rb.getString("rep5"));	
  	reportFile5_11=new File(path+rb.getString("rep5_11"));	
	reportFile1=new File(path+rb.getString("repre"));
	reportFile2=new File(path+rb.getString("repre5"));
	reportFileLogros=new File(path+rb.getString("logros"));
	reportFileninguno=new File(path+rb.getString("ninguno"));
	reportFileDescriptores=new File(path+rb.getString("descriptores"));
	reportFileLogros5=new File(path+rb.getString("logros5"));
	reportFileLogros5_11=new File(path+rb.getString("logros5_11"));
	reportFilePreescolar_sin_img=new File(path+rb.getString("repPre_sin_img"));
	reportFilePreescolar5_sin_img=new File(path+rb.getString("repPre5_sin_img"));
	reportFileNuevoFormato=new File(path+rb.getString("reportFileNuevoFormato"));
		*/
  	int n=1;
	  System.out.println("RESUMEN AREAS: APOYO ESCOLAR 2010 INICIANDO...");
	  super.init(config);
	  cursor=new Cursor();
	...
	**/
    ...
    Connection con=null;
  	...
  	try{
		  ...
		  pst=con.prepareStatement(rb.getString("actualizar_resumenAreas_en_cola"));
			posicion=1;
		 	pst.executeUpdate();
		 	pst.close();
		 	//System.out.println("actualizn en cola los resumenes areas generando");
		 	con.commit();
		 	}
  	 catch(Exception e ){
