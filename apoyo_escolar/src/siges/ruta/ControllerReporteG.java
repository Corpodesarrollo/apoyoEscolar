...
public class ControllerReporteG  extends HttpServlet{
  ...
  public void init(ServletConfig config) throws ServletException{
    ...
    rb=ResourceBundle.getBundle("siges.ruta.reportes_ruta");
  	...
  	reportFile=new File(path+"/"+rb.getString("jasper_gestacion"));	
  	int n=1;
	  System.out.println("GESTACION INICIANDO");
	  super.init(config);
	  cursor=new Cursor();
	  reporteDao=new ReporteDAO(cursor);
	  
		if (!reporteDao.updateColaReportesEstadoCero("actualizar_reportes_en_cola",2)){
	 		   System.out.println("n****NO ACTUALIZn LOS REPORTES Q ESTABAN EN ESTADO CERO.. A ESTADO -1****!");
	 		   return;
	 		}	  
	  t=new Thread(new ReporteRutaG(cursor,contextoTotal,path,path1,reportFile,n++));	  
	  t.start();
	  return;
  }		
  ...
  public void destroy(){
    try{
      t.stop();
  	  System.out.println("GESTACION FINALIZANDO");
    }catch(Exception ex){}
  }
