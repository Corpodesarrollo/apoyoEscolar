...
public class ReporteRuta extends Thread {
	...
	private final int tipoN = 1;

	/* Constructor de la clase */

	public ReporteRuta(Cursor c, String cont, String p, String p1,
			File reportF, int nn) {
		super("hilo_nutricion" + nn);
		cursor = c;
		path = p;
		path1 = p1;
		reportFile = reportF;
	...
	public void run() {
		...
		try {
			...
			reporteDao.updateDatosReporte("2", filtro.getNombrereportezip(),
					"-1", filtro.getUsuarioid(), "update_datos_reporte", 1);
			reporteDao.updateDatosReporteFechaFin(
					new java.sql.Timestamp(System.currentTimeMillis())
							.toString(), "2", filtro.getNombrereportezip(),
					filtro.getUsuarioid(), "update_datos_reporte_fecha_fin", 1);
	...
	public boolean procesoJasper(FiltroBeanVO filtro, Map parameterscopy) {
		...
		try {
			...
			reporteDao.updateDatosReporteFechaGen(
					...
					filtro.getUsuarioid(), "update_datos_reporte_fecha_gen", 1);

			if (!reporteDao.callableStatements(filtro, vigencia, modulo)) {
				System.out
			...
			if (reporteDao.ejecutarJasper(filtro.getUsuarioid(),
					"queryEjecutarjasper")) {
				System.out.println("SE DEBE EJECUTAR EL JASPER DE NUTRICION");
				if (reporteDao.getExcel(filtro, reportFile, parameterscopy,
						modulo, context)) {
					reporteDao.ponerReporteMensajeListo(
							modulo,
							filtro.getUsuarioid(),
			...
			reporteDao.ponerReporteMensaje(
					...
					"ReporteActualizarBoletinGenerando", "Ocurrio Exception:_"
							+ e);
			reporteDao.updateDatosReporte("2", filtro.getNombrereportezip(),
					"0", filtro.getUsuarioid(), "update_datos_reporte", 1);
			reporteDao.updateDatosReporteFechaFin(
					new java.sql.Timestamp(System.currentTimeMillis())
							.toString(), "2", filtro.getNombrereportezip(),
					filtro.getUsuarioid(), "update_datos_reporte_fecha_fin", 1);
