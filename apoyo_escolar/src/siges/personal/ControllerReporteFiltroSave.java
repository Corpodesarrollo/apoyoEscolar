...
public class ControllerReporteFiltroSave extends HttpServlet {
	...
	private String s1 = "/estudiante/ControllerReporteFiltroEdit.do";
	...
	public String process(HttpServletRequest request,
		...
		String nom;
		ant = "/personal/filtroReporte.jsp";
		sig = "/personal/filtroReporte.jsp";
		er = "/error.jsp";
		home = "/bienvenida.jsp";
		...
		try {
			...
			pst.setLong(posicion++, Long.parseLong(!filtro.getPersonal()
					.equals("-9") ? filtro.getPersonal() : "-9"));
			rs = pst.executeQuery();
			if (!rs.next()) {
				...
				pst = con.prepareStatement(rb3.getString("hay_registros"));
				posicion = 1;
				pst.clearParameters();
				pst.setLong(posicion++, Long.parseLong(login.getInstId()));
				...
				if (rs.next()) {
					...
					persona = (!filtro.getPersonal().equals("-9") ? "_Doc_"
							+ filtro.getPersonal() : "");
					...
					nom = sede
							...
							+ persona
							+ "_Fecha_"
							+ f2.toString().replaceAll(" ", "_")
									.replace(':', '-').replace('.', '-');
					archivo = "Hojas_de_vida_personal_" + nom + ".pdf";
					archivozip = "Hojas_de_vida_personal_" + nom + ".zip";

					if (!reporteDao.insertarDatosReporte(filtro,
							login.getInstId(), f2, archivozip, archivo,
							login.getUsuarioId())) {
						setMensaje("No se insertn en DATOS_REPORTE_PERSONAL");
						request.setAttribute("mensaje", mensaje);
						return s1;
					}
					// System.out
					// .println("Se insertn el ZIP en Reporte con estado -1");
					if (!reporteDao.ponerReporte(modulo, login.getUsuarioId(),
							rb3.getString("reportes.PathReportes") + archivozip
									+ "", "zip", "" + archivozip, "-1",
							"ReporteInsertarEstado")) {
					...
					siges.util.Logger.print(
							login.getUsuarioId(),
							"Peticinn de Reporte_Hoja_vida_personal:Institucion:_"
					...
					if (cont != null && !cont.equals(""))
						if (!reporteDao.updatePuestoReporte(cont, archivozip,
								login.getUsuarioId(), "update_puesto_reporte")) {
					...
					rs.close();
					pst.close();
					setMensaje("nNo se encontrarnn registros de personal para la solicitud!");
					request.setAttribute("mensaje", mensaje);
					return s1;
				}
			...
			reporteDao.ponerReporteMensaje("2", modulo, login.getUsuarioId(),
					...
					"Ocurrin excepcinn ControllerFiltroSave");
		} finally {
			try {
				if (cursor != null)
					cursor.cerrar();
	...
	public boolean asignarBeans(HttpServletRequest request)
		...
		filtro = (FiltroBeanForm) session.getAttribute("filtroPers");
		String sentencia = "";
		return true;
	}
