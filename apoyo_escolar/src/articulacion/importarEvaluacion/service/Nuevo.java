...
import articulacion.importarEvaluacion.service.Excel;
...
public class Nuevo extends Service {
	...
	public String[] process(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// String
		// path=rb.getString("importarEvaluacionArt.pathSubir")+"."+login.getUsuarioId();
		...
		try {
			...
			nombre[0] = val[2];
			nombre[1] = val[3];
			String resultado = validarArchivo(request, tipo, login, nombre, importarADAO);
			if (resultado != null) {
	...
	public String[] subirArchivo(javax.servlet.http.HttpServletRequest request) throws ServletException, IOException {
		String[] a = new String[4];
		int tipo = -1;
		Login login = (Login) request.getSession().getAttribute("login");
		rb = ResourceBundle.getBundle("articulacion.importarEvaluacion.bundle.sentencias");
		String path = rb.getString("importarEvaluacionArt.pathSubir") + "." + login.getUsuarioId();
		String pathSubir = Ruta.get(context, path);
		String pathTemporal = Ruta.get(context, rb.getString("importarEvaluacionArt.temporal"));
		String fileName = "";
...
//			fileItems = fu.parseRequest(request, 4096, 1024 * 1024, pathTemporal);
			...
			// :"+e.toString());
			// return (tipo*-1);
			a[1] = "VERIFIQUE LA SIGUIENTE INFORMACION: \n\n  - " + "El archivo no tiene un tamano especificado :" + e.toString() + "\n";
			a[0] = "" + (tipo * -1);
			e.printStackTrace();
	...
	public String validarArchivo(HttpServletRequest request, int tipo, Login login, String[] nombre, ImportarArticulacionDAO importarADAO) throws ServletException, IOException {
		...
		String[] params = { login.getInstId(), login.getSedeId(), login.getJornadaId(), login.getUsuarioId() };
		path = rb.getString("estudianteEvaluacionArt.PathDownload") + "." + login.getUsuarioId();// ruta
																									// de
																									// carpetas
		relativo = Ruta.getRelativo(context, path);
		pathPlantilla = Ruta2.get(context, rb.getString("estudianteEvaluacionArt.PathPlantilla")) + rb.getString("estudianteEvaluacionArt.NombrePlantilla");// archivo
																																							// deplantilla
		pathDownload = Ruta.get(context, path);// path del nuevo archivo
		// validar estructura del archivo
		if (!excel.validarFormatoEstudiante(tipo, nombre, pathPlantilla, pathDownload, params, login.getUsuarioId(), login.getPerfil())) {
			if (!excel.getError()) {
				ponerReporte(login.getUsuarioId(), relativo + nombre[1], "xls", "Inconsistencias en importacinn de evaluacinn de Estudiantes: ", 10);
				request.setAttribute("plantilla", relativo + nombre[1]);
			}
			return "VERIFIQUE LA SIGUIENTE INFORMACION: \n\n  - " + excel.getMensaje() + "\n";
		}
		// importar archivo/
		ResultadoVO res = excel.importarEstudiante(tipo, nombre, pathPlantilla, pathDownload, login.getUsuarioId(), login.getMetodologiaId());
		if (res == null) {
			if (!excel.getError()) {
				request.setAttribute("plantilla", relativo + nombre[1]);
			}
			...
			if (!item.isFormField()) {
				...
				if (fileName.trim().compareTo("") != 0) {
					...
					if (!aceptado) {
						...
						a += "1. No es una hoja de calculo de Microsoft Excel (" + tipoa + ")\n";
	...
	public String getNombre(int tipo) {
		String nom = "";
		nom = "Inconsistencias_Evaluacion_Estudiantes" + f2.toString().replace(':', '-').replace('.', '-') + ".xls";
