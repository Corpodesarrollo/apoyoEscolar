...
public class ControllerFiltroEdit extends HttpServlet {
	...
	private FiltroBeanReporte filtro;
	private FiltroBeanReporteDocente filtroDoc;
	private FiltroBeanReporteEspacio filtroEsp;
	private String sede;
	...
	public String process(HttpServletRequest request,
		...
		String home;
		int tipo;
		sig = "/articulacion/reporteHorario/FiltroGrupo.jsp";
		sig2 = "/articulacion/reporteHorario/FiltroDocente.jsp";
		sig3 = "/articulacion/reporteHorario/FiltroEspacioFisico.jsp";
		ant = "/index.jsp";
		er = "/error.jsp";
		home = "/bienvenida.jsp";
		err = false;
		...
		try {
			...
			if (request.getParameter("tipo") == null
				...
				session.removeAttribute("filtroEsp");
				tipo = 1;
			} else
			...
			case 1:
				...
				break;
			case 2:
				filtroDocente(request);
				sig = sig2;
				break;
			case 3:
				filtroEspacio(request);
				sig = sig3;
				break;
			}
	...
	public void filtroGrupo(HttpServletRequest request)
			throws ServletException, IOException {

		try {
			borrarBeansGrupo(request);
			// System.out.println("*********////*********nfiltroGrupo!**********//**********");
			Collection list;
			Object[] o;
			String s;
			int z;
	...
	public void filtroDocente(HttpServletRequest request)
			throws ServletException, IOException {
		Connection con = null;
		...
		try {
			...
			if (request.getAttribute("filtroDocenteF") == null)
				request.setAttribute("filtroDocenteF",
						util.getFiltro(rb.getString("lista_docentes"), list));

			if (request.getAttribute("filtroEstructuraF") == null)
				request.setAttribute("filtroEstructuraF",
						util.getFiltro(rb.getString("lista_estructuras"), list));
	...
	public void filtroEspacio(HttpServletRequest request)
			throws ServletException, IOException {
		Connection con = null;
		PreparedStatement pst = null;
		ResultSet rs = null;
		...
		try {
			...
			if (request.getAttribute("filtroTipoEspacioF") == null)
				request.setAttribute("filtroTipoEspacioF",
						util.getFiltro(rb.getString("listaTipoEspacio")));
			...
			if (request.getAttribute("filtroEspacioF") == null)
				request.setAttribute("filtroEspacioF", util.getFiltro(
						rb.getString("listaEspacioInstitucion"), list));
	...
	public boolean asignarBeans(HttpServletRequest request)
		...
		filtro = (FiltroBeanReporte) session.getAttribute("filtro");
		filtroDoc = (FiltroBeanReporteDocente) session
				.getAttribute("filtroDoc");
		filtroEsp = (FiltroBeanReporteEspacio) session
	 ...
	 *            request
	 */
	public void borrarBeansGrupo(HttpServletRequest request)
			throws ServletException, IOException {
		session.removeAttribute("filtro");
	}
	 ...
	 *            request
	 */
	public void borrarBeansDocente(HttpServletRequest request)
			throws ServletException, IOException {
		session.removeAttribute("filtroDoc");
	}
	 ...
	 *            request
	 */
	public void borrarBeansEspacio(HttpServletRequest request)
			throws ServletException, IOException {
		session.removeAttribute("filtroEsp");
	}
