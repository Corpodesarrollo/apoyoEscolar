package articulacion.repEstudiantesArt.dao;
 ...
 * 
 */
public class repEstArtDAO extends Dao {
	...
	public repEstArtDAO(Cursor cur) {
		super(cur);
		bytes = null;
		rb3 = ResourceBundle
	...
	public boolean isEmptyEstXTutor(
			FiltroBeanReporteEstXTutor filtroBeanReporteTutor) throws Exception {
		Connection con = null;
		PreparedStatement pst = null;
		ResultSet rs = null;
		int posicion = 1;
		try {
			con = cursor.getConnection();
			rs = null;
			pst = con.prepareStatement(rb3.getString("isEmptyEstXTutor"));
			posicion = 1;
			pst.setLong(posicion++, filtroBeanReporteTutor.getInstitucion()
					.longValue());
			pst.setLong(posicion++, filtroBeanReporteTutor.getSede()
					.longValue());
			pst.setLong(posicion++, filtroBeanReporteTutor.getJornada()
					.longValue());
			pst.setString(posicion++, String.valueOf(filtroBeanReporteTutor
					.getDocente().longValue()));
			pst.setString(posicion++, String.valueOf(filtroBeanReporteTutor
					.getDocente().longValue()));
			rs = pst.executeQuery();
			if (rs.next())
