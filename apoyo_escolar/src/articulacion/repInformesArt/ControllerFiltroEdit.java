...
public class ControllerFiltroEdit extends HttpServlet {
	...
	public String process(HttpServletRequest request,
		...
		String home;
		int tipo;
		sig = "/articulacion/repInformesArt/FiltroConstancias.jsp";
		ant = "/index.jsp";
		er = "/error.jsp";
		home = "/bienvenida.jsp";
		err = false;
		try {
			...
			switch (tipo) {
			case 1:
				filtroConstancias(login, session, request);
				break;
	...
	public void filtroConstancias(Login login, HttpSession session,
			HttpServletRequest request) throws ServletException, IOException {

		try {
			borrarBeansConstancia(session, request);
			...
			// SEDE
			request.setAttribute("lSedeVO", repInfArt.getSede(inst, sede));
			// JORNADA
			request.setAttribute("lJornadaVO",
					repInfArt.getJornada(inst, sede, jor));
			// ESPECIALIDAD
			request.setAttribute("lEspecialidadVO",
					repInfArt.getAjaxEspecialidad(inst));
		} catch (Throwable th) {
			th.printStackTrace();
	 ...
	 *            request
	 */
	public void borrarBeansConstancia(HttpSession session,
			HttpServletRequest request) throws ServletException, IOException {
		session.removeAttribute("filtroC");
	}
