package articulacion.repPlanEstudiosArt.dao;
...
import siges.io.Zip;
import articulacion.repPlanEstudiosArt.vo.FiltroBeanReporteAsignacion;
import articulacion.repPlanEstudiosArt.vo.FiltroBeanReportePlan;
 ...
 * 
 */
public class repPlanArtDAO extends Dao {
	...
	public repPlanArtDAO(Cursor cur) {
		super(cur);
		bytes = null;
		rb3 = ResourceBundle
	...
	public boolean isEmptyPlan(FiltroBeanReportePlan filtroBeanReportePlan)
			throws Exception {
		Connection con = null;
		PreparedStatement pst = null;
		ResultSet rs = null;
		...
		try {
			con = cursor.getConnection();
			rs = null;
			pst = con.prepareStatement(rb3.getString("isEmptyPlan"));
			posicion = 1;
			pst.setLong(posicion++, filtroBeanReportePlan.getInstitucion()
					.longValue());
			pst.setLong(posicion++, filtroBeanReportePlan
					.getFilAnoVigenciaArt().longValue());
			pst.setLong(posicion++, filtroBeanReportePlan
					.getFilPerVigenciaArt().longValue());
			pst.setLong(posicion++, filtroBeanReportePlan.getFilComponenteArt()
					.longValue());
			pst.setLong(posicion++, filtroBeanReportePlan.getFilSemestre()
					.longValue());
			pst.setLong(posicion++, filtroBeanReportePlan.getFilEspecialidad()
					.longValue());
			pst.setLong(posicion++, filtroBeanReportePlan.getFilEspecialidad()
					.longValue());
			rs = pst.executeQuery();
	...
	// FUNCION PARA VERIFICAR DATOS EN REPORTE ASIGNACION ACADEMICA
	public boolean isEmptyAsignacion(
			FiltroBeanReporteAsignacion filtroBeanReporteAsig) throws Exception {
		Connection con = null;
		PreparedStatement pst = null;
		ResultSet rs = null;
		int posicion = 1;
		try {
			con = cursor.getConnection();
			rs = null;
			pst = con.prepareStatement(rb3.getString("isEmptyAsignacion"));
			posicion = 1;
			pst.setLong(posicion++, filtroBeanReporteAsig.getInstitucion()
					.longValue());
			pst.setLong(posicion++, filtroBeanReporteAsig
					.getFilAnoVigenciaArt().longValue());
			pst.setLong(posicion++, filtroBeanReporteAsig
					.getFilPerVigenciaArt().longValue());
			pst.setLong(posicion++, filtroBeanReporteAsig.getSede().longValue());
			pst.setLong(posicion++, filtroBeanReporteAsig.getJornada()
					.longValue());
